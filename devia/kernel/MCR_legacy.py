...
# Força UTF-8 na saída do terminal (evita "são" → "s?o")
...
class MCR:
    ...
    def predizer_com_entropia(self, a, ent_pi_ref=None):
        """Prediz com temperatura derivada da entropia normalizada por PI."""
        a = str(a)
        if a not in self.transicoes or not self.transicoes[a]:
            return (None, 0.0)
        ent_pi = ent_pi_ref if ent_pi_ref else 3.2573
        temperatura = min(1.0, self.entropia(a) / max(ent_pi, 0.01))
...
class MCRSuperposicao:
    ...
    def superposicao_lazy(self, nivel_a, valor_a, nivel_b, valor_b, mk_a=None, mk_b=None):
        """Retorna um estado de superposicao que NAO colapsa ate observar().
        
        Diferenca de colidir(): nao escolhe o melhor resultado na hora.
        Mantem TODOS os resultados possiveis. O colapso ocorre APENAS
        quando .observar() e chamado — e usa a entropia do MOMENTO
        da leitura para decidir qual caminho seguir.
        """
        self.total += 1
        pred_a = None; conf_a = 0.0
        if mk_a: pred_a, conf_a = mk_a.predizer(valor_a)
        pred_b = None; conf_b = 0.0
        if mk_b: pred_b, conf_b = mk_b.predizer(valor_b)
        
        resultados = {}
        if pred_a and pred_b:
            for ctx_nivel, ctx_valor in [(nivel_a, valor_a), (nivel_b, valor_b)]:
                r, c = self.coupling.esfera.predizer_cross(ctx_nivel, **{ctx_nivel: ctx_valor})
                if r and c > 0: resultados[r] = resultados.get(r, 0) + c
        if not resultados:
            if pred_a: resultados[pred_a] = conf_a
            if pred_b: resultados[pred_b] = conf_b
        if not resultados:
            for ctx_nivel in ["byte", "palavra", "token_tipo"]:
                if ctx_nivel not in [nivel_a, nivel_b]:
                    r, c = self.coupling.esfera.predizer_cross(nivel_a, **{ctx_nivel: valor_a})
                    if r and c > 0.1: resultados[r] = c; break
        
        return SuperposicaoState(resultados, self.mk_colisoes, self.total,
                                 f"{nivel_a}({valor_a}) x {nivel_b}({valor_b})")


class SuperposicaoState:
    """Estado de superposicao — so colapsa quando observar() e chamado.
    
    A entropia do CONTEXTO no momento do colapso decide o resultado.
    """
    def __init__(self, resultados, mk_colisoes, total, desc):
        self.resultados = resultados
        self.mk_colisoes = mk_colisoes
        self.total = total
        self.desc = desc
        self.colapsado = False
        self.resultado_final = None
    
    def observar(self, entropia_contexto=None):
        """Colapsa a superposicao.
        
        Se entropia_contexto for fornecida, usa para ponderar a escolha.
        Alta entropia = exploracao (escolhe mais aleatorio).
        Baixa entropia = deterministico (escolhe o mais provavel).
        """
        if self.colapsado:
            return self.resultado_final
        
        if not self.resultados:
            self.resultado_final = (None, 0.0, {"colisao": "vazia"})
            self.colapsado = True
            return self.resultado_final
        
        if entropia_contexto is not None and entropia_contexto > 0.01:
            # Temperatura derivada da entropia do contexto
            temp = min(1.0, entropia_contexto / 3.2573)
            if temp > 0:
                palavras = list(self.resultados.keys())
                pesos = list(self.resultados.values())
                soma = sum(pesos)
                pesos_norm = [p/soma for p in pesos]
                pesos_temp = [p ** (1.0/max(temp, 0.01)) for p in pesos_norm]
                total_temp = sum(pesos_temp)
                probs = [p/total_temp for p in pesos_temp]
                escolhida = _rand.choices(palavras, weights=probs, k=1)[0]
                conf = probs[palavras.index(escolhida)]
                self.resultado_final = (escolhida, round(conf, 3), {"colisao": self.desc, "total": self.total})
                self.colapsado = True
                self.mk_colisoes.aprender(f"COL:{self.desc[:30]}", str(escolhida)[:20])
                return self.resultado_final
        
        # Deterministico: escolhe o melhor
        melhor = max(self.resultados, key=self.resultados.get)
        conf = self.resultados[melhor]
        self.resultado_final = (melhor, round(conf, 3), {"colisao": self.desc, "total": self.total})
        self.colapsado = True
        return self.resultado_final
...
class MCRCoupling:
    ...
    def registrar_nivel(self, nome):
        """Registra dinamicamente um novo nivel no coupling."""
        if nome not in self.niveis:
            self.niveis.append(nome)
            for o in self.niveis:
                if o not in self.matriz: self.matriz[o] = {}
                self.matriz[o][nome] = 0.0
                if nome not in self.matriz: self.matriz[nome] = {}
                self.matriz[nome][o] = 0.0
            for o in self.niveis:
                if o not in self.cooc: self.cooc[o] = {}
                self.cooc[o][nome] = 0
                if nome not in self.cooc: self.cooc[nome] = {}
                self.cooc[nome][o] = 0
...
class MCRGenesis:
    ...
    def aprender_de_arquivos(self, diretorio, max_arquivos=50):
        """Aprende padroes byte de arquivos reais no diretorio.
        
        Cada arquivo e lido como sequencia de bytes. As transicoes
        sao aprendidas no mk_byte global. O sistema descobre padroes
        estruturais (inicio, meio, fim) de QUALQUER tipo de arquivo.
        """
            ...
            for arq in arquivos:
                ...
                try:
                    with open(caminho, 'rb') as f:
                        dados = f.read(10000)
                    if len(dados) > 50:
                        # Aprende transicoes de byte
                        for i in range(len(dados)-1):
                            self.cerebro.mk_byte.aprender(
                                f"B:{dados[i]:02x}", f"B:{dados[i+1]:02x}")
                        # Aprende padrao de inicio (primeiros 20 bytes)
                        inicio_bytes = dados[:20]
                        for i in range(min(5, len(inicio_bytes)-1)):
                            self.cerebro.mk_byte.aprender(
                                f"INI:{i}", f"B:{inicio_bytes[i]:02x}")
                        # Aprende padrao de fim (ultimos 10 bytes)
                        fim_bytes = dados[-10:]
                        for i in range(min(3, len(fim_bytes)-1)):
                            self.cerebro.mk_byte.aprender(
                                f"FIM:{i}", f"B:{fim_bytes[i]:02x}")
                        n += 1
                except: pass
            if n >= max_arquivos: break
        return n
    
    def gerar_estrutura(self, semente_byte="B:00", n_variacoes=5):
        """Gera N variacoes de estrutura aprendida dos arquivos."""
        mk = self.cerebro.mk_byte
        geracoes = []
        for i in range(n_variacoes):
            seq = mk.gerar_com_entropia(semente_byte, passos=30)
            if len(seq) >= 5:
                bytes_lista = []
                for t in seq:
                    if t.startswith('B:'):
                        try: bytes_lista.append(int(t.split(':')[1], 16))
                        except: pass
                if bytes_lista:
                    geracoes.append(bytes(bytes_lista))
        return geracoes
...
class MCRParserMinimo:
    ...
    def _classificar(self, palavra, pos=0):
        """Classifica palavra por entropia — sem GOFAI, sem listas."""
        p = palavra.lower()
        
        # Fallback heuristico universal (funciona mesmo sem cerebro)
        if len(p) <= 2 and p in ('o','a','os','as','um','uma','uns','umas','em','de','para','por','com','e','ou','se'):
            return 'funcional'
        if p in ('mais','menos','tanto','quanto'):
            return 'comp'
        if palavra[0].isupper() and len(palavra) > 1 and pos > 0:
            return 'nome'
        
        if not self.cerebro:
            return 'nome'
        
        # 1. Palavra funcional por diluicao de entropia posicional
        if self.cerebro.mk_pos.total > 0:
            posicoes_distintas = 0
            for pos_chave in [f"POS:{i}" for i in range(8)]:
                if pos_chave in self.cerebro.mk_pos.freq:
                    for cand, prob in self.cerebro.mk_pos.predizer_n(pos_chave, 20):
                        if cand.lower() == p:
                            posicoes_distintas += 1; break
            if posicoes_distintas >= 3:
                return 'funcional'
        
        # 2. Verbo por entropia
        if self._eh_verbo_por_entropia(palavra, pos):
            return 'verbo'
        
        # 3. Nome proprio (se maiusculo no meio da frase)
        if palavra[0].isupper() and len(palavra) > 1 and pos > 0:
            return 'nome'
        
        # 4. Nome (fallback universal)
        return 'nome'
        ...
        # 4. Adjetivo (depois de artigo ou nome, antes de nome)
        if pos > 0 and pos < 4 and len(p) > 2 and p.endswith(('oso','osa','vel','al','ico','ante','ente')):
            return 'adj'
        
        # 5. Nome (fallback universal)
        return 'nome'
    _classificar_sem_jaccard = _classificar  # alias para compatibilidade

    def _extrair_tripla(self, tokens):
        """Extrai uma tripla de uma sentenca tokenizada."""
        # Remove artigos e pronomes do inicio (tokens funcionais)
        while tokens and self._classificar(tokens[0], 0) in ('funcional',):
            tokens = tokens[1:]
        ...
        if pos_v < 0 and len(tokens) >= 3:
            v = tokens[1].lower()
            if self._classificar(tokens[1], 1) != 'funcional':
                pos_v = 1; verbo = v
        if pos_v < 0:
            return None
        ...
        for t in reversed(tokens[:pos_v]):
            ...
            if cls in ('nome',) or (t[0].isupper() and len(t)>1):
                sujeito = t
                break
            if cls not in ('funcional',) and len(t) > 1:
                sujeito = t
                break
        if not sujeito:
            return None
        ...
        for i, t in enumerate(depois):
            ...
            if cls == 'funcional':
                if len(t) < 6 and t.lower() in ('de','para','em','com','por','a','ate','desde','entre','sem','sob','sobre','sem','sob','sobre','apos','antes','durante','perante','tras'):
                    for j in range(i+1, len(depois)):
                        cls2 = self._classificar(depois[j], pos_v+1+j)
                        if cls2 in ('nome',):
                            return (sujeito, f"{verbo}_{t}", depois[j])
                        if cls2 not in ('funcional',):
                             return (sujeito, f"{verbo}_{t}", depois[j])
            if cls in ('nome',):
                objeto = t
                break
            if cls not in ('funcional',) and cls != 'verbo':
                objeto = t
                break
...
# ═══════════════════════════════════════════════════════════════════
# [20d] MCRKnowledgeGraph — conhecimento pesquisavel (0 LLM)
# ═══════════════════════════════════════════════════════════════════

class MCRKnowledgeGraph:
    """Grafo de conhecimento pesquisavel por keyword + Jaccard + expansao.
    
    Cada item tem: erro, causa, solucao, ctx, tags.
    Busca por keyword com pesos por campo. Re-ranqueamento por Jaccard
    de bytes entre a pergunta e a solucao. Expansao por contexto.
    Persistencia em JSON.
    """
    def __init__(self, db_path=None):
        self.db_path = db_path or os.path.join(CACHE_DIR, "kg")
    ...
    def _carregar(self):
        for fname in os.listdir(self.db_path):
            if fname.endswith('.json'):
                ctx = fname[:-5]
                try:
                    with open(os.path.join(self.db_path, fname), encoding='utf-8') as f:
                        self._cache[ctx] = json.load(f).get('itens', [])
                except: self._cache[ctx] = []
    
    @staticmethod
    def _jaccard_bytes(texto_a, texto_b):
        ba = texto_a.encode('utf-8'); bb = texto_b.encode('utf-8')
    ...
    def aprender(self, erro, causa, solucao, ctx="geral", tags=None):
        item = {'erro': erro, 'causa': causa, 'solucao': solucao,
                'ctx': ctx, 'tags': tags or [], 'timestamp': time.time()}
        self._cache.setdefault(ctx, []).append(item)
        self._dirty.add(ctx)
    
    def salvar(self):
        for ctx in self._dirty:
            itens = self._cache.get(ctx, [])
            with open(os.path.join(self.db_path, f"{ctx}.json"), 'w', encoding='utf-8') as f:
                json.dump({'ctx': ctx, 'itens': itens}, f)
        self._dirty.clear()
    
    def buscar(self, texto, max_r=5):
        palavras = set(re.findall(r'\w{3,}', texto.lower()))
        if not palavras: return []
        scores = []
        for ctx, itens in self._cache.items():
            for item in itens:
                alvo = (item['erro'] + ' ' + item['causa'] + ' ' + item['solucao']).lower()
                score = 0
                for p in palavras:
                    if p in item['erro'].lower(): score += 5
                    elif p in ctx.lower(): score += 4
                    elif p in item['causa'].lower(): score += 3
                    elif p in alvo: score += 2
                if score > 0:
                    jac = self._jaccard_bytes(texto, item['solucao']) if item['solucao'] else 0
                    nota = score * 0.7 + jac * 50 * 0.3
                    scores.append((nota, jac, item))
        scores.sort(key=lambda x: (-x[0], -x[1]))
        return [s[2] for s in scores[:max_r]]
    
    def buscar_expandido(self, texto, max_r=10):
        ...
        for ctx, itens in self._cache.items():
            for item in itens:
                alvo = (item['erro'] + ' ' + item['causa'] + ' ' +
                       item['solucao'] + ' ' + ctx).lower()
                score = 0
                for p in palavras:
                    if p in alvo: score += 5 if p in item['erro'].lower() else 2
                    elif len(p) >= 4:
                        for pal in alvo.split():
                            if len(pal) >= 4 and _SM(None, p, pal).ratio() > 0.75:
                                score += 1; break
                if score > 0: scores[id(item)] = (score, item)
        # Expansao por contexto
        ctxs = set(i[1].get('ctx','') for i in scores.values())
        for ctx in ctxs:
            for item in self._cache.get(ctx, []):
                if id(item) not in scores:
                    scores[id(item)] = (1.0, item)
        ordenados = sorted(scores.values(), key=lambda x: -x[0])
        return [s[1] for s in ordenados[:max_r]]
    
    def listar_ctxs(self):
        return {ctx: len(itens) for ctx, itens in self._cache.items()}


# ═══════════════════════════════════════════════════════════════════
# [20e] MCRSupervisor — classificacao de intencao + ferramentas
# ═══════════════════════════════════════════════════════════════════

class MCRSupervisor:
    """Classifica intencao e executa ferramentas ANTES de gerar.
    
    1. Classifica texto em tipo/subtipo (codigo, factual, procedimental)
    2. Se menciona arquivo, executa grep/compile no arquivo REAL
    3. Se conhecimento insuficiente, sugere buscar mais dados
    4. Retorna (tipo, subtipo, contexto_enriquecido)
    """
    
    _KEYWORD_MAP = [
        (r"(cri(a|r|e|ar)\s+(um|uma)?\s*(npc|item|spell|monster|quest))", "CRIAR_OBJETO", 90),
        (r"(o que e|explique|como funciona|o que eh)", "PERGUNTA_CONCEITO", 80),
        (r"(quanto|custa|preco|preco)", "PERGUNTA_PRECO", 80),
        (r"(quem|quando|onde|qual|quais|que horas|que dia|que data|hoje)", "PERGUNTA_FATO", 80),
        (r"(como criar|como fazer|passo a passo|tutorial)", "PROCEDIMENTO", 70),
        (r"(alterar|editar|modificar|corrigir)", "ACAO_EDITAR", 80),
        (r"(deletar|remover|apagar|excluir)", "ACAO_DELETAR", 90),
        (r"(ajuda|help|o que voce faz|comandos)", "AJUDA", 90),
        (r"(ola|oi|bom dia|boa tarde)", "SAUDACAO", 80),
    ]
    
    def __init__(self, cerebro, kg):
        self.cerebro = cerebro
        self.kg = kg
    
    def classificar(self, texto):
        """Retorna (tipo, subtipo) via keyword matching — 0 LLM."""
        ...
        for pattern, intent, priority in self._KEYWORD_MAP:
            if re.search(pattern, texto_lower):
                if priority > best_score:
                    best_score = priority
                    best_intent = intent
        return best_intent if best_score >= 40 else "CHAT"
    
    def preparar_contexto(self, texto, intent):
        """Executa ferramentas pre-geracao. Retorna contexto enriquecido."""
        ctx_extra = ""
        import re
        
        # 0. Ferramentas factuais (respostas diretas, sem Markov)
        if intent in ("PERGUNTA_FATO", "PERGUNTA_PRECO", "PERGUNTA_CONCEITO"):
            import datetime as _dt
            if any(p in texto.lower() for p in ["hora", "horas", "horario", "horário"]):
                ctx_extra += f"[FATO] Sao {time.strftime('%H:%M')}."
            if any(p in texto.lower() for p in ["data", "dia", "hoje"]):
                ctx_extra += f" Hoje e {time.strftime('%d/%m/%Y')}."
            if any(p in texto.lower() for p in ["dia da semana"]):
                ctx_extra += f" Hoje e {time.strftime('%A')}."
        
        # 1. Se texto menciona arquivo + "linha"/"bug"/"problema" → grep
        arq = re.search(r'([\w\-]+\.(py|lua|cpp|h|js))', texto)
        if arq and any(p in texto.lower() for p in ['linha', 'bug', 'erro', 'problema']):
            caminho = self._find_file(arq.group(1))
            if caminho:
                try:
                    import subprocess
                    out = subprocess.check_output(
                        ['findstr', '/N', 'BUG\\|TODO\\|FIXME\\|HACK\\|XXX', caminho]
                        if os.name == 'nt' else
                        ['grep', '-n', 'BUG\\|TODO\\|FIXME\\|HACK\\|XXX', caminho],
                        stderr=subprocess.DEVNULL, timeout=5).decode('latin-1')
                    if out:
                        ctx_extra += f"\n[grep] {arq.group(1)}:\n{out[:1000]}"
                except: pass
        
        # 2. Se texto menciona .py + "sintaxe"/"compile" → compile()
        if arq and any(p in texto.lower() for p in ['sintaxe', 'compile', 'valido']):
            caminho = self._find_file(arq.group(1))
            if caminho and caminho.endswith('.py'):
                try:
                    with open(caminho, encoding='utf-8') as f:
                        compile(f.read(), caminho, 'exec')
                except SyntaxError as e:
                    ctx_extra += f"\n[compile] Erro de sintaxe: {e.msg} linha {e.lineno}"
        ...
        # 3. Busca no KG
        kg_results = self.kg.buscar(texto, max_r=5)
        if kg_results:
            ctx_extra += "\n[KG] " + " | ".join(
                f"{r['solucao'][:100]}" for r in kg_results[:3])
        
        return ctx_extra.strip()
    
    def _find_file(self, nome):
        """Encontra arquivo nos diretorios do projeto."""
        for raiz in ['', 'nichos', 'scripts', 'docs']:
    ...
    def rotear(self, texto):
        """Pipeline: classificar → preparar → retornar contexto."""
        intent = self.classificar(texto)
        ctx = self.preparar_contexto(texto, intent)
        return intent, ctx


# ═══════════════════════════════════════════════════════════════════
# [20f] MCRContextCrew — busca paralela em KG + docs + codigo + web
# ═══════════════════════════════════════════════════════════════════

class MCRContextCrew:
    """Busca contexto em multiplas fontes em PARALELO.
    
    ThreadPoolExecutor dispara: KG, Docs, Codigo fonte, Web.
    Retorna contexto enriquecido com marcadores de fonte.
    """
    def __init__(self, cerebro, kg):
        self.cerebro = cerebro
        self.kg = kg
    ...
    def _extrair_termos(self, texto, max_t=5):
        palavras = re.findall(r'\b[a-zA-Z]{3,}\b', texto.lower())
        return list(dict.fromkeys(p for p in palavras if p not in 
            frozenset(['que','para','com','por','como','dos','das','mas','era',
                      'seu','sua','tem','sao','muito','pode','quem','ate','sobre'])))[:max_t]
    
    def _buscar_kg(self, termos):
        if not self.kg: return []
        resultados = []
        for ctx, itens in self.kg._cache.items():
            for item in itens:
                alvo = (item['erro']+' '+item['solucao']).lower()
                if any(t in alvo for t in termos):
                    resultados.append((item['solucao'][:300], f'KG:{ctx}'))
        return resultados
    
    def _buscar_docs(self, termos):
                ...
                for f in files:
                    ...
                    with open(os.path.join(root, f), encoding='utf-8', errors='replace') as fh:
                        for paragrafo in re.split(r'\n\s*\n', fh.read()):
                            if any(t in paragrafo.lower() for t in termos):
                                rel = os.path.relpath(os.path.join(root, f))
                                resultados.append((paragrafo[:300], f'Docs:{rel}'))
        except: pass
        return resultados
    
    def _buscar_codigo(self, termos):
        resultados = []
        for raiz in ['MCR.py', 'nichos']:
            if os.path.isfile(raiz):
                try:
                    with open(raiz, encoding='utf-8', errors='replace') as f:
                        for i, line in enumerate(f.readlines()[:200]):
                            if any(t in line.lower() for t in termos) and len(line.strip()) > 20:
                                resultados.append((line.strip()[:200], 'Code:' + str(raiz) + ':L' + str(i+1)))
                except: pass
            elif os.path.isdir(raiz):
                for root, _, files in os.walk(raiz):
                    for f in files[:50]:
                        if not f.endswith(('.py', '.lua', '.md')): continue
                        fpath = os.path.join(root, f)
                        try:
                            with open(fpath, encoding='utf-8', errors='replace') as fh:
                                for i, line in enumerate(fh.readlines()[:100]):
                                    if any(t in line.lower() for t in termos) and len(line.strip()) > 20:
                                        rel = os.path.relpath(fpath)
                                        resultados.append((line.strip()[:200], 'Code:' + str(rel) + ':L' + str(i+1)))
                        except: pass
        return resultados[:8]
    
    def _buscar_web(self, termos):
        try:
            import urllib.request as _ur, urllib.parse as _up, json
            consulta = '+'.join(termos[:3])
            url = f"https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch={consulta}&format=json&srlimit=2"
            req = _ur.Request(url, headers={'User-Agent': 'MCR-DevIA/1.0'})
            with _ur.urlopen(req, timeout=5) as r:
                data = json.loads(r.read().decode('utf-8'))
            resultados = []
            for item in data.get('query', {}).get('search', [])[:2]:
                snippet = re.sub(r'<[^>]+>', '', item.get('snippet', ''))
                resultados.append((f"{item.get('title','')}: {snippet[:200]}", f"Web:{item['title'][:30]}"))
            return resultados
        except: return []
    
    def executar(self, pergunta, max_termos=5):
        termos = self._extrair_termos(pergunta, max_termos)
        ...
        with ThreadPoolExecutor(max_workers=4) as ex:
            futuros = {
                ex.submit(self._buscar_kg, termos): "KG",
                ex.submit(self._buscar_docs, termos): "Docs",
                ex.submit(self._buscar_codigo, termos): "Code",
                ex.submit(self._buscar_web, termos): "Web",
            }
            for f in as_completed(futuros, timeout=10):
                try: todas.extend(f.result())
                except: pass
        
        resultado = "\n".join(f"[{fonte}] {texto}" for fonte, texto in todas) if todas else ""
        self._cache[h] = resultado
        return resultado
...
class CerebroAGI:
    def __init__(self):
        ...
        # KG + Supervisor + ContextCrew (reintegrados do historico MCR-DevIA)
        self.kg = MCRKnowledgeGraph()
        self.supervisor = MCRSupervisor(self, self.kg)
        self.context_crew = MCRContextCrew(self, self.kg)
        # PI como referencia de infinito: pre-load 10000 digitos
        self.mk_pi = MCR("pi")
        self._carregar_pi()
    
    def _carregar_pi(self):
        """Pre-carrega PI como referencia de infinito com gerador infinito."""
        # Constante com 10000 digitos de PI (para consumo imediato)
        self._pi_digits = ("314159265358979323846264338327950288419716939937510582097494"
        ...
        for i in range(len(self._pi_digits)-1):
            self.mk_pi.aprender(self._pi_digits[i], self._pi_digits[i+1])
        # Estado do gerador infinito (streaming apos os 10000 digitos)
        self._pi_idx = 0
        self._pi_leibniz_k = 0
        self._pi_leibniz_sum = 0.0
        self.coupling.alimentar("pi", "byte", "3", "B:33")
    
    def _proximo_digito_pi(self):
        """Streaming infinito de digitos de PI.
        
        Retorna os 10000 primeiros do cache, depois gera por
        aproximacao de Leibniz para continuar infinitamente.
        """
        if self._pi_idx < len(self._pi_digits):
            d = self._pi_digits[self._pi_idx]
            self._pi_idx += 1
            return d
        # Geracao infinita por Leibniz: pi/4 = 1 - 1/3 + 1/5 - 1/7 + ...
        while True:
            termo = ((-1) ** self._pi_leibniz_k) / (2 * self._pi_leibniz_k + 1)
            self._pi_leibniz_sum += termo
            self._pi_leibniz_k += 1
            pi_approx = self._pi_leibniz_sum * 4
            digito = str(pi_approx).split('.')[1][0] if '.' in str(pi_approx) else '0'
            # So retorna quando a precisao permite um digito estavel
            if self._pi_leibniz_k > 1000 and self._pi_leibniz_k % 100 == 0:
                return digito
            if self._pi_leibniz_k > 100000:
                return str(int(pi_approx * 10) % 10)
    ...
    def ciclo_autonomo(self, texto="", max_passos=20):
        ...
        for passo in range(max_passos):
            ...
            # Genesis: se detectou gap, gera nova acao e registra
            if passo == 0 and self.total_ciclos % 10 == 0:
                try:
                    diag = self.genesis.diagnosticar()
                    if diag.get('gaps'):
                        for gap in diag['gaps'][:2]:
                            codigo = self.genesis.projetar(gap)
                            nome_classe = f"MCR{gap['nome'].split('_')[0].capitalize()}"
                            if nome_classe not in self._acoes_internas:
                                self._acoes_internas[nome_classe] = lambda ctx, g=gap: {"gap": g['nome'], "status": "projetado"}
                                self.mk_orq.aprender(estado_str, nome_classe)
                except:
                    pass
    ...
    def alimentar(self, texto, nome=None, tipo="conv"):
        ...
        if self.hiper.dimensoes and self.total_ciclos % max(1, 10 - min(self.total_ciclos//5, 8)) == 0:
            ...
            if val["instaveis"] and self.total_ciclos > 10:
                # Instabilidade detectada — auto-evolui para corrigir
                for _ in range(min(len(val["instaveis"]), 3)):
                    r = self.auto_evolution.ciclo()
                    if r.get('mutado'):
                        pass  # threshold ajustado para estabilizar o nivel instavel
    ...
    def _gerar_original(self, texto, passos=6):
        ...
        # Passos dinamicos via MCRDecisorUniversal
        try:
            passos = MCRDecisorUniversal.decidir_passos("gerar_original",
                {"n_topicos": len(self.topicos), "n_bytes": self.mk_byte.total})
        except:
            pass
        ...
        for _ in range(passos):
            ...
            if semente not in self.mk_palavra.freq:
                if len(palavras) > 2:
                    ...
                    if semente not in self.mk_palavra.freq and len(palavras) > 1:
                        semente = palavras[-2]
                    elif semente not in self.mk_palavra.freq: break
                elif len(palavras) > 1:
                    semente = palavras[-1]
                    if semente not in self.mk_palavra.freq:
                        semente = palavras[-2] if len(palavras) > 1 else None
                        if semente is None or semente not in self.mk_palavra.freq: break
                else: break
            ...
            if cands:
                ...
                if mod[melhor] > 0.01:
                    pred = melhor; conf = mod[melhor]
            
            # Fallback: esfera cross-dimensional
            if pred is None or conf < 0.05:
                ...
                for n in ["token_tipo", "linha", "byte_delta", "hash_curto", "byte"]:
                    if n in self.hiper.dimensoes or n == "byte":
                        ctx = {"palavra": semente}
                        if n == "byte": ctx["byte"] = ultimo_byte
                        pred_esf, conf_esf = self.coupling.esfera.predizer_cross("palavra", **ctx)
                        if pred_esf and conf_esf > conf:
                            pred = pred_esf; conf = conf_esf
                            if conf > 0.3: break
            
            # Fallback: superposicao
            if pred is None or conf < 0.01:
                ultimo_byte = f"B:{ord(semente[-1]):02x}" if semente else "B:00"
                novo, conf2, meta = self.superposicao.colidir(
                    "palavra", semente, "byte", ultimo_byte, self.mk_palavra, self.mk_byte)
                if novo and conf2 > 0.05: pred = novo; conf = conf2
            ...
            if pred is None or conf < 0.01:
                ...
                if novos: pred = novos[0]; conf = 0.05
            ...
            if self.entropia.esta_em_loop():
                ...
                if cands and len(cands) > 1:
                    ...
                    if palavra_alt:
                        palavras[-1] = palavra_alt; self.entropia.alimentar(palavra_alt); continue
                if len(palavras) > 2:
                    semente = palavras[-2]
                    ...
                    if cands2:
                        for alt, _ in cands2:
                            if alt != palavra_loop and alt != palavras[-2]:
                                palavras[-1] = alt; self.entropia.alimentar(alt); break
                        continue
                break
            
            # Parada por entropia normalizada por PI
            try:
                ent_pi = self.mk_pi.entropia_media()
                if ent_pi > 0:
                    ent = self.mk_palavra.entropia(pred) if pred in self.mk_palavra.freq else 0
                    if ent > 2.0: break  # token imprevisivel, parar
            except: pass
    ...
    def _cadeia_pensamento(self, texto, intencao="", passos=4):
        ...
        # Gera intencao como pre-contexto (antes de gerar resposta)
        if not intencao_ativa or intencao_ativa == palavras[0]:
            try:
                # Usa coupling para inferir intencao a partir do texto
                intent_cross = self.coupling.esfera.predizer_cross("intencao", palavra=palavras[0])
                if intent_cross and intent_cross[0] and intent_cross[1] > 0.1:
                    intencao_ativa = intent_cross[0]
            except:
                pass
        ...
        for passo in range(passos):
            ...
            if novo_token is None and len(cadeia) >= 2:
                ...
                if chave_m2 in self.mk_palavra.freq:
                    pred, conf = self.mk_palavra.predizer_com_entropia(chave_m2, ent_pi_ref=self.mk_pi.entropia_media())
    ...
    def gerar(self, texto, passos=None, pergunta=""):
        ...
        # Supervisor: ferramentas factuais (horas, data, calculos) ANTES de gerar
        ctx_super = ""
        try:
            intent = self.supervisor.classificar(texto)
            ctx_super = self.supervisor.preparar_contexto(texto, intent)
        except:
            pass
        
        # ContextCrew: contexto multi-fonte
        try:
            ctx_extra = self.context_crew.executar(texto, max_termos=3)
        except:
            ctx_extra = ""
        
        # Monta contexto: supervisor (factual) + contextcrew (so docs/kg, sem codigo)
        ctx_combinado = ctx_super
        if ctx_extra:
            # Filtra linhas de codigo (Code:) — mantem so Docs: e KG:
            ctx_limpo = "\n".join(
                line for line in ctx_extra.split('\n')
                if not line.strip().startswith('[Code]')
            )
            if ctx_limpo and len(ctx_limpo) > 10:
                ctx_combinado = (ctx_combinado + " " + ctx_limpo).strip()
        
        texto_com_contexto = (ctx_combinado + "\n" + texto) if ctx_combinado and len(ctx_combinado) > 10 else texto
...
def _rodar_autonomo(cerebro):
    ...
    try:
        while True:
            ...
            # 8. Genesis: aprende padrao de arquivos e gera novas estruturas
            if total_ciclos > 0 and total_ciclos % 30 == 0:
                try:
                    # Aprende de arquivos no diretorio de log/cache
                    n_aprendidos = cerebro.genesis.aprender_de_arquivos(
                        CACHE_DIR, max_arquivos=5)
                    if n_aprendidos > 0:
                        _log(f"  Genesis: aprendeu {n_aprendidos} arquivos")
                    # Gera nova estrutura aprendida
                    geracoes = cerebro.genesis.gerar_estrutura("B:00", n_variacoes=3)
                    if geracoes:
                        _log(f"  Genesis: gerou {len(geracoes)} novas estruturas")
                except Exception as e:
                    _log(f"  Erro Genesis: {e}")
