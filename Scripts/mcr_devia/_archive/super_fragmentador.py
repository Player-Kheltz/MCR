...
class SuperFragmentador:
    ...
    def __init__(self, modelo_padrao: str = "qwen2.5-coder:7b",
                 ctx_alvo: int = 1024):
    ...
    def processar(self, modelo: str = None, tarefa: str = "code",
        ...
        for i, frag in enumerate(self.fragmentos):
            if callback:
                ...
                for ref in frag.referencias:
                    ...
                    if ref_frag and ref_frag.resultado:
                        contexto_extra += f"\n[Fragmento {ref}]: {str(ref_frag.resultado)[:200]}"
            ...
            print(f"  Fragmento {i+1}/{len(self.fragmentos)}: {str(frag.resultado)[:80]}...")
