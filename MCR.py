...
class MCRConexao:
    ...
    def _avaliar_ponte(self, topico_a: str, topico_b: str, palavra: str
        ...
        especificidade = 1.0 - min(1.0, freq_global / max(1, len(self.motor.topicos) * 0.5))  # 0.5 = metade dos topicos

        # PROFUNDIDADE (peso 2)
        eq_p = __import__(__name__).__dict__.get('_EQUACAO_ATUAL', _EQUACAO_ATUAL)
        _passos = eq_p.get('passos_base', 6)
        cadeia_a = len(mk_a.gerar(palavra, passos=_passos))
        cadeia_b = len(mk_b.gerar(palavra, passos=_passos))
        profundidade = min(1.0, (cadeia_a + cadeia_b) / 10)

        eq = __import__(__name__).__dict__.get('_EQUACAO_ATUAL', _EQUACAO_ATUAL)
        w_div = eq.get('ponte_divergencia', 5)
        w_esp = eq.get('ponte_especificidade', 3)
        w_prof = eq.get('ponte_profundidade', 2)

        score = (divergencia * w_div + especificidade * w_esp + profundidade * w_prof) / 10
...
class MCRMotor:
    ...
    def _encontrar_ponte(self, topico_a: str, topico_b: str
        ...
        if melhor:
            ...
            texto_b = self.topicos[topico_b]['texto']
            ...
            eq = __import__(__name__).__dict__.get('_EQUACAO_ATUAL', _EQUACAO_ATUAL)
            th_cont = eq.get('threshold_conteudo', 0.6)
            th_parc = eq.get('threshold_parcial', 0.3)
            # Se os textos sao identicos ou quase -> conteudo_compartilhado
            j_ab = MCRByteUtils.jaccard_bytes(texto_a, texto_b)
            if j_ab > 0.8:
                tipo = 'conteudo_compartilhado'
            else:
                tipo = ('conteudo_compartilhado' if score >= th_cont
                        else 'conteudo_mas_parcial' if score >= th_parc
                        else 'byte_only')
    ...
    def _byte_bridge(self, topico_a: str, topico_b: str
        ...
        texto_a = self.topicos[topico_a]['texto']
        seq_a = mk_a.gerar(f"B:{texto_a.encode('utf-8')[0]:02x}", passos=__import__(__name__).__dict__.get("_EQUACAO_ATUAL", {}).get("passos_base", 6))
    ...
    def conectar(self, topico_a: str, topico_b: str, forcar: bool = False) -> Optional[Dict]:
        ...
        if tipo_p in ('conteudo_compartilhado', 'conteudo_mas_parcial'):
            ...
            for _ in range(14):
                ...
                atual = prox
            if not atingiu:
                seq_gerada = mk_pal_a.gerar(semente, passos=__import__(__name__).__dict__.get("_EQUACAO_ATUAL", {}).get("passos_base", 6))
        ...
        if not sequencia:
            ...
            _pas = __import__(__name__).__dict__.get('_EQUACAO_ATUAL', _EQUACAO_ATUAL).get('passos_base', 6)
            seq_a = mk_a.gerar(inicio_a, passos=_pas)
            estados_b = set(mk_b.freq.keys())
            ponte = None
            for e in seq_a:
                if e in estados_b:
            ...
            if ponte is None:
                return None
            seq_b = mk_b.gerar(ponte, passos=__import__(__name__).__dict__.get("_EQUACAO_ATUAL", {}).get("passos_base", 6))
    ...
    def _coerencia_token(self, seq: str) -> float:
        pal = seq.split()
        if len(pal) < MCRDecisorUniversal._limiar("palavra_min", " ".join(pal)):
            return 0.0  # guard clause
        ok = 0
    ...
    def _autoavaliar(self, sequencia: str, texto_a: str, texto_b: str, tipo_ponte: str
        ...
        eq = __import__(__name__).__dict__.get('_EQUACAO_ATUAL', _EQUACAO_ATUAL)
        formula = eq.get('formula', 'by + pa + tk')

        # Componentes normalizados (0-1 cada)
        ...
        inter_a = len(cs & ca) / max(len(ca | cs), 1)
        inter_b = len(cs & cb) / max(len(cb | cs), 1)
        pa = (c_pal + inter_a + inter_b) / 3  # palavra
        ...
        tem_a = 1.0 if tipos_seq & tipos_a else 0.0
        tem_b = 1.0 if tipos_seq & tipos_b else 0.0
        tk = (c_tok + tem_a + tem_b) / 3  # token

        h = MCRByteUtils.entropia_bytes(sequencia) / 8.0  # entropia normalizada

        # AVALIA A FORMULA DINAMICAMENTE
        try:
            nota_base = eval(formula, {'__builtins__': {}}, {
                'by': by, 'pa': pa, 'tk': tk, 'h': h,
                'max': max, 'min': min, 'sum': sum,
            })
        except Exception:
            nota_base = by + pa + tk  # fallback seguro

        nota_base = max(0.0, min(10.0, nota_base * 5))

        # PENALIDADE
        penalidade = {'conteudo_compartilhado': eq.get('penalidade_compartilhado', 0.0),
                      'conteudo_mas_parcial': eq.get('penalidade_parcial', 0.3),
                      'byte_only': eq.get('penalidade_byte', 0.7)}.get(tipo_ponte, eq.get('penalidade_none', 0.9))

        nota = nota_base * (1 - penalidade)
        ...
        return nota, {
            ...
            'formula': formula,
            'equacao': f"{formula}={nota_base:.2f}x(1-{penalidade:.1f})={nota:.1f}",
        }
...
class MCRWebLearn:
    ...
    def buscar(self, termo: str) -> int:
        """Busca um termo na web (DuckDuckGo + fallback Wikipedia)."""
        ...
        # 2. Fallback Wikipedia — tenta paginas comuns de conhecimento
        if not textos:
            # Busca na Wikipedia usando a pergunta COMPLETA como termo
            for lang in ['pt', 'en']:
                try:
                    search_url = (
                        f"https://{lang}.wikipedia.org/w/api.php?"
                        f"action=query&list=search&srsearch={quote(termo)}"
                        f"&format=json&srlimit=3"
                    )
                    req = Request(search_url, headers={'User-Agent': ua})
                    with urlopen(req, timeout=10) as resp:
                        search = _json.loads(resp.read().decode('utf-8', errors='replace'))
                    for page_data in search.get('query', {}).get('search', []):
                        titulo = page_data['title']
                        try:
                            ext_url = (
                                f"https://{lang}.wikipedia.org/w/api.php?"
                                f"action=query&prop=extracts&exintro=1&explaintext=1"
                                f"&titles={quote(titulo)}&format=json"
                            )
                            req = Request(ext_url, headers={'User-Agent': ua})
                            with urlopen(req, timeout=10) as resp:
                                ext = _json.loads(resp.read().decode('utf-8', errors='replace'))
                            for _, page in ext.get('query', {}).get('pages', {}).items():
                                if page.get('extract'):
                                    textos.append(page['extract'])
                                    break
                        except Exception:
                            continue
                except Exception:
        ...
        # Alimenta todos os textos — a Equacao MCR decide relevancia
        for texto in textos:
            if len(texto) > 30:
                nome = f"web:{termo[:20]}"
                self.motor.alimentar(texto, nome)
...
class MCRGenerator:
    ...
    # ─── GERACAO POR MULTIPLAS INSTANCIAS SOBREPOSTAS ─────

    @staticmethod
    def _segmentar(texto: str) -> List[str]:
        """Segmenta codigo em blocos sobrepostos (funcoes, condicionais, loops).
        Cada bloco vira uma instancia que se sobrepoe com a anterior.
        Isso cria transicoes REPETIDAS que Markov aprende."""
        if not texto:
            return []
        blocos = []
        linhas = texto.split('\n')
        ...
        for linha in linhas:
            ...
            if 'end' in linha.strip().lower() and nivel > 0:
                if bloco_atual:
                    blocos.append('\n'.join(bloco_atual))
                    # Sobrepõe: mantém últimas 2 linhas
                    if len(bloco_atual) > 2:
                        bloco_atual = bloco_atual[-2:]
                    else:
                        bloco_atual = []
                nivel -= 1
            elif any(kw in linha.strip().lower() for kw in ['function ', 'if ', 'for ', 'while ']):
    ...
    def _alimentar_instancias(self, textos: List[str], prefixo: str = 'code'):
        """Alimenta MULTIPLAS instancias sobrepostas no motor.
        Cada instancia tem 3-10 linhas e se sobrepoe com a vizinha."""
        for i, texto in enumerate(textos):
            nome = f'{prefixo}_inst_{i}'
            if nome not in self.motor.topicos:
                self.motor.alimentar(texto, nome)

    def _gerar_por_similaridade(self, pedido: str, max_candidatos: int = 5) -> str:
        """Gera codigo encontrando a instancia mais similar no repertorio,
        usando sua estrutura como template e preenchendo por assinatura."""
        if not self.motor.topicos:
            return self._gerar_codigo(pedido)

        # Encontra o topico mais similar ao pedido
        melhores = []
        for nome, dados in self.motor.topicos.items():
            texto = dados.get('texto', '')
            if not texto or len(texto) < 30:
                continue
            j = MCRByteUtils.jaccard_bytes(pedido, texto)
            if j > 0.1:
                melhores.append((j, nome, texto))

        melhores.sort(key=lambda x: -x[0])
        if not melhores:
            return self._gerar_codigo(pedido)

        # Pega o mais similar, usa as primeiras linhas como semente
        melhor_texto = melhores[0][2]
        linhas = melhor_texto.split('\n')
        semente = linhas[0] if linhas else pedido

        resultado = self.motor.gerar_por_assinatura(semente, passos=15)
        if len(resultado) < 20:
            return melhor_texto
        return resultado

    def gerar_estruturado(self, pedido: str, textos_fonte: List[str] = None) -> str:
        """Gera codigo ESTRUTURADO por multiplas instancias sobrepostas.

        1. Segmenta os textos em instancias sobrepostas
        2. Alimenta no motor (cria repeticoes)
        3. Gera por similaridade com o pedido
        4. Valida e retorna"""
        if textos_fonte:
            for t in textos_fonte:
                blocos = self._segmentar(t)
                self._alimentar_instancias(blocos)
        return self._gerar_por_similaridade(pedido)
...
class MCRComandos:
    ...
    def master(self, pedido: str = '') -> Dict:
        ...
        if 'analisar' in pedido.lower() or 'ler' in pedido.lower():
            ...
            # Busca conhecimento relevante antes de responder
            resposta_base = self._buscar_resposta(pedido)
            if resposta_base:
                return {'resposta': resposta_base[:300], 'fonte': 'conhecimento'}
    ...
    def _buscar_resposta(self, pedido: str) -> str:
        """Busca no conhecimento o TRECHO mais relevante para o pedido.
        Divide em paragrafos, encontra o de maior similaridade."""
        if not self.motor.topicos:
            return ''
        ...
        for nome, dados in self.motor.topicos.items():
            ...
            if not texto or len(texto) < 20:
                continue
            # Divide em paragrafos (ou frases)
            paragrafos = texto.replace('\n\n', '|').replace('\n', ' ').split('|')
            for p in paragrafos:
                p = p.strip()
                if len(p) > 20:
                    j = MCRByteUtils.jaccard_bytes(pedido, p)
                    if j > 0.05:
                        todos_paragrafos.append((j, p))

        if not todos_paragrafos:
            return ''

        todos_paragrafos.sort(key=lambda x: -x[0])
        return todos_paragrafos[0][1][:500]


# ═══════════════════════════════════════════════════════════════
# MCR_EVOLUIR — a equacao otimiza a si mesma
# ═══════════════════════════════════════════════════════════════

# Constantes ATUAIS da Equacao MCR (extraidas do codigo)
# O MCR vai testar variacoes destes valores e descobrir
# qual combinacao produz a MAIOR coerencia de assinatura.
_EQUACAO_ATUAL = {
    'formula': '2*by + 1*pa',
    'peso_byte': 1,
    'peso_palavra': 13,
...
# Estruturas de formula testaveis pela evolucao
_FORMULAS_DISPONIVEIS = [
    'by + pa + tk',           # soma simples (original)
    'by + pa + tk + h',       # soma + entropia
    '(by + pa + tk) / 3',     # media
    'by * pa + tk',           # byte * palavra + token
    'pa + tk',                # sem byte
    'pa',                     # so palavra
    'max(by, pa, tk)',        # maximo
    'by + pa + tk + by*pa',   # interacao
]


def mcr_evoluir(geracoes: int = 10, variacao: float = 0.2) -> Dict:
    """Faz o MCR evoluir a propria equacao.

    A cada geracao:
      1. Gera N variacoes dos parametros
      2. Testa cada variacao contra dados conhecidos
      3. A variacao com MAIOR fitness vira a nova equacao
      4. Repete ate estabilizar
    """
    motor = MCRMotor()
    historico = []
    melhor_fitness = 0.0
    equacao_atual = dict(_EQUACAO_ATUAL)
    _EQUACAO_ATUAL.update(equacao_atual)

    # Alimenta dados de referencia
    motor.alimentar('SPA progressao aventureiro com dominios elementais Fogo Gelo Terra', 'ref_spa')
    motor.alimentar('SHC habilidades contextuais com posturas sinergias e dominios', 'ref_shc')

    for geracao in range(geracoes):
        fitness_atual = _calcular_fitness(motor, dict(_EQUACAO_ATUAL))
        historico.append({
            'geracao': geracao,
            'fitness': round(fitness_atual, 4),
        })

        if fitness_atual > melhor_fitness:
            melhor_fitness = fitness_atual

        variacoes = _gerar_variacoes(dict(_EQUACAO_ATUAL), variacao)
        melhores_var = []

        for eq_var in variacoes:
            fit = _calcular_fitness(motor, eq_var)
            melhores_var.append((fit, eq_var))

        melhores_var.sort(key=lambda x: -x[0])

        if melhores_var and melhores_var[0][0] > fitness_atual:
            _EQUACAO_ATUAL.update(melhores_var[0][1])
            equacao_atual = dict(_EQUACAO_ATUAL)
            print(f'  G{geracao}: fitness {fitness_atual:.4f} -> {melhores_var[0][0]:.4f}')
        else:
            variacao *= 0.8
            print(f'  G{geracao}: fitness {fitness_atual:.4f} (estagnou)')

        if variacao < 0.01:
            break

    return {
        'equacao_final': dict(_EQUACAO_ATUAL),
...
def _gerar_variacoes(equacao: Dict, variacao: float) -> List[Dict]:
    """Gera N variacoes da equacao, cada uma com um parametro ligeiramente diferente."""
    import random as _rand
    _rand.seed()
    variacoes = []

    # Variacoes de PARAMETROS (pesos)
    for chave in equacao:
        if chave == 'formula':
            continue
        if chave in ('threshold_conteudo', 'threshold_parcial', 'penalidade_compartilhado',
                     'penalidade_parcial', 'penalidade_byte', 'penalidade_none'):
            continue
        if chave in ('conf_min_base', 'passos_base'):
            continue

        valor = equacao[chave]
        if isinstance(valor, (int, float)):
            for delta in [-variacao, variacao]:
                nova_eq = dict(equacao)
                novo_valor = valor * (1 + delta) if valor != 0 else delta
                if isinstance(valor, int):
                    novo_valor = max(1, int(novo_valor))
                else:
                    novo_valor = max(0.0, min(1.0, round(novo_valor, 2)))
                nova_eq[chave] = novo_valor
                variacoes.append(nova_eq)

    # Variacoes de ESTRUTURA da formula
    for formula_atual in _FORMULAS_DISPONIVEIS:
        if formula_atual != equacao.get('formula', 'by + pa + tk'):
            nova_eq = dict(equacao)
            nova_eq['formula'] = formula_atual
            variacoes.append(nova_eq)
...
def _calcular_fitness(motor: MCRMotor, equacao: Dict) -> float:
    """Calcula o FITNESS de uma equacao.
    
    Define a equacao no escopo global, executa conexoes reais,
    e mede a qualidade dos resultados.
    """
    import __main__ as _main
    # Atualiza a equacao global
    _EQUACAO_ATUAL.update(equacao)

    fitness = 0.0
    n_testes = 0

    # Testa conexoes entre topicos usando a equacao ATUAL
    pares = [('ref_spa', 'ref_shc'), ('ref_spa', 'ref_fibonacci'),
             ('ref_shc', 'ref_quadrados')]
    for a, b in pares:
        if a in motor.topicos and b in motor.topicos:
            c = motor.conectar(a, b, forcar=True)
            if c:
                fitness += c['nota'] / 10.0  # nota 0-10 normalizada
                n_testes += 1

    # Entropia media (quanto menor, mais estruturado)
    h_byte = motor.mk_byte.entropia_media()
    fitness += 1.0 / (h_byte + 0.1)
    n_testes += 1

    return fitness / max(n_testes, 1)


if __name__ == '__main__':
    import sys as _sys

    if '--evoluir' in _sys.argv[1:]:
        resultado = mcr_evoluir(geracoes=15)
        print('MCR EVOLUCAO DA EQUACAO')
        print('=' * 60)
        print(f'Fitness final: {resultado["fitness_final"]:.4f}')
        print(f'Geracoes: {resultado["geracoes"]}')
        print()
        print('Equacao final:')
        for k, v in resultado['equacao_final'].items():
            print(f'  {k}: {v}')
        print()
        print('Historico:')
        for h in resultado['historico']:
            print(f'  G{h["geracao"]:2d}: fitness={h["fitness"]:.4f}')

    elif '--hardcode' in _sys.argv[1:]:
