--[[
    Projeto MCR ? buff_system.lua (v4.0)
    Gestão de buffs temporários com feedback visual.
    Life Leech e Mana Leech processados diretamente no PosturaDmg (postura.lua).
--]]

function applySpeedBurst(player, multiplier, durationMs)
    if (player:getStorageValue(STORAGE.BUFF_BASE + 2) or 0) > 0 then
        return
    end
...
function processBuffs(player)
    -- Speed burst
    local burstActive = player:getStorageValue(STORAGE.BUFF_BASE + 2) or 0
    if burstActive == 0 then
        player:setStorageValue(STORAGE.BUFF_BASE + 1, 0)
    end
    ...
    -- Life leech ? a expiração limpa a storage (a cura é feita no PosturaDmg)
