 ...
 * Modifica\347\365es para o Projeto MCR \251 2026 Equipe MCR
 * Repositório original: https://github.com/opentibiabr/canary
 * Licen\347a: https://github.com/opentibiabr/canary/blob/main/LICENSE
 */
...
#ifdef _WIN32
	// [MCR] Define o t\355tulo da janela do console como "Projeto MCR"
	...
	g_dispatcher().addEvent(
		[this] {
			try {
				...
				// [MCR] Log do protocolo em portugu\352s
				...
				logger.info("Inicializando o estado do jogo...");
				...
				if (g_configManager().getBoolean(TOGGLE_MAINTAIN_MODE)) {
					...
					g_webhook().sendMessage(":yellow_square: Servidor agora **online** _(acesso restrito \340 equipe)_");
				} else {
...
void CanaryServer::setWorldType() {
	...
	if (worldType == "pvp") {
		...
		throw FailedToInitializeCanary(
			fmt::format(
				"Tipo de mundo desconhecido: {}. Tipos válidos: pvp, não-pvp e pvp-enforced",
...
void CanaryServer::logInfos() {
	// [MCR] Logs de inicialização em portugu\352s, com nome e versão do Projeto MCR
...
std::string CanaryServer::getCompiler() {
	...
	return compiler = fmt::format("Estúdio Visual Microsoft {}", _MSC_VER);
...
void CanaryServer::validateDatapack() {
	...
	if (!useAnyDatapack && datapackName != "data-canary" && datapackName != "data-otservbr-global") {
		throw FailedToInitializeCanary(fmt::format("O nome do datapack de massa '{}' é inválido. Nomes válidos: 'data-canary', "
		                                           "'data-otservbr-global', ou definido USE_ANY_DATAPACK_FOLDER = true em config.lua.",
		                                           datapackName));
	}
}
...
void CanaryServer::loadModules() {
	...
	logger.debug("Carregando scripts do n\372cleo na pasta: {}/", coreFolder);
...
void CanaryServer::modulesLoadHelper(bool loaded, std::string moduleName) {
	...
	if (!loaded) {
		throw FailedToInitializeCanary(fmt::format("Não foi poss\355vel carregar: {}", moduleName));
	}
}
