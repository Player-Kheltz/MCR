	...
	if (const std::string spellName = asLowerCaseString(spell->name);
		...
		if (!spell->outfitMonster.empty()) {
			...
			g_logger().error("[Monsters::deserializeSpell] - "
			                 "Monstro de roupa faltando ou item n\343o feiti\347o de roupa para: {}",
			                 description);
			return false;
		}
		...
		g_logger().error("[Monsters::deserializeSpell] - "
		                 "{} par\342metro desconhecido ou ausente n\343o feiti\347o com nome: {}",
		                 description, spell->name);
	}
...
std::shared_ptr<MonsterType> Monsters::getMonsterType(const std::string &name, bool silent /* = false*/) const {
    // Converte para Latin-1, depois minúsculas, e usa como chave
    std::string key = asLowerCaseString(toLatin1(name));
    auto it = monsters.find(key);
    if (it != monsters.end()) {
        return it->second;
    }
    if (!silent) {
        g_logger().error("[Monsters::getMonsterType] - Monster with name {} not exist", key);
    }
    return nullptr;
}
...
bool Monsters::tryAddMonsterType(const std::string &name, const std::shared_ptr<MonsterType> &mType) {
    ...
    if (monsters.contains(key)) {
        g_logger().debug("[{}] o monstro com nome '{}' já existe", __FUNCTION__, key);
        return false;
    }
    monsters[key] = mType;
    return true;
