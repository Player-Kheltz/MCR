			...
			if (strcasecmp(childMonsterNode.name(), "monster") == 0) {
				...
				if (spawnTimeAttr) {
					...
					if (xmlSpawnTime > 0) {
						...
						g_logger().warn("Valor de spawntime '{}' inv\341lido para monstro '{}'. Configurando o tempo de respawn padr\343o: {}", spawnTimeAttr.value(), nameAttribute.value(), scheduleInterval);
					}
				} else {
					g_logger().warn("Atributo de spawntime ausente para o monstro '{}'. Configurando o tempo de respawn padr\343o: {}", nameAttribute.value(), scheduleInterval);
				}
...
bool SpawnMonster::spawnMonster(uint32_t spawnMonsterId, spawnBlock_t &sb, const std::shared_ptr<MonsterType> &monsterType, bool startup /*= false*/) {
	...
	if (startup) {
		...
		g_logger().trace("[SpawnMonster] Gerando {} em {}", monsterType->name, sb.pos.toString());
...
bool SpawnMonster::addMonster(const std::string &name, const Position &pos, Direction dir, uint32_t scheduleInterval, uint32_t weight /*= 1*/) {
	...
	if (!monsterType) {
		g_logger().error("Não foi possível encontrar {}", name);
		return false;
	}
	...
	if (sb) {
		if (sb->monsterTypes.contains(monsterType)) {
			g_logger().error("[SpawnMonster] Monstro {} j\341 existe n\343o bloco de spawn em {}", name, pos.toString());
			return false;
		}
		if (monsterType->isBoss() && !sb->monsterTypes.empty()) {
			g_logger().error("[SpawnMonster] Monstro chefe {} foi adicionado ao bloco de spawn com outros monstros. Isto n\343o \351 permitido.", name);
			return false;
		}
		if (sb->hasBoss()) {
			g_logger().error("[SpawnMonster] Monstro {} foi adicionado ao bloco de spawn com um chefe. Isto n\343o \351 permitido.", name);
			return false;
		}
	}
	if (!sb) {
	...
	for (const auto &[mType, weight] : monsterTypes) {
		...
		if (mType->isBoss()) {
			if (monsterTypes.size() > 1) {
				g_logger().warn("[SpawnMonster] Monstro chefe {} foi adicionado ao bloco de spawn com outros monstros. Isto n\343o \351 permitido.", mType->name);
			}
			return mType;
		}
		totalWeight += weight;
