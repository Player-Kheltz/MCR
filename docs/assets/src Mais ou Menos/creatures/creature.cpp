...
void Creature::onCreatureDisappear(const std::shared_ptr<Creature> &creature, bool isLogout) {
    if (getAttackedCreature() == creature && !creature->isRemoved() && creature->getPosition().z != getPosition().z) {
        return;
    }
    if (getFollowCreature() == creature && !creature->isRemoved() && creature->getPosition().z != getPosition().z) {
        return;
    }
			...
			if ((monster && monster->isFamiliar() && checkSummonDist) || (teleportSummon && !protectionZoneCheck && checkSummonDist)) {
				...
				if (const auto &masterTile = creatureMaster->getTile()) {
					if (masterTile->hasFlag(TILESTATE_TELEPORT)) {
						g_logger().warn("[{}] n\343o pode teletransportar a invoca\347\343o, a posi\347\343o tem teletransporte. {}", __FUNCTION__, creatureMaster->getPosition().toString());
					} else {
...
bool Creature::dropCorpse(const std::shared_ptr<Creature> &lastHitCreature, const std::shared_ptr<Creature> &mostDamageCreature, bool lastHitUnjustified, bool mostDamageUnjustified) {
	metrics::method_latency measure(__METRICS_METHOD_NAME__);
	bool hasSPAInventory = false;
	if (auto monster = getMonster()) {
		if (monster->getInventoryContainer()) {
			hasSPAInventory = true;
		}
	}

	if ((!lootDrop && getMonster()) && !hasSPAInventory) {
		// Summons normais (sem loot, sem inventário SPA): apenas efeito visual e eventos de death
		...
		if (tile && splash) {
			...
			if (tile && corpse) {
				g_game().internalAddItem(tile, corpse, INDEX_WHEREEVER, FLAG_NOLIMIT);
					// MCR: despeja o inventário do escudeiro no corpo, se for uma Criatura SPA com inventário
					if (hasSPAInventory) {
						if (auto monster = getMonster()) {
							monster->dropInventoryOnDeath(corpse);
						}
					}
...
void Creature::mitigateDamage(const CombatType_t &combatType, BlockType_t &blockType, int32_t &damage) const {
	if (combatType != COMBAT_MANADRAIN && combatType != COMBAT_LIFEDRAIN && combatType != COMBAT_AGONYDAMAGE) { // Increase mitigate damage
		...
		g_logger().trace("criatura [mitiga\347\343o]: {}, dano original: {}, dano de mitiga\347\343o: {}", getName(), originalDamage, damage);
...
void Creature::onGainExperience(uint64_t gainExp, const std::shared_ptr<Creature> &target) {
	...
	if (!m->isFamiliar()) {
		...
		TextMessage message(MESSAGE_EXPERIENCE_OTHERS, fmt::format("{} ganhou {} ponto de experi\352ncia{}.", ucfirst(getNameDescription()), gainExp, (gainExp != 1 ? "s" : "")));
...
bool Creature::knockback(Direction direction) {
    const Position& currentPos = getPosition();
    Position destPos = getNextPosition(direction, currentPos);
    
    // Corrigir: getTile retorna std::shared_ptr<Tile>, não Tile*
    auto tile = g_game().map.getTile(destPos);
    if (!tile || tile->hasFlag(TILESTATE_BLOCKPATH) || tile->hasFlag(TILESTATE_BLOCKSOLID)) {
        return false;
    }
    
    if (tile->getCreatures() && !tile->getCreatures()->empty()) {
        return false;
    }
    
    g_game().addMagicEffect(currentPos, CONST_ME_POFF);
    g_game().addMagicEffect(destPos, CONST_ME_HITAREA);
    
    // Corrigir: usar static_self_cast<Creature>() para obter um shared_ptr compatível com Thing
    g_game().internalTeleport(static_self_cast<Creature>(), destPos, false);
...
double_t Creature::getReflectPercent(CombatType_t combatType, bool useCharges /*= false*/) const {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em getReflectPercent: {}", e.what());
	}
	return 0;
}
...
void Creature::setReflectPercent(CombatType_t combatType, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em setReflectPercent: {}", e.what());
	}
}
...
int32_t Creature::getReflectFlat(CombatType_t combatType, bool useCharges /* = false*/) const {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em getReflectFlat: {}", e.what());
	}
	return 0;
}
...
void Creature::setReflectFlat(CombatType_t combatType, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em setReflectFlat: {}", e.what());
	}
}
...
int32_t Creature::getAbsorbFlat(CombatType_t combat) const {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em getAbsorbFlat: {}", e.what());
	}
	return 0;
}
...
void Creature::setAbsorbFlat(CombatType_t combat, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em setAbsorbFlat: {}", e.what());
	}
}
...
int32_t Creature::getAbsorbPercent(CombatType_t combat) const {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em getAbsorbPercent: {}", e.what());
	}
	return 0;
}
...
void Creature::setAbsorbPercent(CombatType_t combat, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em setAbsorbPercent: {}", e.what());
	}
}
...
int32_t Creature::getIncreasePercent(CombatType_t combat) const {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em getIncreasePercent: {}", e.what());
	}
	return 0;
}
...
void Creature::setIncreasePercent(CombatType_t combat, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice est\341 fora do intervalo em setIncreasePercent: {}", e.what());
	}
}
...
bool Creature::goToPosition(const Position& target) {
    ...
    if (!getPathTo(target, listDir, fpp) || listDir.empty()) {
        g_logger().info("[MCR-WALK] {}::goToPosition FALHOU para {}", getName(), target.toString());
        return false;
    }
