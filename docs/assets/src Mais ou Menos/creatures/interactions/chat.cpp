...
void PrivateChatChannel::invitePlayer(const std::shared_ptr<Player> &player, const std::shared_ptr<Player> &invitePlayer) {
	...
	ss << player->getName() << " convida voc\352 para " << player->getPossessivePronoun() << " canal de bate-papo privado.";
...
void PrivateChatChannel::excludePlayer(const std::shared_ptr<Player> &player, const std::shared_ptr<Player> &excludePlayer) {
	...
	std::ostringstream ss;
	ss << excludePlayer->getName() << " foi exclu\355do.";
	player->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, ss.str());
...
Chat::Chat() :
	scriptInterface("Interface de bate-papo"),
	dummyPrivate(std::make_shared<PrivateChatChannel>(CHANNEL_PRIVATE, "Canal de bate-papo privado")) {
	...
	switch (channelId) {
		...
		case CHANNEL_PRIVATE: {
			...
			for (uint16_t i = 100; i < 10000; ++i) {
				auto [iter, inserted] = privateChannels.try_emplace(i, std::make_shared<PrivateChatChannel>(i, player->getName() + "Canal do"));
				if (inserted) {
