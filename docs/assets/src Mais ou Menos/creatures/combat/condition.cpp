...
bool Condition::isPersistent() const {
	if (m_isPersistent) {
		g_logger().debug("A condi\347\343o {} \351 persistente", conditionType);
		return true;
	}
	if (ticks == -1) {
		return false;
...
int32_t ConditionAttributes::getAbsorbByIndex(uint8_t index) const {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em getAbsorbsValue: {}", index, e.what());
	}
	return 0;
}
...
void ConditionAttributes::setAbsorb(uint8_t index, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em setAbsorb: {}", index, e.what());
	}
}
...
int32_t ConditionAttributes::getAbsorbPercentByIndex(uint8_t index) const {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em getAbsorbPercentByIndex: {}", index, e.what());
	}
	return 0;
}
...
void ConditionAttributes::setAbsorbPercent(uint8_t index, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em setAbsorbPercent: {}", index, e.what());
	}
}
...
int32_t ConditionAttributes::getIncreaseByIndex(uint8_t index) const {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em getIncreaseByIndex: {}", index, e.what());
	}
	return 0;
}
...
void ConditionAttributes::setIncrease(uint8_t index, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em setIncrease: {}", index, e.what());
	}
}
...
int32_t ConditionAttributes::getIncreasePercentById(uint8_t index) const {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em getIncreasePercentById: {}", index, e.what());
	}
	return 0;
}
...
void ConditionAttributes::setIncreasePercent(uint8_t index, int32_t value) {
	try {
		...
		g_logger().error("O \355ndice {} est\341 fora do intervalo em setIncreasePercent: {}", index, e.what());
	}
}
		...
		if (internalHealthTicks >= getHealthTicks(creature)) {
			...
			if (isBuff && realHealthGain > 0) {
				if (player) {
					...
					TextMessage message(MESSAGE_HEALED, "Voc\352 foi curado por " + healString);
					...
					if (!spectators.empty()) {
						...
						message.text = player->getName() + " foi curado para " + healString;
...
bool ConditionFeared::canWalkTo(const std::shared_ptr<Creature> &creature, Position pos, Direction moveDirection) const {
	...
	if (!creature) {
		g_logger().error("[{}] criatura \351 nullptr", __FUNCTION__);
		return false;
	}
...
bool ConditionFeared::getFleePath(const std::shared_ptr<Creature> &creature, const Position &pos, std::vector<Direction> &dirList) {
	...
	do {
		for (uint8_t wsize : walkSize) {
			g_logger().debug("[{}] Verificando o \355ndice {} com walkSize de {}", __FUNCTION__, fleeIndx, wsize);
			...
			if (isStuck(creature, pos)) { // Check if it is possible to walk to any direction
				g_logger().debug("[{}] N\343o consigo caminhar para lugar nenhum", __FUNCTION__);
				return false;
			}
			...
			switch (m_directionsVector[fleeIndx]) {
				case DIRECTION_NORTH:
					futurePos.y += wsize;
					g_logger().debug("[{}] Tentando fugir para o NORTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_NORTHEAST:
					futurePos.x += wsize;
					futurePos.y -= wsize;
					g_logger().debug("[{}] Tentando fugir para o NORDESTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_EAST:
					futurePos.x -= wsize;
					g_logger().debug("[{}] Tentando fugir para LESTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_SOUTHEAST:
					futurePos.x -= wsize;
					futurePos.y += wsize;
					g_logger().debug("[{}] Tentando fugir para SUDESTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_SOUTH:
					futurePos.y += wsize;
					g_logger().debug("[{}] Tentando fugir para o SUL para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_SOUTHWEST:
					futurePos.x += wsize;
					futurePos.y += wsize;
					g_logger().debug("[{}] Tentando fugir para SUDOESTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_WEST:
					futurePos.x += wsize;
					g_logger().debug("[{}] Tentando fugir para OESTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;

				case DIRECTION_NORTHWEST:
					futurePos.x += wsize;
					futurePos.y -= wsize;
					g_logger().debug("[{}] Tentando fugir para NOROESTE para {} [{}]", __FUNCTION__, futurePos.toString(), wsize);
					break;
				case DIRECTION_NONE:
					break;
			}
	...
	g_logger().debug("[{}] Encontrado caminho dispon\355vel para {} com {} passos", __FUNCTION__, futurePos.toString(), found_size);
	return true;
}
