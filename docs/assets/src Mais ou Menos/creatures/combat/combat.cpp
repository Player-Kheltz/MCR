...
#include "mcr/spa/spa_manager.hpp"
...
void Combat::setChainCallback(uint8_t chainTargets, uint8_t chainDistance, bool backtracking) {
	...
	g_logger().trace("ChainCallback criado: {}, com alvos: {}, dist\342ncia: {}, retrocesso: {}", params.chainCallback != nullptr, chainTargets, chainDistance, backtracking);
}
...
void Combat::CombatHealthFunc(const std::shared_ptr<Creature> &caster, const std::shared_ptr<Creature> &target, const CombatParams &params, CombatDamage* data) {
	if (!data) {
		g_logger().error("[{}]: CombatDamage \351 nullptr", __FUNCTION__);
		return;
	}
	assert(data);
	...
	g_logger().trace("[{}] (antigo) eventcallback: 'creatureOnCombat', dano prim\341rio: '{}', secund\341rio: '{}'", __FUNCTION__, damage.primary.value, damage.secondary.value);
	g_callbacks().executeCallback(EventCallback_t::creatureOnCombat, caster, target, std::ref(damage));
	g_logger().trace("[{}] (novo) eventcallback: 'creatureOnCombat', dano prim\341rio: '{}', secund\341rio: '{}'", __FUNCTION__, damage.primary.value, damage.secondary.value);
	...
	if (attackerPlayer) {
		...
		if (attackerPlayer && attackerPlayer->getPlayerVocationEnum() == VOCATION_MONK_CIP && damage.origin == ORIGIN_FIST) {
			...
			const auto monkMeeleBonus = attackerPlayer->kv()->get("monge-b\341sico-atk-b\364nus");
		...
		damage.damageMultiplier += attackerPlayer->wheel().getMajorStatConditional("Empoderamento Divino", WheelMajor_t::DAMAGE);
		g_logger().trace("Multiplicador de dano do Empoderamento Divino da Roda {}", damage.damageMultiplier);
	}
	...
	if (g_game().combatChangeHealth(caster, target, damage)) {
		...
		// MCR – Event Bus do SPA: dano causado por jogador
		lua_State* L = g_luaEnvironment().getLuaState();
		if (L) {
			lua_pushstring(L, "onPlayerDealtDamage");
			Lua::pushUserdata(L, attackerPlayer);
			Lua::pushUserdata(L, target);
			lua_pushinteger(L, damage.primary.value);
			lua_pushinteger(L, static_cast<int>(damage.primary.type));
			lua_pushinteger(L, static_cast<int>(damage.secondary.type));
			SPAManager::getInstance().dispatchEvent("onPlayerDealtDamage", L, 6);
			lua_pop(L, 6);
		}
...
bool Combat::checkFearConditionAffected(const std::shared_ptr<Player> &player) {
	...
	if (party) {
		...
		g_logger().debug("[{}] O jogador \351 membro de um grupo, {} membros podem ser temidos", __FUNCTION__, affectedCount);
...
uint16_t Combat::monkEffectByElementalBond(CombatType_t combatType, uint16_t effect) {
	switch (effect) {
		...
		case CONST_ME_WHITE_ENERGY_SPARK:
			return effect; // N\343o possui variantes
		default:
			return effect;
	}
}
...
void Combat::setupChain(const std::shared_ptr<Weapon> &weapon) {
	...
	setArea(area);
	g_logger().trace("Arma: {}, tipo de elemento: {}", Item::items[weapon->getID()].name, weapon->params.combatType);
...
bool Combat::doCombatChain(const std::shared_ptr<Creature> &caster, const std::shared_ptr<Creature> &target, bool aggressive) const {
	...
	g_logger().debug("[{}] Alvos da cadeia: {}", __FUNCTION__, targets.size());
...
std::vector<std::pair<Position, std::vector<uint32_t>>> Combat::pickChainTargets(const std::shared_ptr<Creature> &caster, const CombatParams &params, uint8_t chainDistance, uint8_t maxTargets, bool backtracking, bool aggressive, const std::shared_ptr<Creature> &initialTarget /* = nullptr */) {
	...
	while (!targets.empty() && targets.size() <= maxTargets && backtrackingAttempts > 0) {
		...
		if (closestSpectator) {
			g_logger().trace("[{}] espectador mais pr\363ximo: {}", __FUNCTION__, closestSpectator->getName());
		...
		if (backtracking) {
			g_logger().debug("[{}] retrocesso", __FUNCTION__);
			targets.pop_back();
	...
	g_logger().debug("[{}] resultMap: {} em {} ms", __FUNCTION__, resultMap.size(), bm_pickChain.duration());
	return resultMap;
}
...
void ValueCallback::getMinMaxValues(const std::shared_ptr<Player> &player, CombatDamage &damage, bool useCharges) const {
	...
	if ((lua_gettop(L) + parameters + 1) != size0) {
		LuaScriptInterface::reportError(nullptr, "Tamanho da pilha alterado!");
	}
...
void MatrixArea::setValue(uint32_t row, uint32_t col, bool value) const {
	if (row < rows && col < cols) {
		...
		g_logger().error("[{}] O acesso excede o limite superior do bloco de mem\363ria");
		throw std::out_of_range("O acesso excede o limite superior do bloco de mem\363ria");
	}
}
