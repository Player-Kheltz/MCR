...
TalkActionResult_t Spells::playerSaySpell(const std::shared_ptr<Player> &player, std::string &words) {
	...
	if (maxOnline > 1 && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER && tile && !tile->hasFlag(TILESTATE_PROTECTIONZONE)) {
		...
		if (countOutsizePZ >= maxOutsizePZ) {
			player->sendTextMessage(MESSAGE_FAILURE, fmt::format("Voc\352 n\343o pode lan\347ar feiti\347os enquanto tiver {} personagem(s) fora de uma zona de prote\347\343o.", maxOutsizePZ));
	...
	if (player->hasCondition(CONDITION_FEARED)) {
		player->sendTextMessage(MESSAGE_FAILURE, "Voc\352 \351 temido.");
		return TALKACTION_FAILED;
	}
	...
	if (instant) {
		...
		if (hasInstantSpell(words)) {
			g_logger().warn("[Spells::registerInstantLuaEvent] - "
			                "Duplicar feiti\347o instant\342neo registrado com palavras: {}, em feiti\347o com nome: {}",
			                words, instantName);
			return false;
		}
...
bool Spells::registerRuneLuaEvent(const std::shared_ptr<RuneSpell> &rune) {
	if (rune) {
		...
		if (!inserted) {
			g_logger().warn(
				"[{}] runa registrada duplicada com id: {}, para script: {}",
				__FUNCTION__,
				id,
...
bool Spell::playerSpellCheck(const std::shared_ptr<Player> &player) const {
	...
	} else if (player->getVocationId() != 0 && !vocSpellMap.empty() && !vocSpellMap.contains(player->getVocationId()) && player->getGroup()->id < GROUP_TYPE_GAMEMASTER) {
...
int32_t Spell::getWheelOfDestinyBoost(WheelSpellBoost_t boost, WheelSpellGrade_t grade) const {
	...
	try {
		...
		g_logger().error("[{}] valor de nota inv\341lido, c\363digo de erro: {}", __FUNCTION__, e.what());
	}
	return value;
}
...
void Spell::applyCooldownConditions(const std::shared_ptr<Player> &player) const {
	...
	if (cooldown > 0) {
		...
		g_logger().debug("[{}] nome do feiti\347o: {}, nota: {}, originalCooldown: {}, feiti\347oCooldown: {}, b\364nus: {}, aumento {}", __FUNCTION__, name, spellGrade, cooldown, spellCooldown, player->wheel().getSpellBonus(name, WheelSpellBoost_t::COOLDOWN), augmentCooldownReduction);
...
void Spell::addVocMap(uint16_t vocationId, bool b) {
	if (vocationId == 0XFFFF) {
		g_logger().error("Estouro de voca\347\343o para feiti\347o: {}", getName());
		return;
	}

	g_logger().trace("Adicionando feiti\347o: {} ao voc id: {}", getName(), vocationId);
...
if (result) {
    // Dispara o evento de domínio após o lançamento da magia
		g_creatureEvents().playerSpell(player, getName());
