...
Imbuement* Imbuements::getImbuement(uint16_t id) {
	...
	if (it == imbuementMap.end()) {
		g_logger().warn("Imbuimento {} n\343o encontrado", id);
		return nullptr;
	}
	return &it->second;
}
...
bool Imbuements::loadFromXml(bool /* reloading */) {
	...
	for (const auto &baseNode : doc.child("imbuements").children()) {
		...
		if (strcasecmp(baseNode.name(), "base") == 0) {
			...
			if (!id) {
				g_logger().warn("ID ausente para entrada base");
				continue;
			}
			...
			pugi::xml_attribute id = baseNode.attribute("id");
			if (!id) {
				g_logger().warn("ID ausente para entrada de categoria");
				continue;
			}
			...
			if (!base) {
				g_logger().warn("ID da base de imbuement ausente");
				continue;
			}
			...
			if (groupBase == nullptr) {
				g_logger().warn("A base do grupo '{}' n\343o existe", baseid);
				continue;
			}
			...
			if (!inserted) {
				g_logger().warn("Imbuimento duplicado do ID Base: '{}' ignorado", baseid);
				continue;
			}
			...
			if (!iconBase) {
				g_logger().warn("Falta 'iconid' para entrada de imbuement");
				continue;
			}
			...
			if (!categorybase) {
				g_logger().warn("Categoria de imbuimento ausente");
				continue;
			}
			...
			if (category_p == nullptr) {
				g_logger().warn("Categoria imbuement {} n\343o existe", category);
				continue;
			}
			...
			if (!nameBase) {
				g_logger().warn("Nome do imbuement ausente");
				continue;
			}
			...
			for (const auto &childNode : baseNode.children()) {
				if (!((attr = childNode.attribute("key")))) {
					g_logger().warn("Atributo-chave ausente n\343o ID de imbuement: {}", runningid);
					continue;
				}
				...
				if (strcasecmp(type.c_str(), "item") == 0) {
					if (!((attr = childNode.attribute("value")))) {
						g_logger().warn("ID do item ausente para o nome do imbuement '{}'", imbuement.name);
						continue;
					}
					...
					if (it2 != imbuement.items.end()) {
						g_logger().warn("Item duplicado: {}, nome do equipamento: {} ignorado", childNode.attribute("value").value(), imbuement.name);
						continue;
					}
					...
					if (!((attr = childNode.attribute("type")))) {
						g_logger().warn("Tipo de efeito ausente para o nome do imbuement: {}", imbuement.name);
						continue;
					}
					...
					if (strcasecmp(effecttype.c_str(), "skill") == 0) {
						if (!((attr = childNode.attribute("value")))) {
							g_logger().warn("Valor de efeito ausente para o nome do imbuement {}", imbuement.name);
							continue;
						}
						...
						if (tmpStrValue == "sword") {
							...
							g_logger().warn("Nome da habilidade desconhecida {} n\343o nome do imbuement {}", tmpStrValue, imbuement.name);
							continue;
						}

						if (!((attr = childNode.attribute("bonus")))) {
							g_logger().warn("B\364nus de habilidade ausente para o nome de imbuimento {}", imbuement.name);
							continue;
						}
						...
						if (!((attr = childNode.attribute("combat")))) {
							g_logger().warn("Combate ausente por nome de imbuimento {}", imbuement.name);
							continue;
						}

						CombatType_t combatType = getCombatTypeByName(attr.as_string());
						if (combatType == COMBAT_NONE) {
							g_logger().warn("Tipo de combate desconhecido para o elemento {}", attr.as_string());
							continue;
						}

						if (!((attr = childNode.attribute("value")))) {
							g_logger().warn("Percentual de redu\347\343o de dano ausente para o nome do imbuement {}", imbuement.name);
							continue;
						}
						...
						if (!((attr = childNode.attribute("value")))) {
							g_logger().warn("Valor de velocidade ausente para o nome do imbuement {}", imbuement.name);
							continue;
						}
						...
						if (!((attr = childNode.attribute("value")))) {
							g_logger().warn("Valor limite ausente para o nome do imbuement {}", imbuement.name);
							continue;
						}
...
void ImbuementDecay::startImbuementDecay(const std::shared_ptr<Item> &item) {
	...
	g_logger().debug("Iniciando o decaimento de imbuimento para o item {}", item->getName());
	...
	if (m_eventId == 0) {
		...
		g_logger().trace("Verifica\347\343o programada de decaimento de imbuimento a cada 1 segundo.");
	}
}
...
void ImbuementDecay::stopImbuementDecay(const std::shared_ptr<Item> &item) {
	...
	g_logger().debug("Interrompendo a deteriora\347\343o do imbuimento para o item {}", item->getName());
	...
	if (m_itemsToDecay.empty() && m_eventId != 0) {
		...
		m_eventId = 0;
		g_logger().trace("N\343o h\341 mais itens para decair. Programador de decaimento de imbuement interrompido.");
	}
}
...
void ImbuementDecay::checkImbuementDecay() {
	...
	g_logger().trace("Verificando a deteriora\347\343o do imbuimento para itens {}.", m_itemsToDecay.size());
	...
	for (auto it = m_itemsToDecay.begin(); it != m_itemsToDecay.end();) {
		...
		if (!player) {
			g_logger().debug("O item {} n\343o est\341 nas m\343os de nenhum jogador. Ignorando a decad\352ncia.", item->getName());
			it = m_itemsToDecay.erase(it);
			continue;
		}
		...
		for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); ++slotid) {
			...
			if (!item->getImbuementInfo(slotid, &imbuementInfo)) {
				g_logger().debug("Nenhuma informa\347\343o de imbuement encontrada para o slot {} n\343o item {}", slotid, item->getName());
				continue;
			}
	...
	if (!m_itemsToDecay.empty()) {
		...
		g_logger().trace("Verifica\347\343o de deteriora\347\343o de imbuimento reprogramada.");
	} else {
		m_eventId = 0;
		g_logger().trace("N\343o h\341 mais itens para decair. Programador de decaimento de imbuement interrompido.");
	}
