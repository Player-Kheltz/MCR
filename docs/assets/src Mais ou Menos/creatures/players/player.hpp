...
struct StairTransition {
    Position origin;   // tile da escada (onde o jogador estava antes de subir/descer)
    Position destination; // tile de destino (onde o jogador apareceu)
    uint64_t timestamp;   // para timeouts
    bool isActive;        // true = escada ativa (precisa de teleporte), false = escada passiva
};

struct SurfaceWaypoint {
    Position pos;
    uint64_t timestamp;   // última vez que foi usado (ou criado)
};

static constexpr uint64_t SURFACE_WAYPOINT_EXPIRY_MS = 120000;
...
public:
	...
	std::shared_ptr<const Player> getPlayer() const override {
		...
		// Sintonia Natural data
	struct SintoniaData {
		uint16_t dominioId;
		int64_t timestamp;
	};
	std::deque<SintoniaData> recentActions;

	// Na seção public: da classe Player
	void openRemoteContainer(std::shared_ptr<Container> container);
	void closeContainersByCreature(const std::shared_ptr<Creature> &creature);

	// MCR – Containers remotos (escudeiro, etc.)
	void addRemoteContainer(std::shared_ptr<Container> container);
	bool isRemoteContainer(std::shared_ptr<Container> container) const;
	void removeRemoteContainer(std::shared_ptr<Container> container);
    ...
    const std::deque<StairTransition>& getStairTransitions() const { return m_stairTransitions; }
    void addStairTransition(const StairTransition& trans) { m_stairTransitions.push_back(trans); }
    void popFrontStairTransition() { if (!m_stairTransitions.empty()) m_stairTransitions.pop_front(); }
    void clearStairTransitions() { m_stairTransitions.clear(); }
    bool hasStairTransitions() const { return !m_stairTransitions.empty(); }
	void onFloorChange(const Position& fromPos, const Position& toPos) override;
	std::pair<Position, Position> peekFirstFloorChangeForZ(uint8_t z) const;
	const std::deque<SurfaceWaypoint>& getSurfaceWaypoints() const { return m_surfaceWaypoints; }
	void addSurfaceWaypoint(const Position& pos);
	void touchSurfaceWaypoint(size_t index);   // atualiza timestamp do waypoint no índice
	

	// Retorna a fila apenas para leitura (const)
    const std::deque<std::pair<Position, Position>>& getFloorChangeQueue() const { 
        return m_floorChangeQueue; 
    }

    // Remove uma quantidade específica de elementos do início da fila
    void popFloorChangeQueueElements(size_t amount) {
        while (amount > 0 && !m_floorChangeQueue.empty()) {
	...
	// MCR - DomÃ­nios e Afinidade
	int32_t getDominioAfinidade(uint16_t dominioId) const;
	void addDominioAfinidade(uint16_t dominioId, int32_t amount);
	void setDominioDespertado(uint16_t dominioId, bool despertado);
	bool hasHabilidade(uint16_t habilidadeId) const;
	void learnHabilidade(uint16_t habilidadeId);
	void unlearnHabilidade(uint16_t habilidadeId);
	void unlearnAllHabilidades();
	void learnHabilidadeBatch(const std::vector<uint16_t> &habilidadeIds);
	int32_t getNaturalSpeed() const;
	void setMovementSpeed(int32_t speed);


	// MCR – Modificadores de combate dinâmicos
	std::map<std::string, double> m_combatModifiers;
	double getCombatModifier(const std::string& key) const;
	void setCombatModifier(const std::string& key, double value);
	int32_t getVocationBaseMaxHealth() const;
	int32_t getVocationBaseMaxMana() const;

	// MCR - Sintonia
	void registerDominioAction(uint16_t dominioId);
	// NOVO MEMBRO: mapa de estados ativos por domÃ­nio (0 = nenhum, 1 = Lampejo, 2 = VÃ­nculo)
	std::unique_ptr<std::map<uint16_t, uint8_t>> activeStates;
	bool activeStatesReady = false; 
    mutable std::recursive_mutex dominioMutex;   // único mutex para toda a sintonia
	std::vector<SintoniaData> getActionHistory(size_t maxCount) const;
	void setDominioState(uint16_t dominioId, uint8_t state);
	uint8_t getDominioState(uint16_t dominioId) const;
	const std::map<uint16_t, uint8_t>& getAllActiveStates() const;
	void clearActionHistory();
	// MCR – Storages em lote
	std::map<uint32_t, int32_t> getStorages(std::vector<uint32_t> keys) const;
	void setStorages(std::map<uint32_t, int32_t> data);
	void closeRemoteContainer();
	bool m_forceCloseRemote = false;

	// MCR – Afinidades em lote
	void addDominioAfinidadeBatch(std::map<uint16_t, int32_t> data);
	std::map<uint16_t, int32_t> getDominioAfinidades(std::vector<uint16_t> dominios) const;
	...
	// MCR - Modificador de dano recebido (1.0 = normal)
	float getDamageTakenModifier() const;
	void setDamageTakenModifier(float modifier);

	// MCR â Modificador de velocidade de ataque (1.0 = normal)
	
	float getAttackSpeedModifier() const;
	void setAttackSpeedModifier(float modifier);
	
	// MCR - Modificador de regeneraÃ§Ã£o de mana (1.0 = normal)
	
	float getManaRegenModifier() const;
	void setManaRegenModifier(float modifier);
	
	float getManaRegenTicksModifier() const;
	void setManaRegenTicksModifier(float modifier);

	// MCR - Modificador de regeneraÃ§Ã£o de vida (1.0 = normal)
	
	float getHealthRegenModifier() const;
	void setHealthRegenModifier(float modifier);
	
	float getHealthRegenTicksModifier() const;
	void setHealthRegenTicksModifier(float modifier);

	// MCR - Modificador de Range
	
	int32_t getRangeModifier() const {
    	return rangeModifier;
	}
	void setRangeModifier(int32_t mod) {
    	rangeModifier = mod;
	}

	// MCR â Furtividade (Postura de Guarda com arma de distÃ¢ncia)
	void setStealth(bool stealth);
	bool isStealth() const;

	// MCR â Posturas de Combate
	bool canPushCreaturesFlag = false;
	void setCanPushCreatures(bool value) { canPushCreaturesFlag = value; }
	bool canPushCreatures() const { return canPushCreaturesFlag; }

	// MCR - Vida e Mana Leech
	void setLifeLeechModifier(float mod) { lifeLeechModifier = mod; }
	float getLifeLeechModifier() const { return lifeLeechModifier; }
	void setManaLeechModifier(float mod) { manaLeechModifier = mod; }
	float getManaLeechModifier() const { return manaLeechModifier; }
...
private:
    std::string getVocNameForPronoun(bool female) const;
	int32_t m_remoteContainerCID = -1;
...
private:
	...
	// MCR Private
    std::deque<StairTransition> m_stairTransitions;  // substitui m_currentStairTransition
	std::deque<SurfaceWaypoint> m_surfaceWaypoints;

	// Fila de transições de piso: par (origem, destino)
    std::deque<std::pair<Position, Position>> m_floorChangeQueue;

	// MCR – Lista de containers remotos abertos
	std::vector<std::shared_ptr<Container>> m_remoteContainers;
	
	bool stealth = false;
    float lifeLeechModifier = 0.0f;
    float manaLeechModifier = 0.0f;
	float attackSpeedModifier = 1.0f;
	float manaRegenModifier = 1.0f;
	float healthRegenModifier = 1.0f;
	float manaRegenTicksModifier = 1.0f;
	float healthRegenTicksModifier = 1.0f;
	float damageTakenModifier = 1.0f;
	int32_t rangeModifier = 0;
