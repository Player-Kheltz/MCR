...
void Party::disband() {
	...
	currentLeader->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, "Seu grupo foi dissolvido.");
	...
	for (const auto &member : members) {
		...
		member->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, "Seu grupo foi dissolvido.");
	}
...
bool Party::leaveParty(const std::shared_ptr<Player> &player, bool forceRemove /* = false */) {
	...
	player->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, "Voc\352 saiu da festa.");
	...
	ss << player->getName() << " saiu da festa.";
...
bool Party::passPartyLeadership(const std::shared_ptr<Player> &player) {
	...
	std::ostringstream ss;
	ss << player->getName() << " \351 agora o l\355der do partido.";
	...
	player->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, "Voc\352 agora \351 o l\355der do partido.");
	return true;
}
...
bool Party::joinParty(const std::shared_ptr<Player> &player) {
	...
	std::ostringstream ss;
	ss << player->getName() << " juntou-se \340 festa.";
	broadcastPartyMessage(MESSAGE_PARTY_MANAGEMENT, ss.str());
	...
	ss.str(std::string());
	ss << "Voc\352 se juntou " << leaderName << "'" << (leaderName.back() == 's' ? "" : "s") << " festa. Abra o canal do grupo para se comunicar com seus companheiros.";
...
void Party::revokeInvitation(const std::shared_ptr<Player> &player) {
	...
	std::ostringstream ss;
	ss << leader->getName() << " revogou " << leader->getPossessivePronoun() << " invitation.";
	player->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, ss.str());
...
bool Party::invitePlayer(const std::shared_ptr<Player> &player) {
	...
	std::ostringstream ss;
	ss << player->getName() << " foi convidado para participar da festa (faixa de compartilhamento: " << getMinLevel() << "-" << getMaxLevel() << ").";
	...
	ss.str(std::string());
	ss << leader->getName() << " convidou voc\352 para " << leader->getPossessivePronoun() << " festa (faixa de compartilhamento: " << getMinLevel() << "-" << getMaxLevel() << ").";
	player->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, ss.str());
...
const char* Party::getSharedExpReturnMessage(SharedExpStatus_t value) const {
	switch (value) {
		case SHAREDEXP_OK:
			return "A Experi\352ncia Compartilhada agora est\341 ativa.";
		case SHAREDEXP_TOOFARAWAY:
			return "A Experi\352ncia Compartilhada foi ativada, mas alguns membros do seu grupo est\343o muito distantes.";
		case SHAREDEXP_LEVELDIFFTOOLARGE:
			return "A Experi\352ncia Compartilhada foi ativada, mas a distribui\347\343o de n\355veis do seu grupo \351 muito grande.";
		case SHAREDEXP_MEMBERINACTIVE:
			return "A Experi\352ncia Compartilhada foi ativada, mas alguns membros do seu grupo est\343o inativos.";
		case SHAREDEXP_EMPTYPARTY:
			return "A Experi\352ncia Compartilhada foi ativada, mas voc\352 est\341 sozinho n\343o seu grupo.";
		default:
			return "Ocorreu um erro. Não foi possível ativar a experi\352ncia compartilhada.";
	}
}
...
bool Party::setSharedExperience(const std::shared_ptr<Player> &player, bool newSharedExpActive, bool silent /*= false*/) {
	...
	if (newSharedExpActive) {
		...
		if (!silent) {
			leader->sendTextMessage(MESSAGE_PARTY_MANAGEMENT, "A Experi\352ncia Compartilhada foi desativada.");
		}
	}
