...
namespace {
	...
	bool checkSpellArea(const std::array<SpellType, 5> &spellsTable, const std::string &spellName, uint8_t stage) {
		for (const auto &spellTable : spellsTable) {
			...
			g_logger().debug("\341rea de feiti\347o est\341gio {}, grau {}", stage, size);
			if (spellTable.name == spellName && stage < static_cast<uint8_t>(size)) {
				...
				if (spellData.increase.area) {
					g_logger().debug("[{}] feiti\347o com nome {}, e est\341gio {} tem \341rea de aumento", __FUNCTION__, spellName, stage);
	...
	int checkSpellAdditionalTarget(const std::array<SpellType, 5> &spellsTable, std::string_view spellName, uint8_t stage) {
		for (const auto &spellTable : spellsTable) {
			auto size = std::ssize(spellTable.grade);
			g_logger().debug("soletrar est\341gio alvo {}, grau {}", stage, size);
			if (spellTable.name == spellName && stage < static_cast<uint8_t>(size)) {
				const auto &spellData = spellTable.grade[stage];
	...
	int checkSpellAdditionalDuration(const std::array<SpellType, 5> &spellsTable, std::string_view spellName, uint8_t stage) {
		for (const auto &spellTable : spellsTable) {
			auto size = std::ssize(spellTable.grade);
			g_logger().debug("est\341gio de dura\347\343o do feiti\347o {}, grau {}", stage, size);
			if (spellTable.name == spellName && stage < static_cast<uint8_t>(size)) {
				const auto &spellData = spellTable.grade[stage];
...
bool PlayerWheel::canPlayerSelectPointOnSlot(WheelSlots_t slot, bool recursive) const {
	...
	if (slot == WheelSlots_t::SLOT_GREEN_200) {
		if (playerPoints < 375u) {
			g_logger().debug("Jogador {} tentando manipular byte n\343o slot verde 200 {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		...
		if (playerPoints < 225u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_TOP_150: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		...
		if (playerPoints < 225u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_BOTTOM_150: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		if (canSelectSlotFullOrPartial(WheelSlots_t::SLOT_GREEN_MIDDLE_100)) {
			return true;
		...
		if (playerPoints < 125u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_TOP_100: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		...
		if (playerPoints < 125u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_MIDDLE_100: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		if (canSelectSlotFullOrPartial(WheelSlots_t::SLOT_GREEN_TOP_100)) {
			return true;
		...
		if (playerPoints < 125u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_BOTTOM_100: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		if (canSelectSlotFullOrPartial(WheelSlots_t::SLOT_GREEN_MIDDLE_100)) {
			return true;
		...
		if (playerPoints < 50u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_TOP_75: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		...
		if (playerPoints < 50u) {
			g_logger().debug("Jogador {} tentando manipular byte para SLOT_GREEN_BOTTOM_75: {}", m_player.getName(), fmt::underlying(slot));
			return false;
		}
		if (canSelectSlotFullOrPartial(WheelSlots_t::SLOT_GREEN_50)) {
			return true;
...
void PlayerWheel::revealGem(WheelGemQuality_t quality) {
	...
	if (gemId == 0) {
		g_logger().error("[{}] Falha ao obter o ID da gema para a qualidade {} e a voca\347\343o {}", __FUNCTION__, fmt::underlying(quality), m_player.getVocation()->getVocName());
		return;
	}
	if (!m_player.hasItemCountById(gemId, 1, false)) {
		g_logger().error("[{}] O jogador {} n\343o possui gema com id {}", __FUNCTION__, m_player.getName(), gemId);
		return;
	}
...
PlayerWheelGem &PlayerWheel::getGem(uint16_t index) {
	...
	if (revealedGemsSize <= index) {
		g_logger().error("[{}] Jogador {} tentando obter gema com \355ndice {} mas tem apenas {} gemas", __FUNCTION__, m_player.getName(), index, revealedGemsSize);
		return emptyGem;
	}
...
uint16_t PlayerWheel::getGemIndex(const std::string &uuid) const {
	...
	g_logger().error("[{}] Falha ao encontrar gema com uuid {}", __FUNCTION__, uuid);
	return 0xFF;
}
...
void PlayerWheel::destroyGem(uint16_t index) {
	...
	if (gem.locked) {
		g_logger().error("[{}] Jogador {} destruiu gema bloqueada com \355ndice {}", std::source_location::current().function_name(), m_player.getName(), index);
		return;
	}
	...
	if (lesserFragments > 0) {
		...
		if (returnValue != RETURNVALUE_NOERROR) {
			g_logger().error("Falha ao adicionar {} fragmentos menores ao jogador com nome {}", lesserFragments, m_player.getName());
			m_player.sendCancelMessage(getReturnMessage(RETURNVALUE_CONTACTADMINISTRATOR));
			return;
		}
		g_logger().debug("[{}] O jogador {} destruiu uma gema e recebeu {} fragmentos menores", std::source_location::current().function_name(), m_player.getName(), lesserFragments);
	}
	...
	if (greaterFragments > 0) {
		...
		auto returnValue = g_game().internalPlayerAddItem(m_player.getPlayer(), fragmentsItem, false, CONST_SLOT_WHEREEVER);
		if (returnValue != RETURNVALUE_NOERROR) {
			g_logger().error("Falha ao adicionar {} fragmentos maiores ao jogador com nome {}", greaterFragments, m_player.getName());
			m_player.sendCancelMessage(getReturnMessage(RETURNVALUE_CONTACTADMINISTRATOR));
			return;
		}
		g_logger().debug("[{}] O jogador {} destruiu uma gema e recebeu {} fragmentos maiores", std::source_location::current().function_name(), m_player.getName(), greaterFragments);
	}
...
void PlayerWheel::switchGemDomain(uint16_t index) {
	...
	if (gem.locked) {
		g_logger().error("[{}] Jogador {} tentando destruir gema bloqueada com \355ndice {}", __FUNCTION__, m_player.getName(), index);
		return;
	}
...
void PlayerWheel::setActiveGem(WheelGemAffinity_t affinity, uint16_t index) {
	auto &gem = getGem(index);
	if (!gem) {
		g_logger().error("[{}] Falha ao carregar gem com \355ndice {}", __FUNCTION__, index);
		return;
	}
...
void PlayerWheel::addGems(NetworkMessage &msg) const {
	...
	g_logger().debug("[{}] O jogador {} tem {} joias ativas", __FUNCTION__, m_player.getName(), activeGems.size());
	for (const auto &gem : activeGems) {
		auto index = getGemIndex(gem.uuid);
		g_logger().debug("[{}] Adicionando gema ativa: {} com \355ndice {}", __FUNCTION__, gem.toString(), index);
	...
	for (const auto &gem : m_revealedGems) {
		g_logger().debug("[{}] Adicionando gema revelada: {}", __FUNCTION__, gem.toString());
...
void PlayerWheel::addGradeModifiers(NetworkMessage &msg) const {
	...
	if (modsSupremeIt != modsSupremePositionByVocation.end()) {
		...
		g_logger().error("[{}] ID base de voca\347\343o: {}", std::source_location::current().function_name(), m_player.getVocation()->getBaseId());
	}
}
...
void PlayerWheel::improveGemGrade(WheelFragmentType_t fragmentType, uint8_t pos) {
	...
	switch (fragmentType) {
		...
		case WheelFragmentType_t::Greater:
			...
			break;
		default:
			g_logger().error("[{}] Tipo de fragmento inv\341lido: {}", std::source_location::current().function_name(), static_cast<uint8_t>(fragmentType));
			return;
	}

	if (value == 0 && quantity == 0) {
		g_logger().error("[{}] Jogador {} tentando atualizar gema para nota superior a 3", std::source_location::current().function_name(), m_player.getName());
		return;
	}

	if (!m_player.hasItemCountById(fragmentId, quantity, true)) {
		g_logger().error("[{}] O jogador {} n\343o possui os {} fragmentos necess\341rios com id {}", std::source_location::current().function_name(), m_player.getName(), quantity, fragmentId);
		return;
	}
...
void PlayerWheel::sendOpenWheelWindow(NetworkMessage &msg, uint32_t ownerId) {
	...
	if (!voc) {
		g_logger().error("[{}] Falha ao obter voca\347\343o para o jogador {}", __FUNCTION__, m_player.getName());
		return;
	}
...
void PlayerWheel::saveSlotPointsOnPressSaveButton(NetworkMessage &msg) {
	...
	for (auto slot : magic_enum::enum_values<WheelSlots_t>()) {
		...
		if (slotPoints > maxPointsPerSlot) {
			m_player.sendTextMessage(MESSAGE_TRADE, "Algo deu errado, tente fazer entrar novamente e tente novamente ou entre em contato com o administrador");
			g_logger().error("[{}] poss\355vel manipula\347\343o do pacote do cliente usando programa n\343o autorizado", __FUNCTION__);
			g_logger().warn("Jogador: {}, erro n\343o slot: {}, total de pontos: {}, m\341ximo de pontos: {}", m_player.getName(), slotPoints, slot, maxPointsPerSlot);
			return;
		}
	...
	if (!sortedTableRetry.empty()) {
		m_player.sendTextMessage(MESSAGE_TRADE, "Algo deu errado, tente relogar e tente novamente");
	...
	g_logger().debug("Jogador: {} s\343o salvas as informa\347\365es de todos os slots em: {} milissegundos", m_player.getName(), bm_saveSlot.duration());
}
...
void PlayerWheel::loadKVScrolls() {
	const auto &scrollKv = m_player.kv()->scoped("roda do destino")->scoped("scrolls");
	if (!scrollKv) {
		return;
	}
...
void PlayerWheel::loadDBPlayerSlotPointsOnLogin() {
	...
	for (size_t i = 0; i < size; i++) {
		...
		if (propStream.read<uint8_t>(slot) && propStream.read<uint16_t>(points)) {
			...
			g_logger().debug("Jogador: {}, carregado aponta {} para o slot {}", m_player.getName(), points, slot);
		}
	}
}
...
bool PlayerWheel::saveDBPlayerSlotPointsOnLogout() const {
	...
	for (uint8_t i = 1; i < wheelSlots.size(); ++i) {
		...
		g_logger().debug("Jogador: {}, salvou pontos {} n\343o slot {}", m_player.getName(), value, i);
	}
...
bool PlayerWheel::canSelectSlotFullOrPartial(WheelSlots_t slot) const {
	if (getPointsBySlotType(slot) == getMaxPointsPerSlot(slot)) {
		g_logger().debug("[{}] pontos n\343o slot {}, m\341ximo de pontos {}", __FUNCTION__, getPointsBySlotType(slot), getMaxPointsPerSlot(slot));
		return true;
	}
	g_logger().debug("[{}] slot {} n\343o est\341 cheio", __FUNCTION__, fmt::underlying(slot));
	return false;
}
...
uint8_t PlayerWheel::getMaxPointsPerSlot(WheelSlots_t slot) const {
	...
	g_logger().error("[{}] jogador: {}, est\341 tentando usar slot desconhecido: {}", __FUNCTION__, m_player.getName(), fmt::underlying(slot));
	return 0u;
}
...
void PlayerWheel::printPlayerWheelMethodsBonusData(const PlayerWheelMethodsBonusData &bonusData) const {
	g_logger().debug("Inicializando a impress\343o das informa\347\365es do WhelPlayerBonusData para o jogador {}", m_player.getName());

	g_logger().debug("Stats:");
	if (bonusData.stats.health > 0) {
		g_logger().debug("  sa\372de: {}", bonusData.stats.health);
	}
	if (bonusData.stats.mana > 0) {
		g_logger().debug("  mana: {}", bonusData.stats.mana);
	}
	if (bonusData.stats.capacity > 0) {
		g_logger().debug("  capacidade: {}", bonusData.stats.capacity);
	}
	if (bonusData.stats.damage > 0) {
		g_logger().debug("  dano: {}", bonusData.stats.damage);
	}
	if (bonusData.stats.healing > 0) {
		g_logger().debug("  cura: {}", bonusData.stats.healing);
	}

	g_logger().debug("Resson\342ncia Vascular:");
	for (size_t i = 0; i < bonusData.unlockedVesselResonances.size(); ++i) {
		...
		g_logger().debug("  Afinidade: {} contagem: {}", affinityName, bonusData.unlockedVesselResonances[i]);
	}

	g_logger().debug("Skills:");
	if (bonusData.skills.melee > 0) {
		g_logger().debug("  corpo a corpo: {}", bonusData.skills.melee);
	}
	if (bonusData.skills.distance > 0) {
		g_logger().debug("  dist\342ncia: {}", bonusData.skills.distance);
	}
	if (bonusData.skills.magic > 0) {
		g_logger().debug("  magia: {}", bonusData.skills.magic);
	}
	...
	if (bonusData.leech.lifeLeech > 0) {
		g_logger().debug("  vidaLeech: {}", bonusData.leech.lifeLeech);
	}

	g_logger().debug("Instant:");
	if (bonusData.instant.battleInstinct) {
		g_logger().debug("  batalhaInstinto: {}", bonusData.instant.battleInstinct);
	}
	if (bonusData.instant.battleHealing) {
		g_logger().debug("  batalhaCura: {}", bonusData.instant.battleHealing);
	}
	if (bonusData.instant.positionalTactics) {
		g_logger().debug("  t\341ticas posicionais: {}", bonusData.instant.positionalTactics);
	}
	if (bonusData.instant.ballisticMastery) {
		g_logger().debug("  bal\355sticaMaestria: {}", bonusData.instant.ballisticMastery);
	}
	if (bonusData.instant.healingLink) {
		g_logger().debug("  curaLink: {}", bonusData.instant.healingLink);
	}
	if (bonusData.instant.runicMastery) {
		g_logger().debug("  Maestria r\372nica: {}", bonusData.instant.runicMastery);
	}
	if (bonusData.instant.focusMastery) {
		g_logger().debug("  focoMaestria: {}", bonusData.instant.focusMastery);
	}
	...
	if (bonusData.stages.giftOfLife > 0) {
		g_logger().debug("  presenteDaVida: {}", bonusData.stages.giftOfLife);
	}
	if (bonusData.stages.divineEmpowerment > 0) {
		g_logger().debug("  Empoderamento divino: {}", bonusData.stages.divineEmpowerment);
	}
	if (bonusData.stages.divineGrenade > 0) {
		g_logger().debug("  granada divina: {}", bonusData.stages.divineGrenade);
	}
	if (bonusData.stages.blessingOfTheGrove > 0) {
		g_logger().debug("  b\352n\347\343oOfTheGrove: {}", bonusData.stages.blessingOfTheGrove);
	}
	if (bonusData.stages.drainBody > 0) {
		g_logger().debug("  drenarBody: {}", bonusData.stages.drainBody);
	}
	if (bonusData.stages.beamMastery > 0) {
		g_logger().debug("  feixeMastery: {}", bonusData.stages.beamMastery);
	}
	...
	if (bonusData.stages.executionersThrow > 0) {
		g_logger().debug("  carrascosThrow: {}", bonusData.stages.executionersThrow);
	}

	g_logger().debug("Avatar:");
	if (bonusData.avatar.light > 0) {
		g_logger().debug("  luz: {}", bonusData.avatar.light);
	}
	if (bonusData.avatar.nature > 0) {
		g_logger().debug("  natureza: {}", bonusData.avatar.nature);
	}
	if (bonusData.avatar.steel > 0) {
		g_logger().debug("  a\347o: {}", bonusData.avatar.steel);
	}
	if (bonusData.avatar.storm > 0) {
		g_logger().debug("  tempestade: {}", bonusData.avatar.storm);
	}

	if (bonusData.momentum > 0) {
		g_logger().debug("b\364nus: {}", bonusData.momentum);
	}

	if (bonusData.mitigation > 0) {
		g_logger().debug("mitiga\347\343o: {}", bonusData.mitigation);
	}
	...
	g_logger().debug("Impress\343o dos dados do jogador conclu\355da!");
}
...
void PlayerWheel::processActiveGems() {
	...
	for (const auto &[uuid, locked, affinity, quality, basicModifier1, basicModifier2, supremeModifier] : activeGems) {
		if (uuid.empty()) {
			g_logger().error("[{}] O jogador {} tem uma gema vazia uuid", __FUNCTION__, playerName);
			continue;
		}
	...
	g_logger().debug("[{}] gemas ativas: {} ", __FUNCTION__, activeGems.size());
...
WheelStageEnum_t PlayerWheel::getPlayerSliceStage(const std::string &color) const {
	...
	if (color == "green") {
		...
		g_logger().error("[{}] erro n\343o jogador da roda {} cor: {}, n\343o corresponde a nenhuma verifica\347\343o e foi ignorado", __FUNCTION__, color, m_player.getName());
	}
	...
	if (affinityNumber < m_bonusRevelationPoints.size()) {
		...
		if (bonusRevelationPoints > 0) {
			...
			g_logger().debug("[{}] Jogador: {}, tem afinidade: {}, pontos de revela\347\343o: {} total de pontos: {}, rela\347\365es: {}", __FUNCTION__, m_player.getName(), magic_enum::enum_name(affinity), bonusRevelationPoints, totalPoints, m_bonusRevelationPoints.size());
		}
	}
...
void PlayerWheel::setStage(WheelStage_t type, uint8_t value) {
	...
	try {
		...
		g_logger().error("[{}]. O tipo {} est\341 fora do alcance. Mensagem de erro: {}", __FUNCTION__, enumValue, e.what());
	}
}
...
void PlayerWheel::setOnThinkTimer(WheelOnThink_t type, int64_t time) {
	...
	try {
		...
		g_logger().error("[{}]. O tipo {} est\341 fora do intervalo, o valor {}. Mensagem de erro: {}", __FUNCTION__, enumValue, time, e.what());
	}
}
...
void PlayerWheel::setMajorStat(WheelMajor_t type, int32_t value) {
	...
	try {
		...
		g_logger().error("[{}]. O tipo {} est\341 fora do intervalo, o valor {}. Mensagem de erro: {}", __FUNCTION__, enumValue, value, e.what());
	}
}
...
void PlayerWheel::addStat(WheelStat_t type, int32_t value) {
	...
	if (enumValue >= static_cast<uint8_t>(WheelStat_t::TOTAL_COUNT)) {
		g_logger().error("[{}]. O tipo {} est\341 fora do intervalo, o valor {}. Mensagem de erro: {}", __FUNCTION__, enumValue, value, "O valor Enum est\341 fora do intervalo");
		return;
	}
...
void PlayerWheel::addResistance(CombatType_t type, int32_t value) {
	...
	if (index >= static_cast<uint8_t>(WheelStat_t::TOTAL_COUNT)) {
		g_logger().error("[{}]. O tipo {} est\341 fora do intervalo, o valor {}. Mensagem de erro: {}", __FUNCTION__, index, value, "O valor Enum est\341 fora do intervalo");
		return;
	}
...
bool PlayerWheel::getInstant(WheelInstant_t type) const {
	...
	try {
		...
		g_logger().error("[{}]. Tipo instant\342neo {}. Mensagem de erro: {}", __FUNCTION__, enumValue, e.what());
	}
	return false;
}
...
int32_t PlayerWheel::getResistance(CombatType_t type) const {
	...
	try {
		...
		g_logger().error("[{}]. Tipo instant\342neo {}. Mensagem de erro: {}", __FUNCTION__, index, e.what());
	}
	return 0;
}
...
uint16_t PlayerWheel::getPointsBySlotType(WheelSlots_t slotType) const {
	try {
		...
		g_logger().error("[{}]. O \355ndice {} est\341 fora do intervalo, tipo de slot inv\341lido. Mensagem de erro: {}", __FUNCTION__, slotType, e.what());
		return 0;
	}
}
...
std::string PlayerWheelGem::toString() const {
	return fmt::format("[PlayerWheelGem] uuid: {}, bloqueado: {}, afinidade: {}, qualidade: {}, basicModifier1: {}, basicModifier2: {}, supremoModifier: {}", uuid, locked, static_cast<IntType>(affinity), static_cast<IntType>(quality), static_cast<IntType>(basicModifier1), static_cast<IntType>(basicModifier2), static_cast<IntType>(supremeModifier));
}
