...
void PlayerVIP::notifyStatusChange(const std::shared_ptr<Player> &loginPlayer, VipStatus_t vipStatus, bool message) const {
	...
	if (message) {
		if (vipStatus == VipStatus_t::Online) {
			m_player.sendTextMessage(TextMessage(MESSAGE_FAILURE, fmt::format("{} conectou.", loginPlayer->getName())));
		} else if (vipStatus == VipStatus_t::Offline) {
			m_player.sendTextMessage(TextMessage(MESSAGE_FAILURE, fmt::format("{} foi desconectado.", loginPlayer->getName())));
		}
	}
}
...
bool PlayerVIP::add(uint32_t vipGuid, const std::string &vipName, VipStatus_t vipStatus) {
	if (vipGuids.size() >= getMaxEntries() || vipGuids.size() == 200) { // max number of buddies is 200 in 9.53
		m_player.sendTextMessage(MESSAGE_FAILURE, "Voc\352 n\343o pode adicionar mais amigos.");
		return false;
	}

	if (!vipGuids.insert(vipGuid).second) {
		m_player.sendTextMessage(MESSAGE_FAILURE, "Este jogador j\341 est\341 na sua lista.");
		return false;
	}
...
void PlayerVIP::addGroupInternal(uint8_t groupId, const std::string &name, bool customizable) {
	if (getGroupByName(name) != nullptr) {
		g_logger().debug("{} - O nome do grupo j\341 existe.", __FUNCTION__);
		return;
	}

	const auto freeId = getFreeId();
	if (freeId == 0) {
		g_logger().debug("{} - Nenhum ID dispon\355vel.", __FUNCTION__);
		return;
	}
...
void PlayerVIP::addGroup(const std::string &name, bool customizable /*= true */) {
	if (getGroupByName(name) != nullptr) {
		m_player.sendCancelMessage("J\341 existe um grupo com este nome. Por favor escolha outro nome.");
		return;
	}

	const auto freeId = getFreeId();
	if (freeId == 0) {
		g_logger().warn("{} - Nenhum ID dispon\355vel.", __FUNCTION__);
		return;
	}
