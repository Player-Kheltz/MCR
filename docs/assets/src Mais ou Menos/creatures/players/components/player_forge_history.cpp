 ...
 * Copyright (\251) 2019-2022 OpenTibiaBR <opentibiabr@outlook.com>
...
bool PlayerForgeHistory::load() {
	...
	if (!result) {
		g_logger().debug("Nenhum hist\363rico de forja encontrado para jogador com ID: {}", playerGUID);
...
bool PlayerForgeHistory::save() {
	...
	for (const auto &history : m_modifiedHistory) {
		...
		if (!insertQuery.addRow(row)) {
			g_logger().warn("Falha ao adicionar entrada de hist\363rico de forja para jogador com ID: {}", playerGUID);
			return false;
		}

		g_logger().debug("Adicionada data de entrada n\343o hist\363rico da forja: {}, para jogador com ID: {}", formatDate(history.createdAt / 1000), playerGUID);
	}

	if (!insertQuery.execute()) {
		g_logger().error("Falha ao executar inser\347\343o de entradas do hist\363rico forjado para jogador com ID: {}", playerGUID);
		return false;
	}
