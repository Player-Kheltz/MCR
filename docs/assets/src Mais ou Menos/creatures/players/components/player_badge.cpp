...
void PlayerBadge::loadUnlockedBadges() {
	...
	g_logger().debug("[{}] - Carregando emblemas desbloqueados: {}", __FUNCTION__, unlockedBadges.size());
	for (const auto &badgeName : unlockedBadges) {
		const Badge &badge = g_game().getBadgeByName(badgeName);
		if (badge.m_id == 0) {
			g_logger().error("[{}] - Emblema {} n\343o encontrado.", __FUNCTION__, badgeName);
			continue;
		}

		g_logger().debug("[{}] - Distintivo {} encontrado para o jogador {}.", __FUNCTION__, badge.m_name, m_player.getName());
