...
bool Vocations::loadFromXml() {
	...
	for (const auto &vocationNode : doc.child("vocations").children()) {
		...
		if (!((attr = vocationNode.attribute("id")))) {
			g_logger().warn("[{}] - ID de voca\347\343o ausente", __FUNCTION__);
			continue;
		}
				...
				if (skillIdAttribute) {
					...
					if (skill_id <= SKILL_LAST) {
						...
						g_logger().warn("[Vocations::loadFromXml] - "
						                "Nenhum ID de habilidade v\341lido: {} para voca\347\343o: {}",
						                skill_id, voc->id);
					}
				} else {
					g_logger().warn("[Vocations::loadFromXml] - "
					                "ID de habilidade ausente para voca\347\343o: {}",
					                voc->id);
				}
...
std::shared_ptr<Vocation> Vocations::getVocation(uint16_t id) {
	...
	if (it == vocationsMap.end()) {
		g_logger().warn("[Vocations::getVocation] - "
		                "Voca\347\343o {} n\343o encontrada",
		                id);
		return nullptr;
	}
	return it->second;
...
std::vector<WheelGemSupremeModifier_t> Vocation::getSupremeGemModifiers() {
	...
	g_logger().debug("Carregando modificadores de gemas supremas para voca\347\343o: {}", vocationName);
	...
	for (const auto &[value, modifierName] : allModifiers) {
		...
		g_logger().debug("Verificando o modificador de gema suprema: {}, targetVocation: {}", modifierName, targetVocation);
