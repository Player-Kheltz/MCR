...
Player::Player(std::shared_ptr<ProtocolGame> p) :
	...
	activeStates = std::make_unique<std::map<uint16_t, uint8_t>>();
	activeStatesReady = true;
}
...
void Player::setID() {
	...
	if (id == 0 && guid != 0) {
		...
		if (id == std::numeric_limits<uint32_t>::max()) {
			g_logger().error("[{}] O jogador {} tem o valor m\341ximo de 'id' de uint32_t", __FUNCTION__, getName());
		}
	}
}

std::string Player::getVocNameForPronoun(bool female) const {
    uint16_t vocId = vocation->getId();
    // Ajusta estes nomes conforme as tuas vocações
    switch (vocId) {
        case 3: return female ? "Maga"      : "Mago";       // Sorcerer
        case 4: return female ? "Druida"    : "Druida";     // Druid (neutro)
        case 2: return female ? "Paladina"   : "Paladino";
        case 1: return female ? "Cavaleira" : "Cavaleiro";  // Knight
        // Adiciona outras vocações conforme necessário
        default: return female ? "Aventureira" : "Aventureiro";  // <-- AQUI
    }
}
...
std::string Player::getDescription(int32_t lookDistance) {
	...
	if (lookDistance == -1) {
		// Determina se o pronome é feminino (fallback para sexo biológico se não definido)
		bool isFemale = (pronoun == PLAYERPRONOUN_SHE);
		if (pronoun == 0) {
			isFemale = (sex == PLAYERSEX_FEMALE);
		}

		// "a você mesmo(a)"
		if (isFemale) {
			s << "a voc\352 mesm";  // "a você mesma"
			s << "a";
		} else {
			s << "a voc\352 mesm";  // "a você mesmo"
			s << "o";
		}
		s << playerTitle << ".";

		if (group->access) {
			s << " Voc\352 \351 " << group->name << '.';
		} else if (vocation->getId() != VOCATION_NONE) {
			// Descrição da vocação com artigo e nome flexionados
			s << " Voc\352 \351 ";
			s << (isFemale ? "uma " : "um ");
			s << getVocNameForPronoun(isFemale) << '.';
		} else {
			// Antes: s << " Voc\352 n\343o tem voca\347\343o.";
			s << " Voc\352 \351 " << (isFemale ? "uma " : "um ") << getVocNameForPronoun(isFemale) << '.';
		}

		if (!loyaltyTitle.empty()) {
			s << " Voc\352 \351 um " << loyaltyTitle << ".";
		}
	...
	if (m_party) {
		if (lookDistance == -1) {
			...
			s << " " << subjectPronoun << " " << getSubjectVerb() << " em uma festa com ";
		}
	...
	if (guild && guildRank) {
		...
		s << guildRank->name << " do " << guild->getName();
		if (!guildNick.empty()) {
			s << " (" << guildNick << ')';
		}

		if (memberCount == 1) {
			s << ", que tem 1 membro, " << guild->getMembersOnline().size() << " deles on-line.";
		} else {
			s << ", que tem " << memberCount << " members, " << guild->getMembersOnline().size() << " deles on-line.";
		}
	}
	return s.str();
}
...
int32_t Player::getDefense(bool sendToClient /* = false*/) const {
	...
	if (shield) {
		...
		if (shield->getDefense() > 0) {
			defenseValue += wheel().getMajorStatConditional("Maestria de Combate", WheelMajor_t::DEFENSE);
		}
...
void Player::addSkillAdvance(skills_t skill, uint64_t count) {
	...
	if (currReqTries >= nextReqTries && vocation->getId() != 0) {
		// player has reached max skill (vocação 0 pode sempre evoluir)
		return;
	}
	...
	while ((skills[skill].tries + count) >= nextReqTries) {
		...
		ss << "Voc\352 avan\347ou para " << getSkillName(skill) << " level " << skills[skill].level << '.';
...
void Player::closeContainer(uint8_t cid) {
    auto container = getContainerByID(cid);
    if (!container) return;

    // MCR – Não fecha container remoto a menos que forçado (morte do escudeiro)
    if (isRemoteContainer(container) && !m_forceCloseRemote) {
        return;
    }

    // Remove da lista de remotos se necessário
    removeRemoteContainer(container);

    // Fecha a janela no cliente
    if (client) {
        client->sendCloseContainer(cid);
    }

    // Limpa o CID remoto se for o nosso container
    if (m_remoteContainerCID == static_cast<int32_t>(cid)) {
        m_remoteContainerCID = -1;
    }

    // Remove da lista de abertos e trata recompensas (código original)
    auto it = openContainers.find(cid);
    if (it != openContainers.end()) {
        const OpenContainer &openContainer = it->second;
        const auto &cont = openContainer.container;
        if (cont && cont->isAnyKindOfRewardChest() && !hasOtherRewardContainerOpen(cont)) {
            removeEmptyRewards();
        }
...
void Player::closeContainersByCreature(const std::shared_ptr<Creature> &creature) {
    if (!creature) return;

    // Obtém o container do inventário da criatura (se for uma Criatura SPA)
    std::shared_ptr<Container> creatureInventory;
    if (auto monster = creature->getMonster()) {
        creatureInventory = monster->getInventoryContainer();
    }

    // Itera sobre todos os containers abertos (slots 0 a 15)
    for (uint8_t cid = 0; cid < 0x10; ++cid) {
        auto container = getContainerByID(cid);
        if (!container) continue;

        // Verifica se este container é exatamente o inventário da criatura
        if (creatureInventory && container == creatureInventory) {
            closeContainer(cid);
            if (client) {
                client->sendCloseContainer(cid);
            }
...
void Player::onReceiveMail() {
	if (isNearDepotBox()) {
		sendTextMessage(MESSAGE_EVENT_ADVANCE, "Nova correspond\352ncia chegou.");
	}
}
...
void Player::closeRemoteContainer() {
    if (m_remoteContainerCID >= 0 && m_remoteContainerCID < 0x10) {
        m_forceCloseRemote = true;
        closeContainer(static_cast<uint8_t>(m_remoteContainerCID));
        sendCloseContainer(static_cast<uint8_t>(m_remoteContainerCID));
        m_forceCloseRemote = false;
        m_remoteContainerCID = -1;
    }
}
...
void Player::sendLootStats(const std::shared_ptr<Item> &item, uint8_t count) {
	...
	if (item->getID() == ITEM_GOLD_COIN || item->getID() == ITEM_PLATINUM_COIN || item->getID() == ITEM_CRYSTAL_COIN) {
		...
		const auto &npc = g_game().getNpcByName("O saqueador")
	) {
...
void Player::sendIcons() {
	...
	for (const auto &icon : magic_enum::enum_values<IconBakragore>()) {
		...
		if (condition) {
			g_logger().debug("[{}] condi\347\343o ativa encontrada Bakragore com subId {}", __FUNCTION__, magic_enum::enum_integer(icon));
			iconBakragore = icon;
		}
	}
...
void Player::sendPing() {
	...
	if (noPongTime >= 60000 && shouldForceLogout) {
		if (canLogout() && g_creatureEvents().playerLogout(static_self_cast<Player>())) {
			g_logger().info("O jogador {} foi expulso devido ao tempo limite do ping. (tem cliente: {})", getName(), client != nullptr);
			if (client) {
...
void Player::onApplyImbuement(const Imbuement* imbuement, const std::shared_ptr<Item> &item, uint8_t slot, bool protectionCharm) {
	...
	if (item->getImbuementInfo(slot, &imbuementInfo)) {
		...
		this->sendImbuementResult("Ocorreu um erro, reabra a janela de imbuement.");
		return;
	}
	...
	for (uint8_t i = 0; i < item->getImbuementSlot(); i++) {
		...
		if (item->getImbuementInfo(i, &existingImbuement) && existingImbuement.imbuement) {
			if (existingImbuement.imbuement->getName() == imbuement->getName()) {
				...
				this->sendImbuementResult("Voc\352 n\343o pode aplicar o mesmo imbuimento em v\341rios slots.");
				return;
			}
		}
		...
		if (imbuementInfo.imbuement == imbuement) {
			...
			sendImbuementResult("Esta imbui\347\343o j\341 est\341 aplicada a este item.");
			return;
		}
	}
	...
	for (auto &[key, value] : items) {
		...
		if (getItemTypeCount(key) + this->getStashItemCount(itemType.id) < value) {
			this->sendImbuementResult("Voc\352 n\343o tem todos os itens necess\341rios.");
			return;
		}
	}
	...
	if (!g_game().removeMoney(thisPlayer, price, 0, true)) {
		const std::string message = fmt::format("Voc\352 n\343o tem {} moedas de ouro.", price);
	...
	g_metrics().addCounter("equil\355brio_decremento", price, { { "player", getName() }, { "context", "aplicar_imbuement" } });
	...
	if (!protectionCharm && uniform_random(1, 100) > baseImbuement->percent) {
		...
		sendImbuementResult("Ah, n\343o!\n\nA imbui\347\343o falhou. Voc\352 perdeu as fontes astrais e o ouro que precisava para o imbuimento.\n\nDa pr\363xima vez, usar um feiti\347o de prote\347\343o para aumentar suas chances.");
		openImbuementWindow(item);
		return;
	}
...
void Player::onClearImbuement(const std::shared_ptr<Item> &item, uint8_t slot) {
	...
	if (!g_game().removeMoney(static_self_cast<Player>(), baseImbuement->removeCost, 0, true)) {
		const std::string message = fmt::format("Voc\352 n\343o tem {} moedas de ouro.", baseImbuement->removeCost);
	...
	g_metrics().addCounter("equil\355brio_decremento", baseImbuement->removeCost, { { "player", getName() }, { "context", "clear_imbuement" } });
...
void Player::openImbuementWindow(const std::shared_ptr<Item> &item) {
	...
	if (item->getImbuementSlot() <= 0) {
		this->sendTextMessage(MESSAGE_EVENT_ADVANCE, "Este item n\343o \351 imbu\355vel.");
		return;
	}

	const auto &itemParent = item->getTopParent();
	if (itemParent && itemParent != getPlayer()) {
		this->sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc\352 tem que pegar o item para imbu\355-lo.");
		return;
	}
...
void Player::updateRegeneration() const {
	...
	if (condition) {
		condition->setParam(CONDITION_PARAM_HEALTHGAIN, static_cast<int32_t>(vocation->getHealthGainAmount() * healthRegenModifier));
		condition->setParam(CONDITION_PARAM_HEALTHTICKS, static_cast<int32_t>(vocation->getHealthGainTicks() * healthRegenTicksModifier));
		condition->setParam(CONDITION_PARAM_MANAGAIN, static_cast<int32_t>(vocation->getManaGainAmount() * manaRegenModifier));
		condition->setParam(CONDITION_PARAM_MANATICKS, static_cast<int32_t>(vocation->getManaGainTicks() * manaRegenTicksModifier));
	}
}
...
void Player::removeMessageBuffer() {
	...
	if (maxMessageBuffer != 0 && MessageBufferCount <= maxMessageBuffer + 1) {
		if (++MessageBufferCount > maxMessageBuffer) {
			...
			std::ostringstream ss;
			ss << "Voc\352 est\341 silenciado por " << muteTime << " seconds.";
...
void Player::addManaSpent(uint64_t amount) {
	...
	while ((manaSpent + amount) >= nextReqMana) {
		...
		std::ostringstream ss;
		ss << "Voc\352 avan\347ou para o n\355vel m\341gico " << magLevel << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
		sendTakeScreenshot(SCREENSHOT_TYPE_SKILLUP);
...
void Player::addExperience(const std::shared_ptr<Creature> &target, uint64_t exp, bool sendText /* = false*/) {
	...
	if (sendText) {
		...
		if (isVip()) {
			...
			if (expPercent > 0) {
				expString = expString + fmt::format(" (B\364nus VIP {}%)", expPercent > 100 ? 100 : expPercent);
			}
		}

		if (handleAnimusMastery) {
			expString = fmt::format("{} (b\364nus de dom\355nio do animus {:.1f}%)", expString, (animusMasteryMultiplier - 1) * 100);
		}

		TextMessage message(MESSAGE_EXPERIENCE, "Voc\352 ganhou " + expString + (handleHazardExperience ? " (Hazard)" : ""));
	...
	if (prevLevel != level) {
		...
		std::ostringstream ss;
		ss << "You advanced from Level " << prevLevel << " para nivelar " << level << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
		sendTakeScreenshot(SCREENSHOT_TYPE_LEVELUP);
	}
...
void Player::removeExperience(uint64_t exp, bool sendText /* = false*/) {
	...
	if (sendText) {
		...
		const std::string expString = fmt::format("Voc\352 perdeu {} ponto de experi\352ncia {}.", lostExp, (lostExp != 1 ? "s" : ""));
	...
	if (oldLevel != level) {
		...
		std::ostringstream ss;
		ss << "You were downgraded from Level " << oldLevel << " para nivelar " << level << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
	}
...
void Player::death(const std::shared_ptr<Creature> &lastHitCreature) {
	...
	if (skillLoss) {
		...
		g_logger().debug("[{}] - experi\352ncia perdida {}", __FUNCTION__, expLoss);
		...
		sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc\352 est\341 morto.");
		std::ostringstream lostExp;
		lostExp << "Voc\352 perdeu " << expLoss << " experience.";
		...
		deathType << "Voc\352 morreu durante ";
		if (pvpDeath) {
			deathType << "PvP.";
		} else {
			deathType << "PvE.";
		...
		if (willNotLoseBless) {
			blessOutput << fmt::format("Voc\352 ainda tem b\352n\347\343os de aventureiro por estar n\343o n\355vel inferior a {}!", adventurerBlessingLevel);
		} else {
			bless.empty() ? blessOutput << "Voc\352 n\343o foi protegido com nenhuma b\352n\347\343o."
						  : blessOutput << "Voc\352 foi aben\347oado com " << bless;
...
std::shared_ptr<Item> Player::getCorpse(const std::shared_ptr<Creature> &lastHitCreature, const std::shared_ptr<Creature> &mostDamageCreature) {
	...
	if (corpse && corpse->getContainer()) {
		...
		ss << "Voc\352 reconhece " << getNameDescription() << ". ";
		...
		if (!responsibleName.empty()) {
			ss << getSubjectPronoun() << " " << getSubjectVerb(true) << " morto por " << responsibleName;
			...
			ss << getSubjectPronoun() << " " << getSubjectVerb(true) << " morto por " << lastHitCreature->getNameDescription();
			if (hasOthers) {
				ss << " e outros";
			}
			ss << '.';
		} else {
			ss << "Nenhum agressor foi identificado.";
		}
...
void Player::stashContainer(const StashContainerList &itemDict) {
	...
	if (totalStowed == 0) {
		sendCancelMessage("Desculpe, n\343o \351 poss\355vel.");
		return;
	}
...
void Player::parseAttackDealtHazardSystem(CombatDamage &damage, const std::shared_ptr<Monster> &monster) const {
	...
	if (monster->getHazardSystemDefenseBoost()) {
		...
		if (stage != 0) {
			damage.exString = fmt::format("(perigo -{}%)", stage / 100.);
			damage.primary.value -= static_cast<int32_t>(std::ceil((static_cast<double>(damage.primary.value) * stage) / 10000));
			if (damage.secondary.value != 0) {
				damage.secondary.value -= static_cast<int32_t>(std::ceil((static_cast<double>(damage.secondary.value) * stage) / 10000));
			}
...
float Player::getHealthRegenModifier() const {
    return healthRegenModifier;
}

void Player::setHealthRegenModifier(float modifier) {
    healthRegenModifier = std::max(0.0f, modifier);
	updateRegeneration(); // força a atualização da condição de regeneração
    sendStats();
}

float Player::getManaRegenModifier() const {
    return manaRegenModifier;
}

void Player::setManaRegenModifier(float modifier) {
    manaRegenModifier = std::max(0.0f, modifier);
    updateRegeneration(); // força a atualização da condição de regeneração
    sendStats();
}

float Player::getHealthRegenTicksModifier() const {
    return healthRegenTicksModifier;
}

void Player::setHealthRegenTicksModifier(float modifier) {
    healthRegenTicksModifier = std::max(0.1f, modifier);
    updateRegeneration();
}

float Player::getManaRegenTicksModifier() const {
    return manaRegenTicksModifier;
}

void Player::setManaRegenTicksModifier(float modifier) {
    manaRegenTicksModifier = std::max(0.1f, modifier);
    updateRegeneration();
}
...
void Player::onWalkComplete() {
    if (g_globalMonsterMap().getTiles().size() > 0) {
        Position myPos = getPosition();

        // Throttle de 500 ms – garante que a vizinhança é actualizada
        static uint64_t lastPlayerScan = 0;
        uint64_t now = getCurrentTimeMs();
        if (now - lastPlayerScan > 500) {
            size_t added = g_globalMonsterMap().scanPlayerTrail(myPos, 25);
            if (added > 0) {
                g_logger().info("[MCR-COGNI] scanPlayerTrail em {} adicionou {} tiles.", myPos.toString(), added);
            }
            lastPlayerScan = now;
        }

        // Força o tile actual como caminhável (elimina qualquer bloqueio fantasma)
        g_globalMonsterMap().addTile(myPos, true, false);
    }

    addSurfaceWaypoint(getPosition());
...
void Player::onCleanseCondition(ConditionType_t type) const {
	...
	if (it != conditionMessages.end()) {
		sendTextMessage(MESSAGE_PARTY, fmt::format("Voc\352 n\343o \351 mais {}. (charme de limpeza)", it->second));
	}
}
...
void Player::onAddCombatCondition(ConditionType_t type) {
	...
	switch (type) {
		case CONDITION_POISON:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 envenenado.");
			break;

		case CONDITION_DROWN:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 se afogando.");
			break;

		case CONDITION_PARALYZE:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 paralisado.");
			break;

		case CONDITION_DRUNK:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 b\352bado.");
			break;
		...
		case CONDITION_GREATERHEX:
			...
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 enfeiti\347ado.");
			break;
		case CONDITION_ROOTED:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 enraizado.");
			break;

		case CONDITION_FEARED:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 \351 temido.");
			break;

		case CONDITION_CURSED:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 amaldi\347oado.");
			break;

		case CONDITION_FREEZING:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 congelando.");
			break;

		case CONDITION_DAZZLED:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 deslumbrado.");
			break;

		case CONDITION_BLEEDING:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 sangrando.");
			break;

		case CONDITION_AGONY:
			sendTextMessage(MESSAGE_FAILURE, "Voc\352 est\341 em agonia.");
			break;
...
void Player::onPlacedCreature() {
	...
	if (!activeEvents.empty()) {
		...
		g_logger().info("[{}] EventScheduler ativo: {}", getName(), eventsList);
	}
}
...
void Player::addHuntingTaskKill(const std::shared_ptr<MonsterType> &mType) {
	...
	if (const auto &option = g_ioprey().getTaskRewardOption(taskSlot)) {
		...
		if ((taskSlot->upgrade && taskSlot->currentKills >= option->secondKills) || (!taskSlot->upgrade && taskSlot->currentKills >= option->firstKills)) {
			...
			const std::string message = "Voc\352 concluiu com sucesso sua tarefa de ca\347a. Sua recompensa est\341 pronta para ser reivindicada!";
...
void Player::addBestiaryKill(const std::shared_ptr<MonsterType> &mType) {
	...
	auto scopedDoubleBestiary = g_kv().scoped("eventscheduler")->get("duplo besti\341rio");
	bool doubleBestiaryEnabled = scopedDoubleBestiary && scopedDoubleBestiary->get<bool>();
	if (doubleBestiaryEnabled) {
		kills *= 2;
		g_logger().debug("[{}] besti\341rio duplo est\341 habilitado.", __FUNCTION__);
	}
...
void Player::addBosstiaryKill(const std::shared_ptr<MonsterType> &mType) {
	...
	auto scopedDoubleBosstiary = g_kv().scoped("eventscheduler")->get("duplo chefe");
	bool doubleBosstiaryEnabled = scopedDoubleBosstiary && scopedDoubleBosstiary->get<bool>();
	if (doubleBosstiaryEnabled) {
		kills *= 2;
		g_logger().debug("[{}] duplo chefe est\341 habilitado.", __FUNCTION__);
	}
...
bool Player::onKilledMonster(const std::shared_ptr<Monster> &monster) {
	...
	if (mType == nullptr) {
		g_logger().error("[{}] O tipo de monstro \351 nulo.", __FUNCTION__);
		return false;
	}
...
void Player::addUnjustifiedDead(const std::shared_ptr<Player> &attacked) {
	...
	sendTextMessage(MESSAGE_EVENT_ADVANCE, "Aviso! O assassinato de " + attacked->getName() + " n\343o foi justificado.");
...
uint32_t Player::getAttackSpeed() const {
    return static_cast<uint32_t>(vocation->getAttackSpeed() * attackSpeedModifier);
}
...
void Player::setAttackSpeedModifier(float modifier) {
    attackSpeedModifier = std::max(0.1f, modifier);
    sendStats();
}
...
double Player::getLostPercent() const {
	...
	g_logger().debug("[{}] - perdaPorcentagem {}", __FUNCTION__, lossPercent);
	g_logger().debug("[{}] - antes da promo\347\343o {}", __FUNCTION__, percentReduction);

	if (isPromoted()) {
		percentReduction += 30 / 100.;
		g_logger().debug("[{}] - ap\363s a promo\347\343o {}", __FUNCTION__, percentReduction);
	}

	g_logger().debug("[{}] - porcentagem total perdida {}", __FUNCTION__, (lossPercent * (1 - percentReduction)) / 100.);
...
uint32_t Player::getMagicLevel() const {
	...
	magic += m_wheelPlayer.getMajorStatConditional("T\341ticas Posicionais", WheelMajor_t::MAGIC); // Revelation bonus
	return magic;
}
...
uint16_t Player::getSkillLevel(skills_t skill) const {
	...
	if (skill == SKILL_FIST) {
		...
		skillLevel += m_wheelPlayer.getMajorStatConditional("T\341ticas Posicionais", WheelMajor_t::DISTANCE);
		skillLevel += m_wheelPlayer.getStat(WheelStat_t::DISTANCE);
	} else if (skill == SKILL_SHIELD) {
		skillLevel += m_wheelPlayer.getMajorStatConditional("Instinto de Batalha", WheelMajor_t::SHIELD);
	} else if (skill == SKILL_MAGLEVEL) {
		skillLevel += m_wheelPlayer.getMajorStatConditional("T\341ticas Posicionais", WheelMajor_t::MAGIC);
		...
		skillLevel += m_wheelPlayer.getMajorStatConditional("Maestria de Combate", WheelMajor_t::CRITICAL_DMG_2);
		skillLevel += m_wheelPlayer.getMajorStatConditional("Maestria Bal\355stica", WheelMajor_t::CRITICAL_DMG);
...
void Player::onThink(uint32_t interval) {
	...
	if (playerTile && !playerTile->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer() && !isExerciseTraining() && !vipStaysOnline) {
		...
		if (idleTime > (kickAfterMinutes * 60000) + 60000) {
			...
			std::ostringstream ss;
			ss << "N\343o houve varia\347\343o em seu comportamento para " << kickAfterMinutes << " minutos. Voc\352 ser\341 desconectado em um minuto se n\343o houver altera\347\343o em suas a\347\365es at\351 ent\343o.";
...
ReturnValue Player::addItemFromStash(uint16_t itemId, uint32_t itemCount) {
	...
	if (retrievableCount == 0) {
		sendMessageDialog("Capacidade insuficiente. Voc\352 n\343o conseguiu recuperar nenhum item.");
		return RETURNVALUE_NOTENOUGHROOM;
	}
	...
	if (!canAddItems) {
		sendMessageDialog("N\343o h\341 espa\347o suficiente. Voc\352 n\343o conseguiu recuperar nenhum item.");
		return RETURNVALUE_NOTENOUGHROOM;
	}
		...
		while (remainingToRetrieve > 0 && availableCapacity >= itemWeight) {
			...
			while (addValue > 0 && cacheIndex < containersCache.size()) {
				...
				if (targetContainer->capacity() > targetContainer->size()) {
					...
					if (!newItem) {
						g_logger().warn("[addItemFromStash] Falha ao criar novo itemId empilh\341vel: {} para o jogador {}", itemId, getName());
						break;
					}
			...
			if (!itemAdded && addValue > 0) {
				g_logger().warn("N\343o h\341 mais espa\347o dispon\355vel para itemId: {}, restante: {}", itemId, addValue);
				break;
			}
		}
	}
	...
	if (!itemType.stackable) {
		while (finalRetrievable > 0 && cacheIndex < containersCache.size()) {
			...
			if (targetContainer->capacity() > targetContainer->size()) {
				...
				if (!newItem) {
					g_logger().warn("[addItemFromStash] Falha ao criar novo itemId: {} para o jogador {}", itemId, getName());
					break;
				}
	...
	sendTextMessage(MESSAGE_STATUS, fmt::format("Recuperado {}x {}.", addedItemCount, itemName));
	...
	if (limitedByCapacity) {
		sendMessageDialog("Capacidade insuficiente. Voc\352 n\343o conseguiu recuperar todos os itens.");
	} else if (limitedBySlots) {
		sendMessageDialog("N\343o h\341 espa\347o suficiente. Voc\352 n\343o conseguiu recuperar todos os itens.");
	}
...
ReturnValue Player::addItemBatch(
	...
	auto checkOverflow = [&](const char* errorCode, bool setError) {
		...
		g_logger().error("[C\363digo de erro: {}] jogador: {}, estouro detectado: ActuallyAdded ({}) > totalCount ({})", errorCode, getName(), state.actuallyAdded, state.totalCount);
		if (setError) {
...
void Player::stowItem(const std::shared_ptr<Item> &item, uint32_t count, bool allItems) {
	if (!item || (!item->isItemStorable() && item->getID() != ITEM_GOLD_POUCH)) {
		sendCancelMessage("Este item n\343o pode ser guardado aqui.");
		return;
	}
	...
	if (allItems) {
		if (item->getContainer()) {
			sendCancelMessage("Voc\352 n\343o pode guardar cont\352ineres.");
			return;
		}
	...
	if (itemDict.empty()) {
		sendCancelMessage("N\343o h\341 itens guard\341veis ??neste cont\352iner.");
		return;
	}

	if (totalItemsToStow >= maxItemsToStow) {
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("Voc\352 atingiu o limite m\341ximo de armazenamento de {} itens. Tente arrumar novamente.", maxItemsToStow));
	}
...
uint8_t Player::getBlessingCount(uint8_t index, bool storeCount) const {
	if (!storeCount) {
		if (index > 0 && index <= blessings.size()) {
			...
			g_logger().error("[{}] - \355ndice fora do intervalo 0-10.", __FUNCTION__);
			return 0;
		}
	}
...
void Player::requestDepotItems() {
	...
	for (const auto &[itemId, itemCount] : getStashItems()) {
		if (Item::items[itemId].wareId <= 0) {
			g_logger().error("{} - O jogador {} possui um item inv\341lido com ID {} n\343o estoque sem bandeira de mercado", __FUNCTION__, getName(), itemId);
			continue;
		}
...
void Player::retrieveAllItemsFromDepotSearch(uint16_t itemId, uint8_t tier, bool isDepot) {
	...
	if (totalRemoved != actuallyRetrieved) {
		g_logger().error(
			"[retrieveAllItemsFromDepotSearch] Removido ({}) != Inserido ({}) para itemId: {}, jogador: {}",
...
std::pair<std::vector<std::shared_ptr<Item>>, std::map<uint16_t, std::map<uint8_t, uint32_t>>> Player::requestLockerItems(const std::shared_ptr<DepotLocker> &depotLocker, bool sendToClient, uint8_t tier) const {
	if (!depotLocker) {
		g_logger().error("{} - O arm\341rio do dep\363sito \351 nullptr", __FUNCTION__);
		return {};
	}
...
bool Player::saySpell(SpeakClasses type, const std::string &text, bool isGhostMode, const Spectators* spectatorsPtr, const Position* pos) {
	if (text.empty()) {
		g_logger().debug("{} - O texto do feiti\347o est\341 vazio para o jogador {}", __FUNCTION__, getName());
		return false;
	}
...
void Player::triggerMomentum() {
	...
	if (getZoneType() != ZONE_PROTECTION && hasCondition(CONDITION_INFIGHT) && ((OTSYS_TIME() / 1000) % 2) == 0 && chance > 0 && randomChance < chance) {
		...
		if (triggered) {
			...
			sendTextMessage(MESSAGE_ATTENTION, "O impulso foi acionado.");
		}
	}
}
	...
	while (it != conditions.end()) {
		...
		if (!spell) {
			if (type == CONDITION_SPELLCOOLDOWN) {
				++it;
				g_logger().error("{} - O jogador {} tem um tempo de espera para um SpellId {} inv\341lido.", __FUNCTION__, getName(), spellId);
				continue;
...
void Player::triggerTranscendence() {
	...
	if (getZoneType() != ZONE_PROTECTION && checkLastAggressiveActionWithin(2000) && ((OTSYS_TIME() / 1000) % 2) == 0 && chance > 0 && randomChance < chance) {
		...
		sendTextMessage(MESSAGE_ATTENTION, "A transcend\352ncia foi acionada.");
...
void Player::forgeFuseItems(ForgeAction_t actionType, uint16_t firstItemId, uint8_t tier, uint16_t secondItemId, bool success, bool reduceTierLoss, bool convergence, uint8_t bonus, uint8_t coreCount) {
	...
	if (!firstForgingItem) {
		g_logger().error("[Log 1] Jogador com nome {} falhou ao fundir item com id {}", getName(), firstItemId);
	...
	if (!secondForgingItem) {
		g_logger().error("[Log 2] Jogador com nome {} falhou ao fundir item com id {}", getName(), secondItemId);
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	if (!exaltationChest) {
		g_logger().error("Falha ao criar ba\372 de exalta\347\343o");
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	const auto &exaltationContainer = exaltationChest->getContainer();
	if (!exaltationContainer) {
		g_logger().error("Falha ao criar cont\352iner de exalta\347\343o");
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	if (!firstForgedItem) {
		g_logger().error("[Log 3] Jogador com nome {} falhou ao fundir item com id {}", getName(), firstItemId);
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	if (convergence) {
		...
		g_metrics().addCounter("equil\355brio_decremento", cost, { { "player", getName() }, { "context", "forge_convergence_fuse" } });
		history.cost = cost;
	} else {
		firstForgedItem->setTier(tier);
		const auto &secondForgedItem = Item::CreateItem(secondItemId, 1);
		if (!secondForgedItem) {
			g_logger().error("[Log 4] Jogador com nome {} falhou ao fundir item com id {}", getName(), secondItemId);
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			return;
		}
		...
		if (success) {
			...
			if (bonus != 3) {
				...
				for (const auto* itemClassification : g_game().getItemsClassifications()) {
					...
					if (!itemClassification->tiers.contains(firstForgedItem->getTier())) {
						g_logger().error("[{}] Falha ao encontrar o n\355vel {} para o item {} na classifica\347\343o {}", __FUNCTION__, firstForgedItem->getTier(), firstForgedItem->getClassification(), itemClassification->id);
						sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
						break;
					}
				...
				g_metrics().addCounter("equil\355brio_decremento", cost, { { "player", getName() }, { "context", "forge_fuse" } });
				history.cost = cost;
			}
			...
			for (const auto* itemClassification : g_game().getItemsClassifications()) {
				...
				if (!itemClassification->tiers.contains(firstForgingItem->getTier() + 1)) {
					g_logger().error("[{}] Falha ao encontrar o n\355vel {} para o item {} na classifica\347\343o {}", __FUNCTION__, firstForgingItem->getTier() + 1, firstForgingItem->getClassification(), itemClassification->id);
					sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
					break;
				}
	...
	if (returnValue != RETURNVALUE_NOERROR) {
		g_logger().error("Falha ao adicionar ba\372 de exalta\347\343o ao jogador com nome {}", getName());
		sendCancelMessage(getReturnMessage(returnValue));
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
...
void Player::forgeTransferItemTier(ForgeAction_t actionType, uint16_t donorItemId, uint8_t tier, uint16_t receiveItemId, bool convergence) {
	...
	if (!donorItem) {
		g_logger().error("[Log 1] Jogador com nome {} falhou ao transferir item com id {}", getName(), donorItemId);
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	if (!receiveItem) {
		g_logger().error("[Log 2] Jogador com nome {} falhou ao transferir item com id {}", getName(), receiveItemId);
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	const auto &exaltationChest = Item::CreateItem(ITEM_EXALTATION_CHEST, 1);
	if (!exaltationChest) {
		g_logger().error("Ba\372 de exalta\347\343o \351 nullptr");
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	const auto &exaltationContainer = exaltationChest->getContainer();
	if (!exaltationContainer) {
		g_logger().error("O cont\352iner de exalta\347\343o \351 nullptr");
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	if (!newReceiveItem) {
		g_logger().error("[Log 6] Jogador com nome {} falhou ao fundir item com id {}", getName(), receiveItemId);
		sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
		return;
	}
	...
	for (const auto &itemClassification : g_game().getItemsClassifications()) {
		...
		if (!itemClassification->tiers.contains(donorItem->getTier())) {
			g_logger().error("[{}] Falha ao encontrar o n\355vel {} para o item {} na classifica\347\343o {}", __FUNCTION__, donorItem->getTier(), donorItem->getClassification(), itemClassification->id);
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			break;
		}
	...
	history.cost = cost;
	g_metrics().addCounter("equil\355brio_decremento", cost, { { "player", getName() }, { "context", "forge_transfer" } });
...
void Player::forgeResourceConversion(ForgeAction_t actionType) {
	...
	if (actionType == ForgeAction_t::DUSTTOSLIVERS) {
		...
		if (cost > dusts) {
			g_logger().error("[{}] Poeira insuficiente", __FUNCTION__);
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			return;
		}
		...
		if (returnValue != RETURNVALUE_NOERROR) {
			g_logger().error("Falha ao adicionar fragmentos {} ao jogador com nome {}", itemCount, getName());
			sendCancelMessage(getReturnMessage(returnValue));
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			return;
		}
		...
		if (cost > sliverCount) {
			g_logger().error("[{}] Fraca insuficiente", __FUNCTION__);
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			return;
		}
		...
		if (returnValue != RETURNVALUE_NOERROR) {
			g_logger().error("Falha ao adicionar um n\372cleo ao jogador com nome {}", getName());
			sendCancelMessage(getReturnMessage(returnValue));
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			return;
		}
		...
		if (dustLevel >= g_configManager().getNumber(FORGE_MAX_DUST)) {
			g_logger().error("[{}] N\355vel m\341ximo alcan\347ado", __FUNCTION__);
			sendForgeError(RETURNVALUE_CONTACTADMINISTRATOR);
			return;
		}
	...
	if (history.actionType == ForgeAction_t::FUSION) {
		if (history.success) {
			detailsResponse << fmt::format(
				"{:s}{:s} <br><br>"
				"Parceiros de fus\343o:"
				"<ul> "
				"<li>"
				"Primeiro item: {:s} {:s}, n\355vel {:s}"
				"</li>"
				"<li>"
				"Segundo item: {:s} {:s}, n\355vel {:s}"
				"</li>"
				"</ul>"
				"<br>"
				"Result:"
		...
		detailsResponse << fmt::format("P\363 {:d} convertido em lascas {:d}.", history.cost, history.gained);
	} else if (history.actionType == ForgeAction_t::SLIVERSTOCORES) {
		history.actionType = ForgeAction_t::DUSTTOSLIVERS;
		detailsResponse << fmt::format("Fractius {:d} convertidos em n\372cleo exaltado {:d}.", history.cost, history.gained);
	} else if (history.actionType == ForgeAction_t::INCREASELIMIT) {
		history.actionType = ForgeAction_t::DUSTTOSLIVERS;
		detailsResponse << fmt::format("Gastou {:d} poeira para aumentar o limite de poeira para {:d}.", history.cost, history.gained + 1);
	} else {
...
void Player::onCreatureAppear(const std::shared_ptr<Creature> &creature, bool isLogin) {
	...
	if (isLogin && creature == getPlayer()) {
		...
		g_logger().info("{} conectou. (Protocolo: {})", name, version);
...
void Player::onRemoveCreature(const std::shared_ptr<Creature> &creature, bool isLogout) {
	...
	if (const auto &player = getPlayer(); player == creature) {
		if (isLogout) {
			...
			g_logger().info("{} desconectou-se", getName());
	...
	if (!modalWindows.empty()) {
		...
		for (const uint32_t modalWindowId : modalWindows) {
			if (modalWindowId == std::numeric_limits<uint32_t>::max()) {
				sendTextMessage(MESSAGE_EVENT_ADVANCE, "Treinamento off-line abortado.");
				break;
			}
		}
		modalWindows.clear();
...
void Player::onCloseContainer(const std::shared_ptr<Container> &container) {
    removeRemoteContainer(container);   // <--- NOVA LINHA
...
void Player::autoCloseContainers(const std::shared_ptr<Container> &container) {
    // MCR: Nunca fechar automaticamente containers remotos (escudeiro)
...
void Player::checkAndShowBlessingMessage() {
	...
	if (willNotLoseBless) {
		...
		sendBlessStatus();
		if (addedBless) {
			blessOutput << fmt::format("Voc\352 recebeu b\352n\347\343os de aventureiro por estar n\343o n\355vel inferior a {}!\nVoc\352 ainda \351 aben\347oado com {}", adventurerBlessingLevel, bless);
		}
	} else {
		bless.empty() ? blessOutput << "Voc\352 perdeu todas as suas b\352n\347\343os." : blessOutput << "Voc\352 ainda \351 aben\347oado com " << bless;
	}
...
void Player::resetOldCharms() {
	...
	if (unlockedAllCharms) {
		totalRefund += 17400;
		g_logger().info("Jogador: {}, tem todos os encantos desbloqueados. Pontos de b\364nus: 17.400", getName());
	}
	...
	g_logger().info("Jogador: {}, pontos de charme recalculados com base n\343o besti\341rio desbloqueado: {}", getName(), totalRefund);
}
...
// --- Domínios e Afinidade ---

void Player::unlearnAllHabilidades() {
    std::ostringstream query;
    query << "DELETE FROM player_habilidades WHERE player_id = " << getGUID();
    g_database().executeQuery(query.str());
}

void Player::learnHabilidadeBatch(const std::vector<uint16_t> &habilidadeIds) {
    if (habilidadeIds.empty()) return;
    std::ostringstream query;
    query << "INSERT INTO player_habilidades (player_id, habilidade_id) VALUES ";
    for (size_t i = 0; i < habilidadeIds.size(); ++i) {
        if (i > 0) query << ", ";
        query << "(" << getGUID() << ", " << habilidadeIds[i] << ")";
    }
    query << " ON DUPLICATE KEY UPDATE habilidade_id = VALUES(habilidade_id)";
    g_database().executeQuery(query.str());
}

int32_t Player::getDominioAfinidade(uint16_t dominioId) const {
    std::ostringstream query;
    query << "SELECT afinidade FROM player_dominios WHERE player_id = " << getGUID() << " AND dominio_id = " << dominioId;
    DBResult_ptr result = Database::getInstance().storeQuery(query.str());
    if (result) {
        return result->getNumber<int32_t>("afinidade");
    }
    return 0;
}

void Player::addDominioAfinidade(uint16_t dominioId, int32_t amount) {
    std::ostringstream query;
    query << "INSERT INTO player_dominios (player_id, dominio_id, afinidade, despertado) VALUES ("
          << getGUID() << ", " << dominioId << ", " << amount << ", 1) ON DUPLICATE KEY UPDATE afinidade = afinidade + " << amount;
    Database::getInstance().executeQuery(query.str());    
}

void Player::setDominioDespertado(uint16_t dominioId, bool despertado) {
    std::ostringstream query;
    query << "INSERT INTO player_dominios (player_id, dominio_id, despertado) VALUES ("
          << getGUID() << ", " << dominioId << ", " << (despertado ? 1 : 0) << ") ON DUPLICATE KEY UPDATE despertado = " << (despertado ? 1 : 0);
    Database::getInstance().executeQuery(query.str());
}

bool Player::hasHabilidade(uint16_t habilidadeId) const {
    std::ostringstream query;
    query << "SELECT 1 FROM player_habilidades WHERE player_id = " << getGUID() << " AND habilidade_id = " << habilidadeId;
    DBResult_ptr result = Database::getInstance().storeQuery(query.str());
    return result != nullptr;
}

void Player::learnHabilidade(uint16_t habilidadeId) {
    if (!hasHabilidade(habilidadeId)) {
        Database::getInstance().executeQuery(
            "INSERT INTO player_habilidades (player_id, habilidade_id) VALUES (" + std::to_string(getGUID()) + ", " + std::to_string(habilidadeId) + ")"
        );
    }
}

uint8_t Player::getDominioState(uint16_t dominioId) const {
    std::lock_guard<std::recursive_mutex> lock(dominioMutex);
    auto it = activeStates->find(dominioId);
    return it != activeStates->end() ? it->second : 0;
}

void Player::setDominioState(uint16_t dominioId, uint8_t state) {
    std::lock_guard<std::recursive_mutex> lock(dominioMutex);
    (*activeStates)[dominioId] = state;
}

void Player::registerDominioAction(uint16_t dominioId) {
    std::lock_guard<std::recursive_mutex> lock(dominioMutex);
    SintoniaData data;
    data.dominioId = dominioId;
    data.timestamp = OTSYS_TIME();
    recentActions.push_back(data);
    if (recentActions.size() > 100) {
        recentActions.pop_front();
    }
    // A linha abaixo foi COMENTADA para evitar crash por aninhamento de eventos
    // g_creatureEvents().playerDominioAction(getPlayer(), dominioId);
}

// MCR – Storages em lote
std::map<uint32_t, int32_t> Player::getStorages(std::vector<uint32_t> keys) const {
    std::map<uint32_t, int32_t> result;
    for (uint32_t key : keys) {
        int32_t value = m_storage.get(key);  // usa o método correto
        result[key] = value;
    }
    return result;
}

void Player::setStorages(std::map<uint32_t, int32_t> data) {
    for (const auto& [key, value] : data) {
        m_storage.add(key, value);  // usa o método correto
    }
}

// MCR – Afinidades em lote (escreve cada domínio individualmente na BD)
void Player::addDominioAfinidadeBatch(std::map<uint16_t, int32_t> data) {
    for (const auto& [dominioId, pontos] : data) {
        addDominioAfinidade(dominioId, pontos);
    }
}

// MCR – Afinidades em lote (consulta a BD com IN)
std::map<uint16_t, int32_t> Player::getDominioAfinidades(std::vector<uint16_t> dominios) const {
    std::map<uint16_t, int32_t> result;
    if (dominios.empty()) {
        return result;
    }

    std::ostringstream query;
    query << "SELECT dominio_id, afinidade FROM player_dominios WHERE player_id = " << getGUID() << " AND dominio_id IN (";
    ...
    if (dbResult) {
        do {
            uint16_t domId = dbResult->getNumber<uint16_t>("dominio_id");
            int32_t af = dbResult->getNumber<int32_t>("afinidade");
            result[domId] = af;
        } while (dbResult->next());
    }
    return result;
...
std::vector<Player::SintoniaData> Player::getActionHistory(size_t maxCount) const {
    std::lock_guard<std::recursive_mutex> lock(dominioMutex);
    std::vector<SintoniaData> result;
    size_t count = 0;
    for (auto it = recentActions.rbegin(); it != recentActions.rend() && count < maxCount; ++it, ++count) {
        result.push_back(*it);
    }
    return result;
}

const std::map<uint16_t, uint8_t>& Player::getAllActiveStates() const {
    static const std::map<uint16_t, uint8_t> emptyMap;
    if (activeStates && activeStatesReady) {
        return *activeStates;
    }
    return emptyMap;
}

void Player::clearActionHistory() {
    recentActions.clear();
    activeStates->clear();
}
...
bool Player::isStealth() const {
    return stealth;
}

float Player::getDamageTakenModifier() const {
    return damageTakenModifier;
}

void Player::setDamageTakenModifier(float modifier) {
    damageTakenModifier = std::max(0.0f, modifier);
}

double Player::getCombatModifier(const std::string& key) const {
    auto it = m_combatModifiers.find(key);
    return it != m_combatModifiers.end() ? it->second : 0.0;
}

void Player::setCombatModifier(const std::string& key, double value) {
    m_combatModifiers[key] = value;
}

void Player::unlearnHabilidade(uint16_t habilidadeId) {
    std::ostringstream query;
    query << "DELETE FROM player_habilidades WHERE player_id = " << getGUID() << " AND habilidade_id = " << habilidadeId;
    g_database().executeQuery(query.str());
}

int32_t Player::getVocationBaseMaxHealth() const {
    auto voc = g_vocations().getVocation(0);
    if (!voc) return 0;
    // gainhp="20" → 20 por nível. Nível 1 = 150 → offset = 130
    return voc->getHPGain() * level + 130;
}

int32_t Player::getVocationBaseMaxMana() const {
    auto voc = g_vocations().getVocation(0);
    if (!voc) return 0;
    // gainmana="25" → 25 por nível. Nível 1 = 50 → offset = 25
    return voc->getManaGain() * level + 25;
}

int32_t Player::getNaturalSpeed() const {
    auto voc = g_vocations().getVocation(0);
    if (!voc) return 110 + (level - 1);
    return voc->getBaseSpeed() + (level - 1);
}

void Player::setMovementSpeed(int32_t speed) {
    setBaseSpeed(static_cast<uint16_t>(speed));
    g_game().changeSpeed(static_self_cast<Player>(), 0);
}

void Player::openRemoteContainer(std::shared_ptr<Container> container) {
    if (!container) return;

    // Fecha qualquer janela já aberta com este mesmo container (evita duplicados)
    for (uint8_t i = 0; i < 0x10; ++i) {
        if (auto existing = getContainerByID(i)) {
			if (existing == container) {
				// Já está aberto – reenvia a janela e garante que está na lista de remotos
				addRemoteContainer(container);   // <-- NOVO
				if (client) {
					client->sendContainer(i, container, container->hasParent(), 0);
				}
				return;
			}
        }
    ...
    // Encontra slot livre
    uint8_t cid = 0;
    while (cid < 0x10 && getContainerByID(cid)) {
        ++cid;
    }
    if (cid == 0x10) {
        closeContainer(0);
    ...
    // Regista e envia
    addContainer(cid, container);
	addRemoteContainer(container);
	m_remoteContainerCID = static_cast<int32_t>(cid);
    if (client) {
        client->sendContainer(cid, container, container->hasParent(), 0);
    }
}
...
void Player::storeFloorChange(const Position& from, const Position& to) {
    // Evita duplicados consecutivos
    if (!m_floorChangeQueue.empty()) {
        auto& last = m_floorChangeQueue.back();
        if (last.first == from && last.second == to) {
            return;
        }
    }
    m_floorChangeQueue.push_back({from, to});  // push_back para deque
}

std::pair<Position, Position> Player::peekFirstFloorChangeForZ(uint8_t z) const {
    for (const auto& trans : m_floorChangeQueue) {
        if (trans.first.z == z) {
            return trans;
        }
    }
...
bool Player::hasPendingFloorChange() const {
    return !m_floorChangeQueue.empty();
}

std::pair<Position, Position> Player::getNextFloorChange() {
    if (m_floorChangeQueue.empty()) return {};
    auto trans = m_floorChangeQueue.front();
    m_floorChangeQueue.pop_front();
    return trans;
}

void Player::clearNextFloorChange() {
    if (!m_floorChangeQueue.empty()) m_floorChangeQueue.pop_front();
}
...
void Player::onFloorChange(const Position& fromPos, const Position& toPos) {
    ...
    // Evita duplicar a mesma transição consecutiva
    if (!m_stairTransitions.empty()) {
        const auto& last = m_stairTransitions.back();
        if (last.origin == trans.origin && last.destination == trans.destination) {
            return; // já foi registada
        }
    }
    m_stairTransitions.push_back(trans);
}
...
void Player::addSurfaceWaypoint(const Position& pos) {
    uint64_t now = getCurrentTimeMs();
    while (!m_surfaceWaypoints.empty() && now - m_surfaceWaypoints.front().timestamp > SURFACE_WAYPOINT_EXPIRY_MS) {
        m_surfaceWaypoints.pop_front();
    }

    if (!m_surfaceWaypoints.empty()) {
        const Position& last = m_surfaceWaypoints.back().pos;
        if (std::abs(pos.x - last.x) + std::abs(pos.y - last.y) < 5) {
            return;
        }
    }

    m_surfaceWaypoints.push_back({pos, now});
}

void Player::touchSurfaceWaypoint(size_t index) {
    if (index < m_surfaceWaypoints.size()) {
        m_surfaceWaypoints[index].timestamp = getCurrentTimeMs();
    }
    // Remove imediatamente waypoints expirados da frente (manutenção)
    uint64_t now = getCurrentTimeMs();
    while (!m_surfaceWaypoints.empty() && now - m_surfaceWaypoints.front().timestamp > SURFACE_WAYPOINT_EXPIRY_MS) {
        m_surfaceWaypoints.pop_front();
    }
