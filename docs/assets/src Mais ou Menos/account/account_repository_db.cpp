...
bool AccountRepositoryDB::save(const std::unique_ptr<AccountInfo> &accInfo) {
	...
	if (!successful) {
		g_logger().error("Falha ao salvar conta:[{}]", accInfo->id);
	}
...
bool AccountRepositoryDB::getCharacterByAccountIdAndName(const uint32_t &id, const std::string &name) {
    g_logger().debug("[DEBUG] getCharacterByAccountIdAndName chamado com nome: {}", name);
	std::string utf8Name = latin1ToUtf8(name);
	g_logger().debug("[DEBUG] getCharacterByAccountIdAndName utf8Name: {}", utf8Name);
    auto result = g_database().storeQuery(fmt::format("SELECT `id` FROM `players` WHERE `account_id` = {} AND `name` = {}", id, g_database().escapeString(utf8Name)));
    if (!result) {
        g_logger().error("Failed to get character: [{}] from account: [{}]!", utf8Name, id);
        return false;
    }
...
bool AccountRepositoryDB::getPassword(const uint32_t &id, std::string &password) {
	...
	if (!result) {
		g_logger().error("Falha ao obter conta:[{}] senha!", id);
		return false;
	}
...
bool AccountRepositoryDB::getCoins(const uint32_t &id, CoinType coinType, uint32_t &coins) {
	...
	if (it == coinTypeToColumn.end()) {
		g_logger().error("[{}] tipo de moeda inv\341lido:[{}]", __FUNCTION__, coinType);
		return false;
	}
...
bool AccountRepositoryDB::setCoins(const uint32_t &id, CoinType coinType, const uint32_t &amount) {
	...
	if (!successful) {
		g_logger().error("Erro ao definir moedas da conta[{}] para [{}]", id, amount);
	}
...
bool AccountRepositoryDB::registerCoinsTransaction(
	...
	if (!successful) {
		g_logger().error(
			"Erro ao registrar transa\347\343o de moeda! account_id:[{}], tipo:[{}], coin_type:[{}], moedas:[{}], descri\347\343o:[{}]",
			id,
			type,
			coinType,
			coins,
...
bool AccountRepositoryDB::loadAccountPlayers(std::unique_ptr<AccountInfo> &acc) const {
	...
	if (!result) {
		g_logger().error("Falha ao carregar jogadores da conta[{}]!", acc->id);
		return false;
	}
