...
AccountErrors_t Account::removeCoins(CoinType type, const uint32_t &amount, const std::string &detail) {
	...
	if (coins < amount) {
		g_logger().info("A conta n\343o tem moedas suficientes! atual[{}], remover:[{}]", coins, amount);
		return RemoveCoins;
	}
...
void Account::registerCoinTransaction(CoinTransactionType transactionType, CoinType type, const uint32_t &amount, const std::string &detail) {
	...
	if (!g_accountRepository().registerCoinsTransaction(m_account->id, transactionType, amount, type, detail)) {
		g_logger().error(
			"Falha ao registrar transa\347\343o: 'conta:[{}], transa\347\343o "
			"tipo:[{}], moedas:[{}], tipo de moeda:[{}], descri\347\343o:[{}]",
...
std::string Account::getPassword() {
	...
	if (!g_accountRepository().getPassword(m_account->id, password)) {
		password.clear();
		g_logger().error("Falha ao obter a senha da conta[{}]!", m_account->id);
	}
...
bool Account::authenticateSession() {
	if (m_account->sessionExpires < getTimeNow()) {
		g_logger().error("A sess\343o expirou para a conta[{}] expirou em [{}] hora atual [{}]!", m_account->id, m_account->sessionExpires, getTimeNow());
		return false;
	}
	return true;
}
...
bool Account::authenticatePassword(const std::string &password) {
	...
	g_logger().error("A senha '{}' n\343o corresponde a nenhuma conta", getPassword());
	return false;
}
