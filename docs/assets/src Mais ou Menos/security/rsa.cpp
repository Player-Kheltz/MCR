...
RSAManager::RSAManager(Logger &logger) :
	...
	if (!n || !d || !mont_ctx) {
		throw std::runtime_error("Falha ao alocar estruturas OpenSSL BIGNUM/Context (BN_new/BN_MONT_CTX_new retornou nulo).");
	}
}
...
void RSAManager::start(const std::string &filename) {
	...
	const char* p = "14299623962416399520070177382898895550795403345466153217470516082934737582776 038882967213386204600674145392845853859217990626450972452084065728686565928113";
	const char* q = "76309791959704047218912018477920021255354012927791239372074475745966927885136 47179235335529307251350570728407373705564708871762033017096809910315212884101";

	try {
		if (!loadPEM(filename)) {
			logger.error("Arquivo {} n\343o encontrado ou v\341lido... Configurando chave rsa padr\343o \n", filename);
			setKey(p, q);
		}
	} catch (const std::exception &e) {
		logger.error("Loading RSA Key from {} failed with error: {}\n", filename, e.what());
		logger.error("Mudando para uma chave padr\343o...");
		setKey(p, q);
	}
}
...
void RSAManager::setKey(const char* pString, const char* qString, int base /* = 10*/) {
	...
	if (!ctx) {
		throw std::runtime_error("Falha ao alocar BN_CTX em setKey");
	}
	...
	try {
		// Parse p and q
		if (base == 10) {
			check(BN_dec2bn(&p_raw, pString), "Falha ao analisar p (decimal)");
			check(BN_dec2bn(&q_raw, qString), "Falha ao analisar q (decimal)");
		} else {
			check(BN_hex2bn(&p_raw, pString), "Falha ao analisar p (hex)");
			check(BN_hex2bn(&q_raw, qString), "Falha ao analisar q (hex)");
		}
	} catch (...) {
	...
	// e = 65537
	check(BN_set_word(e.get(), 65537), "BN_set_word falhou");

	// n = p * q
	check(BN_mul(n.get(), p.get(), q.get(), ctx.get()), "BN_mul (n=p*q) falhou");

	// phi = (p - 1)(q - 1)
	check(BN_sub_word(p.get(), 1), "BN_sub_word (p-1) falhou");
	check(BN_sub_word(q.get(), 1), "BN_sub_word (q-1) falhou");
	check(BN_mul(phi.get(), p.get(), q.get(), ctx.get()), "BN_mul (phi) falhou");

	// d = e^-1 mod phi
	if (!BN_mod_inverse(d.get(), e.get(), phi.get(), ctx.get())) {
		throw std::runtime_error(" BN_mod_inverse falhou ");
	}

	// Pre-compute Montgomery context for n
	if (!BN_MONT_CTX_set(mont_ctx.get(), n.get(), ctx.get())) {
		throw std::runtime_error("BN_MONT_CTX_set falhou");
	}
}
