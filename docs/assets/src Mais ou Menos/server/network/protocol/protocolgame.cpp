...
namespace {
	...
	void sendBosstiarySlotsBytes(NetworkMessage &msg, uint8_t bossRace, uint32_t bossKillCount, uint16_t bonusBossSlotOne, uint8_t killBonus, uint8_t isSlotOneInactive, uint32_t removePrice) {
		...
		msg.addByte(isSlotOneInactive); // Inactive? (Only true if equal to Boss Imupulsionado)
	}
	...
	void calculateAbsorbValues(const std::shared_ptr<Player> &player, NetworkMessage &msg, uint8_t &combats, bool fromPlayerSkills = false) {
			...
			if (imbuementSlots > 0) {
				for (uint8_t slotId = 0; slotId < imbuementSlots; ++slotId) {
					...
					for (uint16_t combat = 0; combat < COMBAT_COUNT; ++combat) {
						...
						g_logger().debug("[redu\347\343o de dano da ciclop\351dia] item imbu\355do {}, reduzido {} por cento, para o elemento {}", item->getName(), imbuementAbsorbPercent, combatTypeToName(indexToCombatType(combat)));
		...
		for (size_t i = 0; i < COMBAT_COUNT; ++i) {
			...
			if (damageModifiers[i] != 10000) {
				...
				g_logger().debug("[{}] CombatType: {}, Modificador de dano: {}, Modificador de cliente resultante: {}", __FUNCTION__, i, damageModifiers[i], clientModifier);
				if (!fromPlayerSkills) {
					msg.addByte(0x04);
				}
	...
	void sendContainerCategory(NetworkMessage &msg, const std::vector<T> &categories = {}, uint8_t categoryType = 0) {
		...
		g_logger().debug("Enviando tipo de categoria '{}', tamanho total das categorias '{}'", categoryType, categories.size());
		...
		for (auto value : categories) {
			...
			g_logger().debug("Enviando n\372mero da categoria '{}', nome da categoria '{}'", static_cast<uint8_t>(value), magic_enum::enum_name(value).data());
	...
	void addCyclopediaSkills(std::shared_ptr<Player> &player, NetworkMessage &msg, skills_t skill) {
		...
		if (skill == SKILL_LIFE_LEECH_AMOUNT) {
			...
			skillWheel += playerWheel.getMajorStatConditional("Maestria de Combate", WheelMajor_t::CRITICAL_DMG_2);
			skillWheel += playerWheel.getMajorStatConditional("Maestria Bal\355stica", WheelMajor_t::CRITICAL_DMG);
...
void ProtocolGame::login(const std::string &name, uint32_t accountId, OperatingSystem_t operatingSystem) {
	...
	if (!foundPlayer) {
		...
		if (!IOLoginData::loadPlayerById(player, player->getGUID(), false)) {
			disconnectClient("Não foi possível carregar seu personagem, entre em contato com um administrador.");
			return;
		}

		// ?? Garanta que o nome esteja em Latin-1, independentemente de como foi carregado
		player->setName(toLatin1(player->getName()));

		g_game().refreshPlayerNameMapping(player);
		...
		if (!g_game().placeCreature(player, player->getLoginPosition()) && !g_game().placeCreature(player, player->getTemplePosition(), false, true)) {
			disconnectClient("A posi\347\343o do templo est\341 errada. Por favor, entre em contato com o administrador.");
			g_logger().warn("A posi\347\343o do templo do jogador {} est\341 errada", player->getName());
			return;
		}
		...
		if (eventConnect != 0 || !g_configManager().getBoolean(REPLACE_KICK_ON_LOGIN)) {
			...
			disconnectClient("Voc\352 j\341 est\341 logado.");
			return;
		}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage &msg) {
	...
	g_logger().trace("Vers\343o do protocolo: {}", version);
	...
	if (!oldProtocol) {
		...
		g_logger().trace("Vers\343o do cliente: {}", clientVersionString);
		if (version >= 1334) {
			auto assetHashIdentifier = msg.getString(); // Assets hash identifier
			g_logger().trace("Identificador de hash de ativo do cliente: {}", assetHashIdentifier);
		}
	}

	if (version < 1334) {
		auto datRevision = msg.get<uint16_t>(); // Dat revision
		g_logger().trace("Revis\343o de dados: {}", datRevision);
	}

	auto gamePreviewState = msg.getByte(); // U8 game preview state
	g_logger().trace("Estado de visualiza\347\343o do jogo: {}", gamePreviewState);
	...
	g_logger().trace("\311 o Mestre do Jogo: {}", isGameMaster);
	...
	if (authType != "session") {
		...
		if (pos == std::string::npos) {
			ss << "Voc\352 deve inserir seu " << (oldProtocol ? "username" : "email") << ".";
			disconnectClient(ss.str());
			return;
		}
	...
	if (foundPlayer && foundPlayer->client) {
		...
		auto message = fmt::format("Voc\352 j\341 est\341 conectado atrav\351s de outro cliente. usar apenas um cliente por vez!");
	...
	if (!oldProtocol && clientVersion != CLIENT_VERSION) {
		ss.str(std::string());
		ss << "Somente clientes com protocolo " << CLIENT_VERSION_UPPER << "." << CLIENT_VERSION_LOWER;
	...
	if (g_game().getGameState() == GAME_STATE_STARTUP) {
		disconnectClient("Gameworld est\341 come\347ando. Por favor, aguarde.");
		return;
	}

	if (g_game().getGameState() == GAME_STATE_MAINTAIN) {
		disconnectClient("Gameworld est\341 em manuten\347\343o. Reconecte-se daqui a pouco.");
		return;
	}
	...
	if (IOBan::isIpBanned(getIP(), banInfo)) {
		...
		ss.str(std::string());
		ss << "Seu IP foi banido at\351 " << formatDateShort(banInfo.expiresAt) << " by " << banInfo.bannedBy << ".\n\nMotivo especificado:\n"
		   << banInfo.reason;
		disconnectClient(ss.str());
		return;
	}
	...
	if (!IOLoginData::gameWorldAuthentication(accountDescriptor, password, characterName, accountId, oldProtocol, getIP())) {
		ss.str(std::string());
		if (authType == "session") {
			ss << "Sua sess\343o expirou. Fa\347a entrar novamente.";
		} else { // authType == "password"
			ss << "Your " << (oldProtocol ? "username" : "email") << " ou a senha n\343o est\341 correta.";
		}
...
void ProtocolGame::parsePacket(NetworkMessage &msg) {
	...
	if (recvbyte != 0x1D && recvbyte != 0x1E) {
		g_logger().debug("BYTE RECEBIDO: 0x{:02X}", recvbyte);
	}
...
void ProtocolGame::parsePacketFromDispatcher(NetworkMessage &msg, uint8_t recvbyte) {
	...
	switch (recvbyte) {
		...
		default:
			...
			g_logger().debug("O jogador '{}' enviou um cabe\347alho de pacote desconhecido: hex[{}], decimal[{}]", player->getName(), asUpperCaseString(hexString), recvbyte);
			break;
	}
}
...
void ProtocolGame::parseChannelInvite(NetworkMessage &msg) {
	const std::string name = toLatin1(msg.getString());
...
void ProtocolGame::parseOpenPrivateChannel(NetworkMessage &msg) {
    std::string receiver = toLatin1(msg.getString());
	...
	if (msg.getBufferPosition() == startBufferPosition) {
		...
		if (outfitType == 0) {
			...
			if (!oldProtocol) {
				...
				g_logger().debug("Bool est\341 montado: {}", setMount);
			}
...
void ProtocolGame::parseQuickLoot(NetworkMessage &msg) {
	...
	g_logger().debug("[{}] variante {}, pos {}, itemId {}, stackPos {}", __FUNCTION__, variant, pos.toString(), itemId, stackpos);
...
void ProtocolGame::parseLootContainer(NetworkMessage &msg) {
	...
	if (action == 0) {
		...
		auto itemId = msg.get<uint16_t>();
		uint8_t stackpos = msg.getByte();
		g_logger().debug("[{}] a\347\343o {}, categoria {}, pos {}, itemId {}, stackPos {}", __FUNCTION__, action, static_cast<uint8_t>(category), pos.toString(), itemId, stackpos);
	...
	g_logger().debug("[{}] tipo de a\347\343o {}", __FUNCTION__, action);
}
...
void ProtocolGame::parseSay(NetworkMessage &msg) {
	...
	switch (type) {
		...
		case TALKTYPE_PRIVATE_RED_TO:
			receiver = toLatin1(msg.getString());
			channelId = 0;
			break;
...
void ProtocolGame::sendHighscores(const std::vector<HighscoreCharacter> &characters, uint8_t categoryId, uint32_t vocationId, uint16_t page, uint16_t pages, uint32_t updateTimer) {
	...
	for (const HighscoreCharacter &character : characters) {
		...
		msg.addString(toLatin1(character.name)); // Character Name
		msg.addString(toLatin1(character.loyaltyTitle)); // Character Loyalty Title
		msg.addByte(character.vocation); // Vocation Id
		msg.addString(toLatin1(serverName)); // World
...
void ProtocolGame::parseRuleViolationReport(NetworkMessage &msg) {
	...
	const std::string &targetName = toLatin1(msg.getString());
...
void ProtocolGame::parseBestiarysendMonsterData(NetworkMessage &msg) {
	...
	if (!mtype) {
		g_logger().warn("[ProtocolGame::parseBestiarysendMonsterData] - "
		                "MonsterType n\343o foi encontrado");
		return;
	}
...
void ProtocolGame::parseCyclopediaMonsterTracker(NetworkMessage &msg) {
	...
	if (it != bestiaryMonsters.end()) {
		...
		if (!mtype) {
			g_logger().error("[{}] jogador {} tem chefe errado com ra\347a {}", __FUNCTION__, player->getName(), monsterRaceId);
			return;
		}
...
void ProtocolGame::sendTeamFinderList() {
	...
	for (const auto &it : teamFinder) {
		...
		msg.addString(toLatin1(leader->getName()));
...
void ProtocolGame::sendLeaderTeamFinder(bool reset) {
	...
	for (const auto &[memberGUID, memberStatus] : teamAssemble->membersMap) {
		...
		msg.addString(toLatin1(member->getName()));
...
void ProtocolGame::parseLeaderFinderWindow(NetworkMessage &msg) {
	...
	switch (action) {
		...
		case 2: {
			...
			switch (memberStatus) {
				case 2: {
					member->sendTextMessage(MESSAGE_STATUS, "Voc\352 est\341 convidado para uma nova equipe.");
					break;
				}
				case 3: {
					member->sendTextMessage(MESSAGE_STATUS, "Sua solicita\347\343o de busca de equipe foi aceita.");
					break;
				}
				case 4: {
					member->sendTextMessage(MESSAGE_STATUS, "Sua solicita\347\343o de localiza\347\343o de equipe foi negada.");
					break;
				}
...
void ProtocolGame::parseMemberFinderWindow(NetworkMessage &msg) {
	...
	if (action == 0) {
		...
		if (action == 1) {
			leader->sendTextMessage(MESSAGE_STATUS, "H\341 uma nova solicita\347\343o para ingressar na sua equipe.");
	...
	if (search == 1) {
		...
		if (race.empty()) {
			g_logger().warn("[ProtocolGame::parseBestiarysendCreature] - "
			                "Corrida n\343o encontrada: {}, pesquisa: {}",
			                raceName, search);
			return;
		}
		text = raceName;
...
void ProtocolGame::parseOfferDescription(NetworkMessage &msg) {
	...
	g_logger().debug("[{}] ID da oferta: {}", __FUNCTION__, offerId);
}
...
void ProtocolGame::sendCyclopediaCharacterBaseInformation() {
	...
	msg.addByte(0x00);
	msg.addString(toLatin1(player->getName()));;
	msg.addString(toLatin1(player->getVocation()->getVocName()));
	msg.add<uint16_t>(player->getLevel());
	AddOutfit(msg, player->getDefaultOutfit(), false);

	msg.addByte(0x01); // Store summary & Character titles
	msg.addString(toLatin1(player->title().getCurrentTitleName())); // character title
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendCyclopediaCharacterStoreSummary() {
	...
	auto remaining = player->kv()->get("aumento de XP de recompensa di\341ria");
	...
	for (const auto &[skillId, skillName] : g_game().getHirelingSkills()) {
		if (player->kv()->scoped("habilidades de mercen\341rio")->get(skillName)) {
			[[maybe_unused]] auto &skill_ref = m_hSkills.emplace_back(skillId);
			g_logger().debug("id da habilidade: {}, nome: {}", skillId, skillName);
		}
	}
	...
	for (const auto &it : g_game().getHirelingOutfits()) {
	    if (player->kv()->scoped("roupas de mercen\341rio")->get(it.second)) {
	        m_hOutfits.emplace_back(it.first);
	        g_logger().debug("id da roupa: {}, nome: {}", it.first, it.second);
	    }
	}
...
void ProtocolGame::sendCyclopediaCharacterInspection() {
	...
	if (player->title().getCurrentTitle() != 0) {
		...
		msg.addString("T\355tulo do personagem");
	...
	if (!player->getLoyaltyTitle().empty()) {
		playerDescriptionSize++;
		msg.addString("T\355tulo de fidelidade");
	...
	for (uint8_t slotId = PreySlot_First; slotId <= PreySlot_Last; slotId++) {
		if (const auto &slot = player->getPreySlotById(static_cast<PreySlot_t>(slotId));
			...
			if (auto mtype = g_monsters().getMonsterTypeByRaceId(slot->selectedRaceId)) {
				...
				desc.append("Criatura desconhecida");
			}

			if (slot->bonus == PreyBonus_Damage) {
				desc.append(" (Dano Melhorado +");
			} else if (slot->bonus == PreyBonus_Defense) {
				desc.append(" (Defesa Melhorada +");
			} else if (slot->bonus == PreyBonus_Experience) {
				desc.append(" (Experi\352ncia aprimorada +");
			} else if (slot->bonus == PreyBonus_Loot) {
				desc.append(" (Saque melhorado +");
			}
			desc.append(fmt::format("{}%, restante", slot->bonusPercentage));
...
void ProtocolGame::sendCyclopediaCharacterDefenceStats() {
	...
	const uint16_t defenseWheel = player->wheel().getMajorStatConditional("Maestria de Combate", WheelMajor_t::DEFENSE);
...
void ProtocolGame::sendCyclopediaCharacterMiscStats() {
	...
	for (const auto &[concoctionId, duration] : activeConcoctions) {
		if (duration == 0) {
			g_logger().error(
				"sendCyclopediaCharacterMiscStats: O jogador {} possui mistura com itemId {} e timeLeft 0, isso n\343o deveria acontecer.",
...
void ProtocolGame::sendTextMessage(const TextMessage &message) {
	if (message.type == MESSAGE_NONE) {
		...
		player->sendTextMessage(MESSAGE_ADMINISTRATOR, "Ocorreu um problema ao solicitar sua mensagem, entre em contato com o administrador");
		return;
	}
...
void ProtocolGame::sendChannel(uint16_t channelId, const std::string &channelName, const UsersMap* channelUsers, const InvitedMap* invitedUsers) {
	...
	if (channelUsers) {
		...
		for (const auto &it : *channelUsers) {
			msg.addString(toLatin1(it.second->getName()));
		}
	} else {
		msg.add<uint16_t>(0x00);
	}
...
void ProtocolGame::sendContainer(uint8_t cid, const std::shared_ptr<Container> &container, bool hasParent, uint16_t firstIndex) {
	...
	if (container->getID() == ITEM_BROWSEFIELD) {
		...
		msg.addString("Navegar pelo campo");
	} else {
		...
		if (category.has_value()) {
			...
			for (const auto &tempCategory : categories) {
				if (tempCategory == category.value()) {
					toSendCategory = true;
					g_logger().debug("categoria encontrada {}", toSendCategory);
				}
			}
...
void ProtocolGame::sendShop(const std::shared_ptr<Npc> &npc) {
	...
	NetworkMessage msg;
	msg.addByte(0x7A);
	msg.addString(toLatin1(npc->getName()));
	...
	auto talkactionHidden = player->kv()->get("npc-shop-item-de-venda-oculto");
...
void ProtocolGame::sendForgeResult(ForgeAction_t actionType, uint16_t leftItemId, uint8_t leftTier, uint16_t rightItemId, uint8_t rightTier, bool success, uint8_t bonus, uint8_t coreCount, bool convergence) {
	...
	writeToOutputBuffer(msg);
	g_logger().debug("Enviar fus\343o de forja: tipo {}, item esquerdo {}, n\355vel esquerdo {}, item direito {}, rightTier {}, sucesso {}, b\364nus {}, coreCount {}, converg\352ncia {}", fmt::underlying(actionType), leftItemId, leftTier, rightItemId, rightTier, success, bonus, coreCount, convergence);
	sendOpenForge();
}
...
void ProtocolGame::sendMarketDetail(uint16_t itemId, uint8_t tier) {
	...
	if (it.isRanged()) {
		...
		if (it.hitChance != 0) {
			...
			ss << "chance de acertar +" << static_cast<int16_t>(it.hitChance) << "%";
			separator = true;
		}
		...
		if (it.attack != 0 && !attackDescription.empty()) {
			attackDescription = fmt::format("{} f\355sico + {}", it.attack, attackDescription);
	...
	if (it.abilities) {
		...
		if (it.abilities->stats[STAT_MAGICPOINTS] != 0) {
			...
			ss << fmt::format(" n\355vel m\341gico {:+}", it.abilities->stats[STAT_MAGICPOINTS]);
		}
		...
		for (uint8_t i = 1; i <= 11; i++) {
			if (it.abilities->specializedMagicLevel[i]) {
				...
				ss << std::showpos << combatName << std::noshowpos << "n\355vel m\341gico +" << it.abilities->specializedMagicLevel[i];
			}
		}
		...
		if (it.abilities->speed != 0) {
			...
			ss << fmt::format("velocidade {:+}", (it.abilities->speed >> 1));
		}
	...
	if (!oldProtocol) {
		...
		if (it.abilities) {
			...
			if (it.abilities->perfectShotDamage > 0) {
				string.clear();
				string << std::showpos << it.abilities->perfectShotDamage << std::noshowpos << " ao alcance " << unsigned(it.abilities->perfectShotRange);
				msg.addString(string.str());
			} else {
				msg.add<uint16_t>(0x00);
			}
		...
		if (it.upgradeClassification > 0 && tier > 0) {
			...
			if (it.isWeapon()) {
				...
				ss << fmt::format("{} ({:.2f}% Ataque)", static_cast<uint16_t>(tier), chance);
			} else if (it.isHelmet()) {
				...
				ss << fmt::format("{} ({:.2f}% Transcend\352ncia)", static_cast<uint16_t>(tier), chance);
			} else if (it.isBoots()) {
				chance = (0.4 * tier * tier) + (1.7 * tier) + 0.4;
				ss << fmt::format("{} ({:.2f}% Amplifica\347\343o)", static_cast<uint16_t>(tier), chance);
			}
			msg.addString(ss.str());
...
void ProtocolGame::sendTradeItemRequest(const std::string &traderName, const std::shared_ptr<Item> &item, bool ack) {
	...
	msg.addString(toLatin1(traderName));
...
void ProtocolGame::sendCreatureSay(const std::shared_ptr<Creature> &creature, SpeakClasses type, const std::string &text, const Position* pos /* = nullptr*/) {
	...
	msg.addString(toLatin1(creature->getCustomName()));
...
void ProtocolGame::sendPrivateMessage(const std::shared_ptr<Player> &speaker, SpeakClasses type, const std::string &text) {
	...
	msg.add<uint32_t>(++statementId);
	if (speaker) {
		msg.addString(toLatin1(speaker->getName()));
		if (!oldProtocol && statementId != 0) {
			msg.addByte(0x00); // Show (Traded)
		}
		msg.add<uint16_t>(speaker->getLevel());
...
void ProtocolGame::sendRestingStatus(uint8_t protection) {
	...
	if (dailyStreak < 2) {
		msg.addString("\301rea de descanso (sem b\364nus ativo)");
	} else {
		std::ostringstream ss;
		ss << "B\364nus de \341rea de descanso ativa: ";
		if (dailyStreak < DAILY_REWARD_DOUBLE_HP_REGENERATION) {
			ss << "\nRegenera\347\343o de Pontos de Vida";
		} else {
			ss << "\nRegenera\347\343o de Pontos de Vida Duplos";
		}
		if (dailyStreak >= DAILY_REWARD_MP_REGENERATION) {
			if (dailyStreak < DAILY_REWARD_DOUBLE_MP_REGENERATION) {
				ss << ",\nRegenera\347\343o de Pontos de mana";
			} else {
				ss << ",\nRegenera\347\343o de Pontos de mana Duplos";
			}
		}
		if (dailyStreak >= DAILY_REWARD_STAMINA_REGENERATION) {
			ss << ",\nRegenera\347\343o de Pontos de Vigor";
		}
		if (dailyStreak >= DAILY_REWARD_SOUL_REGENERATION) {
			ss << ",\nRegenera\347\343o de Pontos de Alma";
		}
		ss << ".";
		msg.addString(ss.str());
	}
...
void ProtocolGame::sendInventoryIds() {
	...
	for (auto [key, amount] : items) {
		...
		if (amount >= 0x40000000) {
			g_logger().warn("[{}] jogador: {}, falha ao escrever a contagem para o item: {} n\355vel: {}, quantidade: {}, contagem total: {}, pulando item.", __FUNCTION__, player->getName(), itemId, tier, amount, totalItemsCount);
			continue;
		}
...
void ProtocolGame::sendOutfitWindow() {
	...
	if (player->isAccessPlayer() && g_configManager().getBoolean(ENABLE_SUPPORT_OUTFIT)) {
		...
		msg.addString("Suporte ao Cliente");
		msg.addByte(0);
		msg.addByte(0x00);
		++outfitSize;

		msg.add<uint16_t>(302);
		msg.addString("Gerente de comunidade");
		msg.addByte(0);
		msg.addByte(0x00);
		++outfitSize;
	}
...
void ProtocolGame::updatePartyTrackerAnalyzer(const std::shared_ptr<Party> &party, bool force) {
	...
	if (showNames) {
		...
		for (const std::shared_ptr<PartyAnalyzer> &analyzer : party->membersData) {
			msg.add<uint32_t>(analyzer->id);
			msg.addString(toLatin1(analyzer->name));
		}
	}
...
void ProtocolGame::parseStashWithdraw(NetworkMessage &msg) {
	...
	if (!player->isAccessPlayer() && !player->isStashMenuAvailable()) {
		player->sendCancelMessage("Voc\352 n\343o pode usar o stash agora.");
		return;
	}

	if (player->isUIExhausted(500)) {
		player->sendCancelMessage("Voc\352 precisa esperar para fazer isso novamente.");
		return;
	}
	...
	switch (action) {
		...
		default:
			g_logger().error("Chave de a\347\343o 'stash' desconhecida: {}", fmt::underlying(action));
			break;
	}
...
void ProtocolGame::parseSendBosstiary() {
	...
	for (const auto &[bossid, name] : mtype_map) {
		...
		if (bossRace < BosstiaryRarity_t::RARITY_BANE || bossRace > BosstiaryRarity_t::RARITY_NEMESIS) {
			g_logger().error("[{}] monstro {} tem ra\347a de chefe errada {}", __FUNCTION__, mType->name, fmt::underlying(bossRace));
			continue;
		}
...
void ProtocolGame::parseSendBosstiarySlots() {
	...
	if (boostedBossName.empty()) {
		g_logger().error("[{}] O nome do chefe aprimorado est\341 vazio", __FUNCTION__);
		return;
	}
	...
	if (!isValidBoostedBoss) {
		g_logger().error("[{}] O Boss Fortalecido '{}' tem uma ra\347a inv\341lida", __FUNCTION__, boostedBossName);
		return;
	}
	...
	if (!isValidBossSlotOne) {
		g_logger().error("[{}] chefe slot1 com ID de corrida '{}' tem uma corrida inv\341lida", __FUNCTION__, bossIdSlotOne);
		return;
	}
	...
	if (!isValidBossSlotTwo) {
		g_logger().error("[{}] chefe slot1 com ID de corrida '{}' tem uma corrida inv\341lida", __FUNCTION__, bossIdSlotTwo);
		return;
	}
	...
	if (bossesUnlockedSize != 0) {
		...
		for (const auto &bossId : bossesUnlockedList) {
			...
			if (!mType) {
				g_logger().error("[{}] monstro {} n\343o encontrado", __FUNCTION__, bossId);
				continue;
			}
