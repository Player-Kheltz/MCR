...
void Protocol::XTEA_transform(uint8_t* buffer, size_t messageLength, bool encrypt) const {
	...
	while (readPos < messageLength) {
		...
		if (std::memcpy(tempBuffer.data(), buffer + readPos, tempBuffer.size()) == nullptr) {
			g_logger().error("[{}] memcpy falhou ao preparar o bloco XTEA", __FUNCTION__);
			return;
		}
...
bool Protocol::XTEA_decrypt(NetworkMessage &msg) const {
	...
	if ((msgLength % 8) != 0) {
		g_logger().error("XTEA_decrypt falhou - tamanho de bloco inv\341lido: {}", msgLength);
	...
	if (innerLength + paddingSize > msgLength) {
		g_logger().error("XTEA_decrypt falhou - comprimento interno inv\341lido: {} + {} > {}", innerLength, paddingSize, msgLength);
		return false;
	}
...
Protocol::ZStream::ZStream() noexcept {
	...
	if (deflateInit2(stream.get(), compressionLevel, Z_DEFLATED, -15, 9, Z_DEFAULT_STRATEGY) != Z_OK) {
		stream.reset();
		g_logger().error("[Protocol::enableCompression()] - Zlib deflateInit2 error: {}", (stream->msg ? stream->msg : " erro desconhecido"));
	}
}
