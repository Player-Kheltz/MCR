...
// Nova inclusão para geração aleatória
#include <random>
#include <sstream>
#include <iomanip>

// Função auxiliar que gera uma chave de sessão aleatória hexadecimal
...
void ProtocolLogin::getCharacterList(const std::string &accountDescriptor, const std::string &password) const {
	...
	if (oldProtocol && !g_configManager().getBoolean(OLD_PROTOCOL)) {
		disconnectClient(fmt::format("Somente a vers\343o do protocolo {}.{} \351 permitida.", CLIENT_VERSION_UPPER, CLIENT_VERSION_LOWER));
		return;
	} else if (!oldProtocol) {
		disconnectClient(fmt::format("Somente a vers\343o do protocolo {}.{} ou desatualizada 11.00 \351 permitida.", CLIENT_VERSION_UPPER, CLIENT_VERSION_LOWER));
		return;
	}

	if (account.load() != AccountErrors_t::Ok || !account.authenticate(password)) {
		std::ostringstream ss;
		ss << (oldProtocol ? "Username" : "Email") << " ou a senha n\343o est\341 correta.";
	...
	// ============================================
	// Adiciona a chave de sessão (ALEATÓRIA e SEGURA)
	output->addByte(0x28);
	output->addString(generateSessionKey());   // <--- CORRIGIDO
	// ============================================

	// Add char list
	auto [players, result] = account.getAccountPlayers();
	if (AccountErrors_t::Ok != result) {
		g_logger().warn("A conta[{}] n\343o conseguiu carregar os jogadores!", account.getID());
	}
	...
	for (const auto &[name, deletion] : players) {
		output->addByte(0);
		output->addString(toLatin1(name));
	}
...
void ProtocolLogin::onRecvFirstMessage(NetworkMessage &msg) {
	...
	if (g_game().getGameState() == GAME_STATE_MAINTAIN) {
		disconnectClient("O Gameworld est\341 em manuten\347\343o.\nConecte-se novamente daqui a pouco.");
		return;
	}
	...
	if (password.empty()) {
		disconnectClient("Senha inv\341lida.");
		return;
	}
