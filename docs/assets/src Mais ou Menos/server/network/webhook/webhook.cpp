...
Webhook::Webhook(ThreadPool &threadPool) :
	...
	if (curl_global_init(CURL_GLOBAL_ALL) != 0) {
		g_logger().error("Falha ao iniciar o curl, nenhuma mensagem de webhook pode ser enviada");
		return;
	}
	...
	if (headers == nullptr) {
		g_logger().error("Falha ao iniciar curl, falha ao anexar cabe\347alhos de solicita\347\343o");
		return;
	}
...
int Webhook::sendRequest(const char* url, const char* payload, std::string* response_body) const {
	...
	if (!curl) {
		g_logger().error("Falha ao enviar mensagem de webhook; curl_easy_init falhou");
		return -1;
	}
	...
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "can\341rio (https://github.com/opentibiabr/canary)");

	const CURLcode res = curl_easy_perform(curl);

	if (res != CURLE_OK) {
		g_logger().error("Falha ao enviar mensagem de webhook com o erro: {}", curl_easy_strerror(res));
...
void Webhook::sendWebhook() {
	...
	if (response_code == 429 || response_code == 504) {
		g_logger().warn("O Webhook encontrou o c\363digo de erro {}, reenfileirando a tarefa.", response_code);
	...
	if (response_code >= 300) {
		g_logger().error(
			"Falha ao enviar mensagem de webhook, c\363digo de erro: {} corpo da resposta: {} corpo da solicita\347\343o: {}",
			response_code,
			response_body,
			task->payload
		);
	...
	g_logger().debug("Webhook enviado com sucesso para {}", task->url);
