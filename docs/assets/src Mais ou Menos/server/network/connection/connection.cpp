...
void Connection::handleTimeout(ConnectionWeak_ptr connectionWeak, const std::error_code &error) {
	...
	if (auto connection = connectionWeak.lock()) {
		if (!error) {
			g_logger().debug("Tempo limite de conex\343o, IP: {}", convertIPToString(connection->getIP()));
		} else {
			g_logger().debug("Tempo limite de conex\343o ou erro: {}, IP: {}", error.message(), convertIPToString(connection->getIP()));
		}
