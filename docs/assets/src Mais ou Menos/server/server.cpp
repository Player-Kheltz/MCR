...
ServiceManager::~ServiceManager() {
	try {
		...
		g_logger().error("{} - Erro de exce\347\343o de captura: {}", __FUNCTION__, exception.what());
	}
}
...
void ServiceManager::run() {
	if (running) {
		g_logger().error("O ServiceManager j\341 est\341 em execu\347\343o!", __FUNCTION__);
		return;
	}
