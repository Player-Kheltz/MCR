...
void Signals::sigbreakHandler() {
	// Dispatcher thread
	g_logger().info("SIGBREAK recebido, desligando o servidor do jogo...");
	g_game().setGameState(GAME_STATE_SHUTDOWN);
}

void Signals::sigtermHandler() {
	// Dispatcher thread
	g_logger().info("SIGTERM recebido, desligando o servidor do jogo...");
	g_game().setGameState(GAME_STATE_SHUTDOWN);
}

void Signals::sigusr1Handler() {
	// Dispatcher thread
	g_logger().info("SIGUSR1 recebido, salvando o estado do jogo...");
	g_globalEvents().save();
...
void Signals::sighupHandler() {
	// Dispatcher thread
	g_logger().info("SIGHUP recebido, recarregando arquivos de configura\347\343o...");

	g_configManager().reload();
	g_logger().info("Configura\347\343o recarregada");

	g_game().raids.reload();
	g_game().raids.startup();
	g_logger().info("Ataques recarregados");

	Item::items.reload();
	g_logger().info("Itens recarregados");

	g_game().mounts->reload();
	g_logger().info("Montarias recarregadas");

	g_events().loadFromXml();
	g_logger().info("Eventos recarregados");

	g_chat().load();
	g_logger().info("Canais de bate-papo recarregados");

	g_luaEnvironment().loadFile(g_configManager().getString(CORE_DIRECTORY) + "/core.lua", "core.lua");
	g_logger().info("core.lua recarregado");
...
void Signals::sigintHandler() {
	// Dispatcher thread
	g_logger().info("SIGINT recebido, desligando o servidor do jogo...");
	g_game().setGameState(GAME_STATE_SHUTDOWN);
}
