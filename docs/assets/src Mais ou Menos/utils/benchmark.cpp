...
void Benchmark::log(std::string_view message) noexcept {
	...
	if (last > 1000.0) {
		g_logger().debug("{} demorou {:.3f} segundos.", message, last / 1000.0);
	} else {
		g_logger().debug("{} demorou {:.3f} milissegundos.", message, last);
	}
}
...
void Benchmark::logInfo(std::string_view message) noexcept {
	...
	if (last > 1000.0) {
		g_logger().info("{} demorou {:.3f} segundos.", message, last / 1000.0);
	} else {
		g_logger().info("{} demorou {:.3f} milissegundos.", message, last);
	}
}
