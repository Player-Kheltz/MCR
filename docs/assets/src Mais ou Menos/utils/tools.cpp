...
void printXMLError(const std::string &where, const std::string &fileName, const pugi::xml_parse_result &result) {
	g_logger().error("[{}] Falha ao carregar {}: {}", where, fileName, result.description());
	...
	g_logger().error("Linha {}:", currentLine);
	g_logger().error("{}", line);
...
std::string formatDate(time_t time) {
	try {
		...
		g_logger().error("Falha ao formatar a data com c\363digo de erro {}", exception.what());
	}
	return {};
}
...
std::string formatDateShort(time_t time) {
	try {
		...
		g_logger().error("Falha ao formatar data abreviada com c\363digo de erro {}", exception.what());
	}
	return {};
}
...
std::string formatDateTime(int64_t ms) {
	try {
		...
		g_logger().error("Falha ao formatar data e hora com c\363digo de erro {}", exception.what());
	}
	return {};
}
...
Direction getDirection(const std::string &string) {
	...
	} else if (string == "southwest" || string == "sudoeste" || string == "south-west" || string == "sw" || string == "4") {
		direction = DIRECTION_SOUTHWEST;
	} else if (string == "southeast" || string == "sudeste" || string == "south-east" || string == "se" || string == "5") {
		direction = DIRECTION_SOUTHEAST;
	} else if (string == "northwest" || string == "noroeste" || string == "north-west" || string == "nw" || string == "6") {
		direction = DIRECTION_NORTHWEST;
	} else if (string == "northeast" || string == "nordeste" || string == "north-east" || string == "ne" || string == "7") {
...
size_t combatTypeToIndex(CombatType_t combatType, std::source_location location) {
	...
	if (enum_index_opt.has_value() && enum_index_opt.value() < COMBAT_COUNT) {
		...
		g_logger().error("[{}] O tipo de combate {} est\341 fora do alcance, chamado de linha '{}:{}' em '{}'", __FUNCTION__, fmt::underlying(combatType), location.line(), location.column(), location.function_name());
...
std::string combatTypeToName(CombatType_t combatType) {
	...
	if (!name.empty() && combatType < COMBAT_COUNT) {
		...
		g_logger().error("[{}] O tipo de combate {} est\341 fora do alcance", __FUNCTION__, fmt::underlying(combatType));
		// Uncomment for catch the function call with debug
...
ItemAttribute_t stringToItemAttribute(const std::string &str) {
	...
	g_logger().error("[{}] atributo tipo {} n\343o est\341 registrado", __FUNCTION__, str);
...
const char* getReturnMessage(ReturnValue value) {
	switch (value) {
		...
		case RETURNVALUE_NOTBOUGHTINSTORE:
			return "Voc\352 n\343o pode mover este item para a caixa de entrada da loja, pois ele n\343o foi comprado na loja.";

		case RETURNVALUE_ITEMCANNOTBEMOVEDPOUCH:
			return "Este item n\343o pode ser movido para l\341. Voc\352 s\363 pode colocar moedas de ouro, platina e cristal em sua bolsa de ouro.";

		case RETURNVALUE_ITEMCANNOTBEMOVEDTHERE:
			return "Este item n\343o pode ser movido para l\341.";

		case RETURNVALUE_REWARDCHESTISEMPTY:
			return "O ba\372 est\341 vazio n\343o momento. Voc\352 n\343o participou de nenhuma batalha nos \372ltimos sete dias ou j\341 reivindicou sua recompensa.";

		case RETURNVALUE_DESTINATIONOUTOFREACH:
			return "O destino est\341 fora de alcance.";

		case RETURNVALUE_NOTMOVABLE:
			return "Voc\352 n\343o pode mover este objeto.";

		case RETURNVALUE_DROPTWOHANDEDITEM:
			return "Solte o objeto de duas m\343os primeiro.";

		case RETURNVALUE_BOTHHANDSNEEDTOBEFREE:
			return "Ambas as m\343os precisam estar livres.";

		case RETURNVALUE_CANNOTBEDRESSED:
			return "Voc\352 n\343o pode vestir esse objeto ali.";

		case RETURNVALUE_PUTTHISOBJECTINYOURHAND:
			return "Coloque este objeto em sua m\343o.";

		case RETURNVALUE_PUTTHISOBJECTINBOTHHANDS:
			return "Coloque este objeto em ambas as m\343os.";

		case RETURNVALUE_CANONLYUSEONEWEAPON:
			return "Voc\352 s\363 pode usar uma arma.";

		case RETURNVALUE_TOOFARAWAY:
			return "Muito longe.";

		case RETURNVALUE_FIRSTGODOWNSTAIRS:
			return "Primeiro des\347a.";

		case RETURNVALUE_FIRSTGOUPSTAIRS:
			return "Primeiro suba as escadas.";

		case RETURNVALUE_NOTENOUGHCAPACITY:
			return "Este objeto \351 muito pesado para voc\352 carregar.";

		case RETURNVALUE_CONTAINERNOTENOUGHROOM:
			return "Voc\352 n\343o pode colocar mais objetos neste cont\352iner.";

		case RETURNVALUE_ONLYAMMOINQUIVER:
			return "Esta aljava cont\351m apenas flechas e virotes.\nVoc\352 n\343o pode colocar nenhum outro item nela.";

		case RETURNVALUE_CREATUREBLOCK:
		case RETURNVALUE_NEEDEXCHANGE:
		case RETURNVALUE_NOTENOUGHROOM:
			return "N\343o h\341 espa\347o suficiente.";

		case RETURNVALUE_CANNOTPICKUP:
			return "Voc\352 n\343o pode pegar este objeto.";

		case RETURNVALUE_CANNOTTHROW:
			return "Voc\352 n\343o pode jogar l\341.";

		case RETURNVALUE_THEREISNOWAY:
			return "N\343o h\341 como.";

		case RETURNVALUE_THISISIMPOSSIBLE:
			return "Isso \351 imposs\355vel.";

		case RETURNVALUE_PLAYERISPZLOCKED:
			return "Voc\352 n\343o pode entrar em uma zona de prote\347\343o ap\363s atacar outro jogador.";

		case RETURNVALUE_PLAYERISNOTINVITED:
			return "Voc\352 n\343o est\341 convidado.";

		case RETURNVALUE_CREATUREDOESNOTEXIST:
			return "A criatura n\343o existe.";

		case RETURNVALUE_DEPOTISFULL:
			return "Voc\352 n\343o pode colocar mais itens neste dep\363sito.";

		case RETURNVALUE_CONTAINERISFULL:
			return "Voc\352 n\343o pode colocar mais itens neste cont\352iner.";

		case RETURNVALUE_CANNOTUSETHISOBJECT:
			return "Voc\352 n\343o pode usar este objeto.";

		case RETURNVALUE_PLAYERWITHTHISNAMEISNOTONLINE:
			return "Um jogador com este nome n\343o est\341 online.";

		case RETURNVALUE_NOTREQUIREDLEVELTOUSERUNE:
			return "Voc\352 n\343o tem o n\355vel de magia necess\341rio para usar esta runa.";

		case RETURNVALUE_YOUAREALREADYTRADING:
			return "Voc\352 j\341 est\341 negociando. Termine esta negocia\347\343o primeiro.";

		case RETURNVALUE_THISPLAYERISALREADYTRADING:
			return "Este jogador j\341 est\341 negociando.";

		case RETURNVALUE_YOUMAYNOTLOGOUTDURINGAFIGHT:
			return "Voc\352 n\343o pode sair durante ou imediatamente ap\363s uma luta!";

		case RETURNVALUE_DIRECTPLAYERSHOOT:
			return "Voc\352 n\343o tem permiss\343o para atirar diretamente nos jogadores.";

		case RETURNVALUE_NOTENOUGHLEVEL:
			return "Voc\352 n\343o tem n\355vel suficiente.";

		case RETURNVALUE_NOTENOUGHMAGICLEVEL:
			return "Voc\352 n\343o tem n\355vel m\341gico suficiente.";

		case RETURNVALUE_NOTENOUGHMANA:
			return "Voc\352 n\343o tem mana suficiente.";

		case RETURNVALUE_NOTENOUGHSOUL:
			return "Voc\352 n\343o tem alma suficiente.";

		case RETURNVALUE_YOUAREEXHAUSTED:
			return "Voc\352 est\341 exausto.";

		case RETURNVALUE_CANONLYUSETHISRUNEONCREATURES:
			return "Voc\352 s\363 pode usar esta runa em criaturas.";

		case RETURNVALUE_PLAYERISNOTREACHABLE:
			return "O jogador n\343o est\341 acess\355vel.";

		case RETURNVALUE_CREATUREISNOTREACHABLE:
			return "A criatura n\343o est\341 acess\355vel.";

		case RETURNVALUE_ACTIONNOTPERMITTEDINPROTECTIONZONE:
			return "Esta a\347\343o n\343o \351 permitida numa zona de prote\347\343o.";

		case RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER:
			return "Voc\352 n\343o pode atacar este jogador.";

		case RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE:
			return "Voc\352 n\343o pode atacar esta criatura.";

		case RETURNVALUE_YOUMAYNOTATTACKAPERSONINPROTECTIONZONE:
			return "Voc\352 n\343o pode atacar uma pessoa em uma zona de prote\347\343o.";

		case RETURNVALUE_YOUMAYNOTATTACKAPERSONWHILEINPROTECTIONZONE:
			return "Voc\352 n\343o pode atacar uma pessoa enquanto estiver em uma zona de prote\347\343o.";

		case RETURNVALUE_YOUCANONLYUSEITONCREATURES:
			return "Voc\352 s\363 pode us\341-lo em criaturas.";

		case RETURNVALUE_TURNSECUREMODETOATTACKUNMARKEDPLAYERS:
			return "Desative o modo seguro se voc\352 realmente deseja atacar jogadores n\343o marcados.";

		case RETURNVALUE_YOUNEEDPREMIUMACCOUNT:
			return "Voc\352 precisa de uma conta premium.";

		case RETURNVALUE_YOUNEEDTOLEARNTHISSPELL:
			return "Voc\352 precisa aprender esse feiti\347o primeiro.";

		case RETURNVALUE_YOURVOCATIONCANNOTUSETHISSPELL:
			return "Sua voca\347\343o n\343o pode usar este feiti\347o.";

		case RETURNVALUE_YOUNEEDAWEAPONTOUSETHISSPELL:
			return "Voc\352 precisa equipar uma arma para usar este feiti\347o.";

		case RETURNVALUE_PLAYERISPZLOCKEDLEAVEPVPZONE:
			return "Voc\352 n\343o pode sair de uma zona pvp ap\363s atacar outro jogador.";

		case RETURNVALUE_PLAYERISPZLOCKEDENTERPVPZONE:
			return "Voc\352 n\343o pode entrar em uma zona pvp ap\363s atacar outro jogador.";

		case RETURNVALUE_ACTIONNOTPERMITTEDINANOPVPZONE:
			return "Esta a\347\343o n\343o \351 permitida em uma zona n\343o PVP.";

		case RETURNVALUE_YOUCANNOTLOGOUTHERE:
			return "Voc\352 n\343o pode sair aqui.";

		case RETURNVALUE_YOUNEEDAMAGICITEMTOCASTSPELL:
			return "Voc\352 precisa de um item m\341gico para lan\347ar este feiti\347o.";

		case RETURNVALUE_CANNOTCONJUREITEMHERE:
			return "Voc\352 n\343o pode conjurar itens aqui.";

		case RETURNVALUE_YOUNEEDTOSPLITYOURSPEARS:
			return "Voc\352 precisa dividir suas lan\347as primeiro.";

		case RETURNVALUE_NAMEISTOOAMBIGUOUS:
			return "O nome do jogador \351 amb\355guo.";

		case RETURNVALUE_CANONLYUSEONESHIELD:
			return "Voc\352 pode usar apenas um escudo.";

		case RETURNVALUE_NOPARTYMEMBERSINRANGE:
			return "Nenhum membro do grupo ao alcance.";

		case RETURNVALUE_YOUARENOTTHEOWNER:
			return "Voc\352 n\343o \351 o propriet\341rio.";

		case RETURNVALUE_YOUCANTOPENCORPSEADM:
			return "Voc\352 n\343o pode abrir este cad\341ver porque voc\352 \351 um administrador.";

		case RETURNVALUE_NOSUCHRAIDEXISTS:
			return "N\343o existe tal ataque.";

		case RETURNVALUE_ANOTHERRAIDISALREADYEXECUTING:
			return "Outro ataque j\341 est\341 em execu\347\343o.";

		case RETURNVALUE_TRADEPLAYERFARAWAY:
			return "O jogador comercial est\341 muito longe.";

		case RETURNVALUE_YOUDONTOWNTHISHOUSE:
			return "Voc\352 n\343o \351 dono desta casa.";

		case RETURNVALUE_TRADEPLAYERALREADYOWNSAHOUSE:
			return "O jogador comercial j\341 possui uma casa.";

		case RETURNVALUE_TRADEPLAYERHIGHESTBIDDER:
			return "O jogador comercial \351 atualmente o licitante com lance mais alto de uma casa leiloada.";

		case RETURNVALUE_YOUCANNOTTRADETHISHOUSE:
			return "Voc\352 n\343o pode negociar esta casa.";

		case RETURNVALUE_YOUDONTHAVEREQUIREDPROFESSION:
			return "Voc\352 n\343o tem a profiss\343o exigida.";

		case RETURNVALUE_NOTENOUGHFISTLEVEL:
			return "Voc\352 n\343o tem n\355vel de punho suficiente";

		case RETURNVALUE_NOTENOUGHCLUBLEVEL:
			return "Voc\352 n\343o tem n\355vel de clube suficiente";

		case RETURNVALUE_NOTENOUGHSWORDLEVEL:
			return "Voc\352 n\343o tem n\355vel de espada suficiente";

		case RETURNVALUE_NOTENOUGHAXELEVEL:
			return "Voc\352 n\343o tem n\355vel de machado suficiente";

		case RETURNVALUE_NOTENOUGHDISTANCELEVEL:
			return "Voc\352 n\343o tem n\355vel de dist\342ncia suficiente";

		case RETURNVALUE_NOTENOUGHSHIELDLEVEL:
			return "Voc\352 n\343o tem n\355vel de blindagem suficiente";

		case RETURNVALUE_NOTENOUGHFISHLEVEL:
			return "Voc\352 n\343o tem n\355vel de pesca suficiente";

		case RETURNVALUE_NOTPOSSIBLE:
			return "Desculpe, n\343o \351 poss\355vel.";

		case RETURNVALUE_REWARDCONTAINERISEMPTY:
			return "Voc\352 j\341 reivindicou sua recompensa.";

		case RETURNVALUE_CONTACTADMINISTRATOR:
			return "Ocorreu um erro, entre em contato com seu administrador.";

		case RETURNVALUE_ITEMISNOTYOURS:
			return "Este item n\343o \351 seu.";

		case RETURNVALUE_ITEMUNTRADEABLE:
			return "Este item n\343o \351 negoci\341vel.";
...
void consoleHandlerExit() {
	g_logger().error("O programa ser\341 fechado ap\363s pressionar a tecla Enter...");
...
uint8_t convertWheelGemAffinityToDomain(uint8_t affinity) {
	switch (affinity) {
		...
		case 3:
			return 2;
		default:
			g_logger().error("Falha ao obter afinidade de gema {}", affinity);
			return 0;
	}
}
