...
void House::updateDoorDescription() const {
	...
	if (owner != 0) {
		ss << "Pertence \340 casa '" << houseName << "'. " << ownerName << " \351 dono desta casa.";
	} else {
		ss << "Pertence \340 casa '" << houseName << "'. Ningu\351m \351 dono desta casa.";
	}

	if (!g_configManager().getBoolean(CYCLOPEDIA_HOUSE_AUCTION)) {
		ss << " Isso \351 " << getSize() << " metros quadrados.";
		const int32_t housePrice = getPrice();
		if (housePrice != -1) {
			if (g_configManager().getBoolean(HOUSE_PURSHASED_SHOW_PRICE) || owner == 0) {
				ss << " Custa " << formatNumber(getPrice()) << " moedas de ouro.";
			}
			std::string strRentPeriod = asLowerCaseString(g_configManager().getString(HOUSE_RENT_PERIOD));
			if (strRentPeriod != "never") {
				ss << " O custo do aluguel \351 " << formatNumber(getRent()) << " moedas de ouro e \351 faturado " << strRentPeriod << ".";
			}
		}
	}
...
bool House::transferToDepot(const std::shared_ptr<Player> &player) const {
	...
	g_logger().info("[House::transferToDepot] Transferência concluída para o jogador '{}'. Total de itens processados: {}, movidos: {}, faltando: {}", player->getName(), totalItemsFound, totalItemsMoved, totalItemsMissing);
	return true;
}
...
bool House::transferToDepot(const std::shared_ptr<Player> &player, const std::shared_ptr<HouseTile> &tile) const {
	...
	if (tile->getHouse().get() != this) {
		g_logger().debug("[{}] casa de azulejos n\343o \351 esta casa", __FUNCTION__);
		return false;
	}
	...
	for (const auto &item : moveItemList) {
		...
		if (item->hasOwner() && !item->isOwner(targetPlayer)) {
			...
			if (itemOwner) {
				...
				g_logger().warn("[{}] propriet\341rio do item '{}' (GUID: {}) n\343o encontrado, ignorando a transfer\352ncia", __FUNCTION__, item->getName(), item->getOwnerId());
				continue;
			}
		}
		...
		if (!itemParent) {
			g_logger().warn("[{}] item '{}' (id: {}) n\343o tem pai (j\341 empacotado ou removido), pulando movimento para evitar duplica\347\343o", __FUNCTION__, item->getName(), item->getID());
			continue;
		}
		...
		if (result != RETURNVALUE_NOERROR) {
			g_logger().error("[{}] Falha ao transferir o item '{}' (id: {}, pos: {}) para '{}', erro: {}", __FUNCTION__, item->getName(), item->getID(), tile->getPosition(), targetPlayer->getName(), result);
			failedItemTransfers.push_back(fmt::format("{} (id: {}) em {}", item->getName(), item->getID(), tile->getPosition().toString()));
			continue;
		}
		...
		movedItemsLog.emplace_back(fmt::format("{} (id: {}) em {}", item->getName(), item->getID(), tile->getPosition().toString()));
	}

	itemsFound = itemsMoved + static_cast<uint32_t>(failedItemTransfers.size());

	if (itemsFound > 0 || itemsMoved > 0) {
		g_logger().info("[House::transferToDepot] {} itens encontrados, {} transferidos com sucesso para o jogador '{}'", itemsFound, itemsMoved, player->getName());

		for (const auto &moved : movedItemsLog) {
			g_logger().info("[House::transferToDepot] Movido: {}", moved);
		}

		if (!failedItemTransfers.empty()) {
			for (const auto &entry : failedItemTransfers) {
				g_logger().warn("[House::transferToDepot] Falha ao transferir: {}", entry);
			}
		}
	}
	...
	for (const auto &tile : houseTiles) {
		if (const auto &items = tile->getItemList()) {
			for (const auto &item : *items) {
				...
				if (item->isWrapable()) {
					foundItem = true;
					g_logger().error("N\343o \351 poss\355vel comprar uma casa com item de embrulho dentro: id '{}', nome '{}'", item->getID(), item->getName());
					break;
				} else if (item->isPickupable()) {
					foundItem = true;
					g_logger().error("N\343o \351 poss\355vel comprar uma casa com item colecion\341vel dentro: id '{}', nome '{}'", item->getID(), item->getName());
					break;
				} else {
					if (item->getContainer() && (item->isPickupable() || item->isWrapable())) {
						foundItem = true;
						g_logger().error("N\343o \351 poss\355vel comprar uma casa com item recipiente dentro: id '{}', nome '{}'", item->getID(), item->getName());
						break;
					}
				}
			}
...
void House::handleWrapableItem(ItemList &moveItemList, const std::shared_ptr<Item> &item, const std::shared_ptr<Player> &player, const std::shared_ptr<HouseTile> &houseTile) const {
	...
	if (item->isWrapContainer()) {
		g_logger().debug("[{}] cont\352iner wrapable encontrado '{}'", __FUNCTION__, item->getName());
	...
	if (newItem->isRemoved() && !newItem->getParent()) {
		g_logger().warn("[{}] item removido durante o embrulho - verifique o tipo de terreno - nome do jogador: {} id do item: {} posi\347\343o: {}", __FUNCTION__, player->getName(), item->getID(), houseTile->getPosition().toString());
		return;
	}
...
std::shared_ptr<HouseTransferItem> HouseTransferItem::createHouseTransferItem(const std::shared_ptr<House> &house) {
	...
	std::ostringstream ss;
	ss << "\311 um documento de transfer\352ncia de casa para '" << house->getName() << "'.";
...
void HouseTransferItem::onTradeEvent(TradeEvents_t event, const std::shared_ptr<Player> &owner) {
	if (event == ON_TRADE_TRANSFER) {
		if (house) {
			auto isTransferOnRestart = g_configManager().getBoolean(TOGGLE_HOUSE_TRANSFER_ON_SERVER_RESTART);
			auto ownershipTransferMessage = " A propriedade ser\341 transferida ap\363s a reinicializa\347\343o do servidor.";
			const auto boughtMessage = fmt::format("Voc\352 comprou a casa com sucesso.{}", isTransferOnRestart ? ownershipTransferMessage : "");
			const auto soldMessage = fmt::format("Voc\352 vendeu sua casa com sucesso.{}", isTransferOnRestart ? ownershipTransferMessage : "");
	...
	for (const auto &it : houseMap) {
		...
		if (daysToReset > 0) {
			...
			if (vipKeep && !activityKeep) {
				g_logger().info("O jogador {} n\343o est\341 logado h\341 {} dias, mas \351 VIP, portanto a casa n\343o ser\341 zerada.", player->getName(), daysToReset);
			} else if (!vipKeep && !activityKeep) {
				g_logger().info("O jogador {} n\343o faz entrar h\341 {} dias, ent\343o a casa ser\341 reiniciada.", player->getName(), daysToReset);
		...
		if (player->getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				std::ostringstream ss;
				ss << "Aviso! \n\343o " << period << " aluguel de " << house->getRent() << " ouro para sua casa \"" << house->getName() << "\" \351 pag\341vel. Tenha isso dentro " << daysLeft << " dias ou voc\352 perder\341 esta casa.";
