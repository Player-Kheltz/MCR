...
void MapCache::parseItemAttr(const std::shared_ptr<BasicItem> &BasicItem, const std::shared_ptr<Item> &item) const {
	...
	// Aplica o destino a todos os itens que o possuam (escadas, rampas, teleportes)
	if (BasicItem->destX != 0 || BasicItem->destY != 0 || BasicItem->destZ != 0) {
		...
		// Se for teleporte, define o destino específico do teleporte
		if (item->getTeleport()) {
...
std::shared_ptr<Tile> MapCache::getOrCreateTileFromCache(const std::shared_ptr<Floor> &floor, uint16_t x, uint16_t y) {
	...
	if (cachedTile->isHouse()) {
		if (const auto &house = map->houses.getHouse(cachedTile->houseId)) {
			...
			g_logger().error("[{}] casa n\343o encontrada para houseId {}", std::source_location::current().function_name(), cachedTile->houseId);
		}
...
bool BasicItem::unserializeItemNode(FileStream &stream, uint16_t x, uint16_t y, uint8_t z) {
	...
	while (stream.startNode()) {
		if (stream.getU8() != OTBM_ITEM) {
			throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] Não foi possível ler o n\363 do item.", x, y, z));
		}
		...
		if (!item->unserializeItemNode(stream, x, y, z)) {
			throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] Falha ao carregar o item.", x, y, z));
		}
