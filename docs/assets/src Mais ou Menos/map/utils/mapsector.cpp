...
void MapSector::removeCreature(const std::shared_ptr<Creature> &c) {
	...
	if (iter == creature_list.end()) {
		g_logger().error("[{}]: Criatura n\343o encontrada em creator_list!", __FUNCTION__);
		return;
	}
	...
	*iter = creature_list.back();
	...
	if (c->getPlayer()) {
		...
		if (iter == player_list.end()) {
			g_logger().error("[{}]: Jogador n\343o encontrado em player_list!", __FUNCTION__);
			return;
		}
		...
		*iter = player_list.back();
		...
		if (iter == monster_list.end()) {
			g_logger().error("[{}]: Monstro n\343o encontrado em player_list!", __FUNCTION__);
			return;
		}
		...
		*iter = monster_list.back();
		...
		if (iter == npc_list.end()) {
			g_logger().error("[{}]: NPC n\343o encontrado em player_list!", __FUNCTION__);
			return;
		}
