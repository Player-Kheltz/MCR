...
void Logger::setLevel(const std::string &name) const {
	debug("Configurando o n\355vel de log para: {}.", name);
...
void Logger::logProfile(const std::string &name, double duration_ms) const {
	...
	if (it == profile_loggers_.end()) {
		try {
			...
			profile_logger->info("Fun\347\343o {} executada em {} ms", name, duration_ms);
		} catch (const spdlog::spdlog_ex &ex) {
			error("Falha na inicializa\347\343o do log do perfil: {}", ex.what());
		}
	} else {
		it->second->info("Fun\347\343o {} executada em {} ms", mutable_name, duration_ms);
	}
}
