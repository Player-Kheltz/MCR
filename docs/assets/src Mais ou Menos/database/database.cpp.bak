...
bool Database::connect(const std::string* host, const std::string* user, const std::string* password, const std::string* database, uint32_t port, const std::string* sock) {
	...
	if (!handle) {
		g_logger().error("Falha ao inicializar o identificador de conex\343o MySQL.");
		return false;
	}

	if (host->empty() || user->empty() || password->empty() || database->empty() || port <= 0) {
		g_logger().warn("Host MySQL, usu\341rio, senha, banco de dados ou porta n\343o fornecidos");
	}
	...
	if (!mysql_real_connect(handle, host->c_str(), user->c_str(), password->c_str(), database->c_str(), port, sock->c_str(), 0)) {
		g_logger().error("Mensagem de erro do MySQL: {}", mysql_error(handle));
		return false;
	}
...
void Database::createDatabaseBackup(bool compress) const {
	...
	if (configFile.is_open()) {
		...
		g_logger().error("Falha ao criar arquivo de configura\347\343o tempor\341rio do MySQL.");
		return;
	}
	...
	if (result != 0) {
		g_logger().error("Falha ao criar backup do banco de dados usando mysqldump.");
		return;
	}
	...
	if (!gzFile) {
		g_logger().error("Falha ao abrir o arquivo gzip para compacta\347\343o.");
		return;
	}

	std::ifstream backupFile(backupFileName, std::ios::binary);
	if (!backupFile.is_open()) {
		g_logger().error("Falha ao abrir arquivo de backup para compacta\347\343o: {}", backupFileName);
		gzclose(gzFile);
		return;
	}
	...
	g_logger().info("Backup do banco de dados compactado com sucesso para: {}", compressedFileName);
				...
				for (const auto &file : std::filesystem::directory_iterator(entry)) {
					if (file.path().extension() == ".gz") {
						...
						if (fileTimeSystemClock < sevenDaysAgo) {
							...
							g_logger().info("Arquivo de backup antigo exclu\355do: {}", file.path().string());
						}
					}
				}
...
bool Database::rollback() {
	if (!handle) {
		g_logger().error("Banco de dados n\343o inicializado!");
		return false;
	}

	if (mysql_rollback(handle) != 0) {
		g_logger().error("Mensagem: {}", mysql_error(handle));
		databaseLock.unlock();
		return false;
	}
...
bool Database::retryQuery(std::string_view query, int retries) {
	while (retries > 0 && mysql_query(handle, query.data()) != 0) {
		g_logger().error("Consulta: {}", query.substr(0, 256));
		g_logger().error("Erro MySQL [{}]: {}", mysql_errno(handle), mysql_error(handle));
	...
	if (retries == 0) {
		g_logger().error("A consulta {} falhou ap\363s {} tentativas.", query, 10);
		return false;
	}
...
bool Database::executeQuery(std::string_view query) {
	...
	g_logger().trace("Executando Consulta: {}", query);
...
DBResult_ptr Database::storeQuery(std::string_view query) {
	...
	g_logger().trace("Armazenando Consulta: {}", query);
...
std::string Database::escapeString(const std::string &s) const {
	...
	if (escaped.empty()) {
		g_logger().warn("Erro ao escapar da string");
	}
	return escaped;
}
...
uint8_t DBResult::getU8FromString(const std::string &string, const std::string &function) {
	...
	if (result > std::numeric_limits<uint8_t>::max()) {
		g_logger().error("[{}] Falha ao obter o valor num\351rico {} para o resultado da tabela de camadas, na chamada de fun\347\343o: {}", __FUNCTION__, result, function);
		return 0;
	}
