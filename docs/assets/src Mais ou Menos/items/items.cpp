...
std::string ItemType::parseAugmentDescription(bool inspect /*= false*/) const {
	...
	if (inspect) {
		...
		return fmt::format("\nAumenta: ({}).", fmt::join(descriptions.begin(), descriptions.end(), ", "));
	}
}
...
bool Items::loadFromXml() {
	...
	std::ifstream file(folder, std::ios::in | std::ios::binary);
	if (!file.is_open()) {
		g_logger().error("[Items::loadFromXml] - Failed to open items.xml from {}", folder);
		return false;
	}
	std::string xmlContent((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
	file.close();

	// Aplica conversão inteligente: UTF‑8 → Latin‑1; se já for Latin‑1, mantém.
	std::string convertedXml = toLatin1(xmlContent);

	// Carrega o buffer convertido no parser
	pugi::xml_parse_result result = doc.load_buffer(convertedXml.c_str(), convertedXml.size());
	if (!result) {
		printXMLError(__FUNCTION__, folder, result);
		return false;
	}

	// Processa os itens (o restante permanece igual)
...
void Items::parseItemNode(const pugi::xml_node &itemNode, uint16_t id) {
	...
	if (id >= 100 && (itemType.id == 0 && (itemType.name.empty() || itemType.name == asLowerCaseString("ator reservado")))) {
		return;
	}
	itemType.id = id;
