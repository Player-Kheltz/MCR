...
namespace {
	...
	void appendSkillBoostDescription(const ItemType &it, const std::shared_ptr<Item> &item, DescriptionList &descriptions, std::ostringstream &ss) {
		...
		if (it.abilities->stats[STAT_MAGICPOINTS]) {
			...
			ss << std::showpos << "n\355vel m\341gico " << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
			skillBoost = true;
		}
		...
		if (skillBoost) {
			descriptions.emplace_back("Aumento de habilidade", ss.str());
		}
	}
	...
	void appendSpecializedMagicLevelDescriptions(const ItemType &it, DescriptionList &descriptions, std::ostringstream &ss) {
		for (uint8_t i = 1; i <= 11; i++) {
			if (it.abilities->specializedMagicLevel[i]) {
				...
				descriptions.emplace_back(combatName + " N\355vel M\341gico", ss.str());
			}
		}
	}
	...
	void appendMagicShieldCapacityDescription(const ItemType &it, DescriptionList &descriptions, std::ostringstream &ss) {
		...
		descriptions.emplace_back("Capacidade do Escudo M\341gico", ss.str());
	}
	...
	void appendPerfectShotDescription(const ItemType &it, DescriptionList &descriptions, std::ostringstream &ss) {
		...
		ss.str("");
		ss << std::showpos << it.abilities->perfectShotDamage << std::noshowpos << " ao alcance " << unsigned(it.abilities->perfectShotRange);
		descriptions.emplace_back("Tiro Perfeito", ss.str());
	}
	...
	void appendDamageReflectionDescription(const ItemType &it, DescriptionList &descriptions, std::ostringstream &ss) {
		...
		descriptions.emplace_back("Reflex\343o de Danos", ss.str());
	}
	...
	void appendAbilityEffects(const ItemType &it, DescriptionList &descriptions, std::ostringstream &ss) {
		...
		if (it.abilities->regeneration) {
			ss.str("");
			ss << "Regenera\347\343o mais r\341pida";
			descriptions.emplace_back("Effect", ss.str());
		}
	...
	void appendFieldProtectionDescription(const ItemType &it, DescriptionList &descriptions, std::ostringstream &ss) {
		for (size_t i = 0; i < COMBAT_COUNT; ++i) {
			...
			descriptions.emplace_back("Prote\347\343o de Campo", ss.str());
		}
	}
	...
	void appendShowAttributesSkills(const ItemType &itemType, const std::shared_ptr<Item> &item, bool &begin, std::ostringstream &itemDescription) {
		...
		if (itemType.abilities->stats[STAT_MAGICPOINTS]) {
			appendAttributeSeparator(begin, itemDescription);
			itemDescription << "n\355vel m\341gico " << std::showpos << itemType.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
		}

		for (uint8_t i = 1; i <= 11; i++) {
			if (itemType.abilities->specializedMagicLevel[i]) {
				appendAttributeSeparator(begin, itemDescription);
				itemDescription << getCombatName(indexToCombatType(i)) << " n\355vel m\341gico " << std::showpos << itemType.abilities->specializedMagicLevel[i] << std::noshowpos;
			}
		}
	...
	void appendShowAttributesAbilities(const ItemType &itemType, bool &begin, std::ostringstream &itemDescription) {
		if (itemType.abilities->magicShieldCapacityFlat || itemType.abilities->magicShieldCapacityPercent) {
			appendAttributeSeparator(begin, itemDescription);
			itemDescription << "Capacidade do Escudo M\341gico " << std::showpos << itemType.abilities->magicShieldCapacityFlat << std::noshowpos << " and " << itemType.abilities->magicShieldCapacityPercent << "%";
		}

		if (itemType.abilities->perfectShotRange) {
			appendAttributeSeparator(begin, itemDescription);
			itemDescription << "tiro perfeito " << std::showpos << itemType.abilities->perfectShotDamage << std::noshowpos << " ao alcance " << unsigned(itemType.abilities->perfectShotRange);
		}

		int16_t reflectPhysical = itemType.abilities->reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)];
		if (reflectPhysical != 0) {
			appendAttributeSeparator(begin, itemDescription);
			itemDescription << "reflex\343o de danos " << std::showpos << reflectPhysical << std::noshowpos;
		}
	}
	...
	void appendShowAttributesProtection(const ItemType &itemType, bool &begin, std::ostringstream &itemDescription) {
		...
		if (!show) {
			...
			appendAttributeSeparator(begin, itemDescription);
			itemDescription << fmt::format("prote\347\343o todos {:+}%", show);
		}
	}
		...
		if (!show) {
			...
			for (size_t i = 0; i < COMBAT_COUNT; ++i) {
				...
				if (fieldProtectionBegin) {
					...
					appendAttributeSeparator(begin, itemDescription);
					itemDescription << "prote\347\343o de campo ";
				} else {
					itemDescription << ", ";
				}
			...
			appendAttributeSeparator(begin, itemDescription);
			itemDescription << fmt::format("prote\347\343o de campo todos {:+}%", show);
		}
	}
}
...
std::shared_ptr<Item> Item::CreateItem(const uint16_t type, uint16_t count /*= 0*/, Position* itemPosition /*= nullptr*/, bool createWrappableItem /* false*/, bool customCharges /* false */) {
	...
	if (createWrappableItem && it.wrapable && it.wrapableTo > 0) {
		...
		wrappedItem->setAttribute(ItemAttribute_t::DESCRIPTION, "Desembrulhe-o em sua pr\363pria casa para criar um <" + it.name + ">.");
		return wrappedItem;
	}
...
bool Item::canAddImbuement(uint8_t slot, const std::shared_ptr<Player> &player, const Imbuement* imbuement) {
	...
	if (itemSlots == 0 || slot >= itemSlots) {
		...
		player->sendImbuementResult("Sele\347\343o de slot inv\341lida.");
		return false;
	}
	...
	if (hasImbuementCategoryId(categoryImbuement->id)) {
		...
		player->sendImbuementResult("Ocorreu um erro, reabra a janela de imbuement.");
		return false;
	}
...
uint8_t Item::getTier() const {
	...
	if (tier > g_configManager().getNumber(FORGE_MAX_ITEM_TIER)) {
		g_logger().error("{} - O item {} tem um n\355vel {} errado", __FUNCTION__, getName(), tier);
		return 0;
	}
	...
	switch (attr) {
		...
		case ATTR_TIER: {
			...
			if (!propStream.read<uint8_t>(tier)) {
				g_logger().error("[{}] n\343o conseguiu ler o n\355vel", __FUNCTION__);
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_AMOUNT: {
			...
			if (!propStream.read<uint16_t>(amount)) {
				g_logger().error("[{}] n\343o conseguiu ler o valor", __FUNCTION__);
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_CUSTOM: {
			uint64_t size;
			if (!propStream.read<uint64_t>(size)) {
				g_logger().error("[{}] n\343o conseguiu ler o tamanho", __FUNCTION__);
				return ATTR_READ_ERROR;
			}
			...
			for (uint64_t i = 0; i < size; i++) {
				...
				std::string key;
				if (!propStream.readString(key)) {
					g_logger().error("[{}] n\343o conseguiu ler o tipo personalizado", __FUNCTION__);
					return ATTR_READ_ERROR;
				};

				CustomAttribute customAttribute;
				if (!customAttribute.unserialize(propStream, __FUNCTION__)) {
					g_logger().error("[{}] n\343o conseguiu ler o valor personalizado", __FUNCTION__);
					return ATTR_READ_ERROR;
				}
		...
		case ATTR_OBTAINCONTAINER: {
			...
			g_logger().trace("Configurando sinalizadores de obten\347\343o do sinalizador {}, para o ID do item {}", flags, getID());
...
void Item::serializeAttr(PropWriteStream &propWriteStream) const {
	...
	if (hasAttribute(ItemAttribute_t::OBTAINCONTAINER)) {
		...
		g_logger().debug("Lendo o sinalizador {}, para o ID do item {}", flags, getID());
	...
	if (item) {
		...
		if (it.isRanged()) {
			...
			if (int32_t hitChance = item->getHitChance();
				...
				ss << "chance de acertar +" << static_cast<int16_t>(hitChance) << "%";
				separator = true;
			}
			...
			if (attack != 0 && !attackDescription.empty()) {
				attackDescription = fmt::format("{} f\355sico + {}", attack, attackDescription);
		...
		if (it.abilities) {
			...
			if (it.abilities->stats[STAT_MAGICPOINTS]) {
				...
				descriptions.emplace_back("N\355vel M\341gico", ss.str());
			}
		...
		if (item->getImbuementSlot() > 0) {
			for (uint8_t i = 0; i < item->getImbuementSlot(); ++i) {
				slotName = fmt::format("Slot de Imbuimento {}", i + 1);
				ss.str("");
				...
				ss << fmt::format("{} {} ({}), dura {:02}:{:02}h enquanto luta.", baseImbuement->name, imbuementInfo.imbuement->getName(), imbuementInfo.imbuement->getDescription(), hours, minutes % 60);
				isTradeable = false;
				descriptions.emplace_back(slotName, ss.str());
			}
		}
		...
		if (it.isRune()) {
			descriptions.emplace_back("Nome do Feiti\347o R\372nico", it.runeSpellName);
		}
		...
		if (weight != 0) {
			...
			ss << " oz";
			if (item->getContainer()) {
				descriptions.emplace_back("Peso total", ss.str());
			} else {
		...
		if (it.minReqLevel != 0) {
			descriptions.emplace_back("N\355vel Requerido", std::to_string(it.minReqLevel));
		}

		if (it.minReqMagicLevel != 0) {
			descriptions.emplace_back("N\355vel M\341gico Requerido", std::to_string(it.minReqMagicLevel));
		}
		...
		if (!weaponName.empty()) {
			descriptions.emplace_back("Tipo de arma", weaponName);
		}

		if (it.slotPosition & SLOTP_BACKPACK) {
			descriptions.emplace_back("Posi\347\343o do corpo", "container");
		} else if (it.slotPosition & SLOTP_HEAD) {
			descriptions.emplace_back("Posi\347\343o do corpo", "head");
		} else if (it.slotPosition & SLOTP_ARMOR) {
			descriptions.emplace_back("Posi\347\343o do corpo", "body");
		} else if (it.slotPosition & SLOTP_LEGS) {
			descriptions.emplace_back("Posi\347\343o do corpo", "legs");
		} else if (it.slotPosition & SLOTP_FEET) {
			descriptions.emplace_back("Posi\347\343o do corpo", "feet");
		} else if (it.slotPosition & SLOTP_NECKLACE) {
			descriptions.emplace_back("Posi\347\343o do corpo", "neck");
		} else if (it.slotPosition & SLOTP_RING) {
			descriptions.emplace_back("Posi\347\343o do corpo", "finger");
		} else if (it.slotPosition & SLOTP_AMMO) {
			descriptions.emplace_back("Posi\347\343o do corpo", "slot extra");
		} else if (it.slotPosition & SLOTP_TWO_HAND) {
			descriptions.emplace_back("Posi\347\343o do corpo", "ambas as m\343os");
		} else if ((it.slotPosition & SLOTP_LEFT) && it.weaponType != WEAPON_SHIELD) {
			descriptions.emplace_back("Posi\347\343o do corpo", "m\343o de arma");
		} else if (it.slotPosition & SLOTP_RIGHT) {
			descriptions.emplace_back("Posi\347\343o do corpo", "m\343o de escudo");
		}
		...
		if (it.elementalBond != COMBAT_NONE) {
			[[maybe_unused]] auto &[desc_key, desc_val] = descriptions.emplace_back("Liga\347\343o Elemental", toPascalCase(getCombatName(it.elementalBond)));
		}
	} else {
		...
		if (it.imbuementSlot > 0) {
			descriptions.emplace_back("Slots de Imbuimento", std::to_string(it.imbuementSlot));
		}
		...
		if (it.showDuration) {
			...
			descriptions.emplace_back("Tempo total de expira\347\343o", "brand-new");
		}
		...
		// Missing Tradeable Conditions
		descriptions.emplace_back("Negoci\341vel n\343o mercado", "yes");
		...
		if (it.elementalBond != COMBAT_NONE) {
			[[maybe_unused]] auto &[desc2_key, desc2_val] = descriptions.emplace_back("Liga\347\343o Elemental", toPascalCase(getCombatName(it.elementalBond)));
		}
	}
...
std::string Item::parseClassificationDescription(const std::shared_ptr<Item> &item) {
	if (item && item->getClassification() >= 1) {
		return fmt::format("\nClassifica\347\343o: {} N\355vel: {}.", item->getClassification(), getTierEffectDescription(item));
	}
	return "";
}
...
std::string Item::getTierEffectDescription(const std::shared_ptr<Item> &item) {
	...
	if (Item::items[item->getID()].weaponType != WEAPON_NONE) {
		effectDescription = fmt::format(" ({:.2f}% Ataque)", item->getFatalChance());
	} else {
		switch (g_game().getObjectCategory(item)) {
			case OBJECTCATEGORY_HELMETS:
				effectDescription = fmt::format(" ({:.2f}% Momento)", item->getMomentumChance());
				break;
			case OBJECTCATEGORY_ARMORS:
				effectDescription = fmt::format(" ({:.2f}% Rude)", item->getDodgeChance());
				break;
			case OBJECTCATEGORY_LEGS:
				effectDescription = fmt::format(" ({:.2f}% Transcend\352ncia)", item->getTranscendenceChance());
				break;
			case OBJECTCATEGORY_BOOTS:
				effectDescription = fmt::format(" ({:.2f}% Amplifica\347\343o)", item->getAmplificationChance());
				break;
			default:
				break;
		}
...
std::string Item::parseShowDurationSpeed(int32_t speed, bool &begin) {
	...
	description << fmt::format("velocidade {:+}", speed);
...
std::string Item::parseShowDuration(const std::shared_ptr<Item> &item) {
	...
	if (item && item->hasAttribute(ItemAttribute_t::DURATION) && duration > 0) {
		description << " que ir\341 expirar em ";
		if (duration >= 86400) {
	...
	if (itemType.armor != 0 || (item && item->getArmor() != 0) || itemType.showAttributes) {
		...
		if (armor != 0) {
			if (begin) {
				...
				itemDescription << ", Bra\347o:" << armor;
			}
		}
...
std::string Item::getDescription(const ItemType &it, int32_t lookDistance, const std::shared_ptr<Item> &item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/) {
	...
	if (it.isRune()) {
		if (it.runeLevel > 0 || it.runeMagLevel > 0) {
			if (const auto &rune = g_spells().getRuneSpell(it.id)) {
				...
				s << " (\"" << it.runeSpellName << "\"). " << (it.stackable && tmpSubType > 1 ? "They" : "It") << " s\363 pode ser usado por ";
				...
				if (it.runeMagLevel > 0) {
					...
					s << " n\355vel m\341gico " << it.runeMagLevel;
				}
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			if (hitChance != 0) {
				s << ", Bater% " << std::showpos << static_cast<int16_t>(hitChance) << std::noshowpos;
			}
			...
			if (it.abilities) {
				...
				if (it.abilities->stats[STAT_MAGICPOINTS]) {
					...
					s << "n\355vel m\341gico " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
				}
				...
				for (uint8_t i = 1; i <= 11; i++) {
					if (it.abilities->specializedMagicLevel[i]) {
						...
						s << getCombatName(indexToCombatType(i)) << " n\355vel m\341gico " << std::showpos << it.abilities->specializedMagicLevel[i] << std::noshowpos;
					}
				}
				...
				if (it.abilities->magicShieldCapacityFlat || it.abilities->magicShieldCapacityPercent) {
					...
					s << "capacidade do escudo m\341gico " << std::showpos << it.abilities->magicShieldCapacityFlat << std::noshowpos << " and " << it.abilities->magicShieldCapacityPercent << "%";
				}
				...
				if (it.abilities->perfectShotRange) {
					...
					s << "tiro perfeito " << std::showpos << it.abilities->perfectShotDamage << std::noshowpos << " ao alcance " << unsigned(it.abilities->perfectShotRange);
				}
				...
				if (it.abilities->reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)]) {
					...
					s << "reflex\343o de danos " << std::showpos << it.abilities->reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)] << std::noshowpos;
				}
				...
				if (show == 0) {
					...
					s << fmt::format("prote\347\343o todos {:+}%", show);
				}
				...
				if (show == 0) {
					...
					for (size_t i = 0; i < COMBAT_COUNT; ++i) {
						...
						s << fmt::format("{} campo {:+}%", getCombatName(indexToCombatType(i)), it.abilities->fieldAbsorbPercent[i]);
					}
				} else {
					if (begin) {
						begin = false;
					...
					s << fmt::format("prote\347\343o de todos os campos {:+}%", show);
				}
			...
			if (it.abilities && it.abilities->elementType != COMBAT_NONE && it.abilities->elementDamage != 0 && !begin) {
				s << " f\355sico + " << it.abilities->elementDamage << ' ' << getCombatName(it.abilities->elementType);
			...
			if (it.abilities) {
				...
				if (it.abilities->regeneration) {
					...
					s << "regenera\347\343o mais r\341pida";
				}
	...
	if (it.showCharges) {
		if (subType == 0) {
			s << " isso tem " << it.charges << " charge" << (subType != 1 ? "s" : "") << " left";
		} else {
			s << " isso tem " << subType << " charge" << (subType != 1 ? "s" : "") << " left";
		}
	}
	...
	if (it.wieldInfo != 0) {
		s << std::endl
		  << "S\363 pode ser manejado corretamente por ";
		...
		if (it.wieldInfo & WIELDINFO_LEVEL) {
			s << " de n\355vel " << it.minReqLevel << " ou superior";
		}
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " n\355vel m\341gico " << it.minReqMagicLevel << " ou superior";
		}
	...
	if (it.elementalBond != COMBAT_NONE) {
		s << fmt::format("\nLiga\347\343o Elemental: {}.", toPascalCase(getCombatName(it.elementalBond)));
	}
...
std::shared_ptr<Item> Item::transform(uint16_t itemId, uint16_t itemCount /*= -1*/) {
	...
	if (cylinder == nullptr) {
		g_logger().info("[{}] falhou ao transformar o item {}, o cilindro \351 nullptr", __FUNCTION__, getID());
		return nullptr;
	}
...
void Player::addRemoteContainer(std::shared_ptr<Container> container) {
    if (!container) return;
    m_remoteContainers.push_back(container);
}

bool Player::isRemoteContainer(std::shared_ptr<Container> container) const {
    for (const auto& c : m_remoteContainers) {
        if (c == container) return true;
    }
    return false;
}

void Player::removeRemoteContainer(std::shared_ptr<Container> container) {
    auto it = std::find(m_remoteContainers.begin(), m_remoteContainers.end(), container);
    if (it != m_remoteContainers.end()) {
        m_remoteContainers.erase(it);
    }
}
