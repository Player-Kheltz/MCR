...
bool CustomAttribute::unserialize(PropStream &propStream, const std::string &function) {
	...
	if (!propStream.read<uint8_t>(type)) {
		g_logger().error("[{}] Falha ao ler o tipo", function);
		return false;
	}
	...
	switch (type) {
		case 1: {
			...
			if (!propStream.readString(readString)) {
				g_logger().error("[{}] falha ao ler string, chamar fun\347\343o: {}", __FUNCTION__, function);
				return false;
			}
			value = readString;
			break;
		...
		case 2: {
			...
			if (!propStream.read<int64_t>(readInt)) {
				g_logger().error("[{}] falha ao ler int64, fun\347\343o de chamada: {}", __FUNCTION__, function);
				return false;
			}
			value = readInt;
			break;
		...
		case 3: {
			...
			if (!propStream.read<double>(readDouble)) {
				g_logger().error("[{}] falhou ao ler duplo, chamar fun\347\343o: {}", __FUNCTION__, function);
				return false;
			}
			value = readDouble;
			break;
		...
		case 4: {
			...
			if (!propStream.read<bool>(readBoolean)) {
				g_logger().error("[{}] falhou ao ler booleano, chamar fun\347\343o: {}", __FUNCTION__, function);
				return false;
			}
			value = readBoolean;
			break;
