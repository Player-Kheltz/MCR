...
void ItemParse::parseTransform(const std::string &stringValue, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	if (stringValue == "transformequipto") {
		...
		if (itemType.transformEquipTo == itemType.decayTo) {
			g_logger().warn("[{}] item com id {} est\341 se transformando ao equipar para o mesmo id de decaimento para '{}'", __FUNCTION__, itemType.id, itemType.decayTo);
			itemType.decayTo = 0;
		}
		...
		if (itemType.transformDeEquipTo == itemType.decayTo) {
			g_logger().warn("[{}] item com id {} est\341 se transformando ao ser desequipado para o mesmo id de decaimento para '{}'", __FUNCTION__, itemType.id, itemType.decayTo);
			itemType.decayTo = 0;
		}
		...
		for (const auto subAttributeNode : attributeNode.children()) {
			...
			if (augmentEnum.has_value()) {
				...
				if (!augmentValueAttributeNode.empty()) {
					...
					g_logger().warn("[{}] - O item '{}' tem um aumento '{}' sem valor", __FUNCTION__, itemType.name, augmentName);
				}
				...
				g_logger().warn("[{}] - Tipo desconhecido '{}'", __FUNCTION__, subValueAttribute.as_string());
			}
		}
	}
}
...
void ItemParse::parseStackSize(const std::string &stringValue, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	if (stringValue == "stacksize") {
		...
		if (stackSize > 255) {
			stackSize = 255;
			g_logger().warn("[{}] Valor de tamanho de pilha inv\341lido: {}. O tamanho da pilha deve estar entre 1 e 255.", __FUNCTION__, stackSize);
		}
...
void ItemParse::parseHouseRelated(std::string_view stringValue, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	if (stringValue == "usedbyhouseguests") {
		g_logger().debug("[{}] item {}, usado pelos convidados {}", __FUNCTION__, itemType.id, valueAttribute.as_bool());
...
void ItemParse::createAndRegisterScript(ItemType &itemType, pugi::xml_node attributeNode, MoveEvent_t eventType /*= MOVE_EVENT_NONE*/, WeaponType_t weaponType /*= WEAPON_NONE*/) {
	...
	if (weaponType != WEAPON_NONE) {
		...
		g_logger().trace("Arma criada com tipo '{}'", getWeaponName(weaponType));
	}
		...
		if (stringKey == "slot") {
			...
			if (moveevent && (moveevent->getEventType() == MOVE_EVENT_EQUIP || moveevent->getEventType() == MOVE_EVENT_DEEQUIP)) {
				if (slotName == "head") {
					...
					g_logger().warn("[{}] tipo de slot desconhecido '{}'", __FUNCTION__, slotName);
				}
			} else if (weapon) {
				...
				} else if (slotName == "hand") {
					// Mantém o SLOTP_TWO_HAND se já estiver definido (ex.: armas fist two-handed)
			...
			if (moveevent) {
				g_logger().trace("Adicionado n\355vel de evento de movimento obrigat\363rio '{}'", numberValue);
				moveevent->setRequiredLevel(numberValue);
				moveevent->setWieldInfo(WIELDINFO_LEVEL);
			} else if (weapon) {
				g_logger().trace("Adicionado n\355vel de arma obrigat\363rio '{}'", numberValue);
			...
			if (elementName == "earth") {
				...
				g_logger().warn("[{}] - tipo de varinha '{}' n\343o existe", __FUNCTION__, elementName);
			}
			...
			if (doubleValue > 0) {
				...
				g_logger().trace("Valor de habilidade em cadeia encontrado '{}' para arma: {}", doubleValue, itemType.name);
			}
			if (doubleValue < 0.1 && subValueAttribute.as_bool() == false) {
				weapon->setDisabledChain();
				g_logger().trace("Corrente desativada para arma: {}", itemType.name);
			}
		}
	}
...
void ItemParse::parseUnscriptedItems(std::string_view stringValue, pugi::xml_node attributeNode, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	if (stringValue == "script") {
		...
		for (const auto &token : tokens) {
			if (token == "moveevent") {
				g_logger().trace("Registrando moveevent para item id '{}', nome '{}'", itemType.id, itemType.name);
				...
				for (const auto &subAttributeNode : attributeNode.children()) {
					...
					if (stringKey == "eventtype") {
						...
						g_logger().trace("Tipo de evento encontrado '{}'", eventTypeName);
						break;
					}
				}
				...
				g_logger().trace("Registrando arma para item id '{}', nome '{}'", itemType.id, itemType.name);
				for (const auto &subAttributeNode : attributeNode.children()) {
					pugi::xml_attribute subKeyAttribute = subAttributeNode.attribute("key");
					if (!subKeyAttribute) {
						continue;
					...
					if (stringKey == "weapontype") {
						...
						g_logger().trace("Tipo de arma encontrado '{}''", subValueAttribute.as_string());
						break;
					}
				}
