...
int32_t Weapon::playerWeaponCheck(const std::shared_ptr<Player> &player, const std::shared_ptr<Creature> &target, uint8_t shootRange) const {
    ...
    int32_t effectiveRange = static_cast<int32_t>(shootRange) + player->getRangeModifier();
	if (effectiveRange < 1) {
        effectiveRange = 1; // nunca menos que 1 tile
    }

    if (std::max<uint32_t>(Position::getDistanceX(playerPos, targetPos), Position::getDistanceY(playerPos, targetPos)) > effectiveRange) {
        return 0;
    }
