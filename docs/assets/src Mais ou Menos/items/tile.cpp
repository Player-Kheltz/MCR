...
bool Tile::hasProperty(const std::shared_ptr<Item> &exclude, ItemProperty prop) const {
	if (!exclude) {
		g_logger().error("[{}]: excluir \351 nullptr", __FUNCTION__);
		return false;
	}
...
std::string Tile::getDescription(int32_t) {
	return "Voc\352 n\343o sabe por que, mas n\343o consegue ver nada!";
}
...
ReturnValue Tile::queryAdd(int32_t, const std::shared_ptr<Thing> &thing, uint32_t, uint32_t tileFlags, const std::shared_ptr<Creature> &) {
	...
	if (const auto &creature = thing->getCreature()) {
		...
		// ========================================================================
		// MCR – Perseguição multi‑piso (pathfinding)
		// ========================================================================
		if (hasBitSet(FLAG_PATHFINDING, tileFlags) && hasFlag(TILESTATE_FLOORCHANGE | TILESTATE_TELEPORT)) {
			if (const auto &monster = creature->getMonster()) {
				// NOVO: usa isPursuingStairs() e getStairOrigin()
		...
		if (const auto &monster = creature->getMonster()) {
			// ========================================================================
			// MCR – Perseguição multi‑piso (entrada no tile)
			// ========================================================================
			// NOVO: usa isPursuingStairs() e getStairOrigin()
			if (monster->isPursuingStairs() && monster->getStairOrigin() == getPosition()) {
...
void Tile::addThing(int32_t, const std::shared_ptr<Thing> &thing) {
	...
	// ═══ MCR – Notificar o mapa cognitivo após adicionar um item ═══
	if (thing && thing->getItem()) {
		g_globalMonsterMap().notifyTileChange(getPosition());
	}
}
...
void Tile::updateThing(const std::shared_ptr<Thing> &thing, uint16_t itemId, uint32_t count) {
    ...
    // ═══ MCR – Notificar após actualização de item ═══
    g_globalMonsterMap().notifyTileChange(getPosition());
}
...
void Tile::replaceThing(uint32_t index, const std::shared_ptr<Thing> &thing) {
	...
	if (isInserted) {
        ...
        // ═══ MCR – Notificar após substituição de item ═══
        g_globalMonsterMap().notifyTileChange(getPosition());
    }
}
...
void Tile::removeThing(const std::shared_ptr<Thing> &thing, uint32_t count) {
	...
	if (item->isAlwaysOnTop()) {
        ...
        // ═══ MCR – Notificar o mapa cognitivo após remover um item ═══
        g_globalMonsterMap().notifyTileChange(getPosition());
    }
}
...
Position Tile::getDestination() const {
    // 1. Teleportes (já funcionava)
    auto teleport = getTeleportItem();
    if (teleport) {
        return teleport->getDestPos();
    }

    // 2. Escadas e rampas com destino explícito (procuramos nos itens do tile)
    const TileItemVector* items = getItemList();
    if (items) {
        for (const auto& item : *items) {
            if (!item) continue;
            const ItemType& it = Item::items[item->getID()];
            // Se o item tem a propriedade de mudar de piso...
            if (it.floorChange) {
                // Tenta obter um destino armazenado (raro, mas possível)
                // Como não temos getDestination() no Item, confiamos no fallback abaixo
                // que assume mesmo (x,y) e z±1.
                // Apenas retornamos Position(0,0,0) para sinalizar que não há destino explícito.
                return Position();
            }
        }
    }
