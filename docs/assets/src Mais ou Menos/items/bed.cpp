...
void BedItem::forceClearSleeper() {
	...
	sleeperGUID = 0;
	sleepStart = 0;
	setAttribute(ItemAttribute_t::DESCRIPTION, "Ningu\351m est\341 dormindo l\341.");
	...
	switch (attr) {
		case ATTR_SLEEPERGUID: {
			...
			if (guid != 0) {
				...
				if (!name.empty()) {
					setAttribute(ItemAttribute_t::DESCRIPTION, name + " est\341 dormindo l\341.");
...
bool BedItem::isBedComplete(const std::shared_ptr<BedItem> &nextBedItem) const {
	...
	g_logger().debug("ID da primeira parte da cama {} nome {}, ID da segunda parte {} nome {}", it.id, firstPart, nextBedItem->getID(), nextPartOf);
...
void BedItem::internalSetSleeper(const std::shared_ptr<Player> &player) {
	const std::string desc_str = player->getName() + " est\341 dormindo l\341.";
