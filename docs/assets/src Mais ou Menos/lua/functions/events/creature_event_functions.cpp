...
void CreatureEventFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "CreatureEvent", "onSpellCast", CreatureEventFunctions::luaCreatureEventOnCallback);
	Lua::registerMethod(L, "CreatureEvent", "onUseItem", CreatureEventFunctions::luaCreatureEventOnCallback);
	Lua::registerMethod(L, "CreatureEvent", "onDealtDamage", CreatureEventFunctions::luaCreatureEventOnCallback);
	Lua::registerMethod(L, "CreatureEvent", "onFightModeChange", CreatureEventFunctions::luaCreatureEventOnCallback);
	Lua::registerMethod(L, "CreatureEvent", "onDominioAction", CreatureEventFunctions::luaCreatureEventOnCallback);
}
...
int CreatureEventFunctions::luaCreatureEventType(lua_State* L) {
	...
	if (creatureEvent) {
		...
		if (tmpStr == "login") {
			...
			creatureEvent->setEventType(CREATURE_EVENT_SPELL);
		} else if (tmpStr == "itemuse") {
			creatureEvent->setEventType(CREATURE_EVENT_ITEM_USE);
		} else if (tmpStr == "dealtdamage") {
    		creatureEvent->setEventType(CREATURE_EVENT_DEALT_DAMAGE);
		} else if (tmpStr == "fightmodechange") {
    		creatureEvent->setEventType(CREATURE_EVENT_FIGHT_MODE_CHANGE);
		} else if (tmpStr == "dominioaction") {
    		creatureEvent->setEventType(CREATURE_EVENT_DOMINIO_ACTION);	
		} else {
