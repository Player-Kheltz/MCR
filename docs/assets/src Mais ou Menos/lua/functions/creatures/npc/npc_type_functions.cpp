...
int NpcTypeFunctions::luaNpcTypeCreate(lua_State* L) {
	...
	// NpcType(name)
	if (Lua::isString(L, 1)) {
		std::string originalName = Lua::getString(L, 1);
		std::string name = toLatin1(originalName);

		// Usa o método público que cria/obtém o tipo automaticamente
		auto npcType = g_npcs().getNpcType(name, true);
		if (!npcType) {
			lua_pushstring(L, "NpcType is nullptr");
			lua_error(L);
			return 1;
		}

		npcType->name = name;
		npcType->nameDescription = "a " + name;   // texto exibido ao passar o mouse
