...
void CreatureFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "Creature", "knockback", CreatureFunctions::luaCreatureKnockback);
...
int CreatureFunctions::luaCreatureKnockback(lua_State* L) {
    auto creature = Lua::getUserdataShared<Creature>(L, 1, "Creature");
    if (!creature) {
        lua_pushboolean(L, false);
        return 1;
    }
    Direction dir = Lua::getNumber<Direction>(L, 2);
    lua_pushboolean(L, creature->knockback(dir));
    return 1;
}
