...
void MonsterFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "Monster", "getRaceId", MonsterFunctions::luaMonsterGetRaceId);	

	Lua::registerMethod(L, "Monster", "createInventory", MonsterFunctions::luaMonsterCreateInventory);
	Lua::registerMethod(L, "Monster", "addItemToInventory", MonsterFunctions::luaMonsterAddItemToInventory);
	Lua::registerMethod(L, "Monster", "getInventoryContainer", MonsterFunctions::luaMonsterGetInventoryContainer);
...
int MonsterFunctions::luaMonsterGetInventoryContainer(lua_State* L) {
    const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
    if (!monster) {
        lua_pushnil(L);
        return 1;
    }
    auto container = monster->getInventoryContainer();
    if (container) {
...
int MonsterFunctions::luaMonsterCreateInventory(lua_State* L) {
	const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
	if (!monster) {
		Lua::pushBoolean(L, false);
		return 1;
	}
	uint16_t size = Lua::getNumber<uint16_t>(L, 2, 10);
	monster->createInventoryContainer(size);
	Lua::pushBoolean(L, true);
	return 1;
}

int MonsterFunctions::luaMonsterAddItemToInventory(lua_State* L) {
	const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
	if (!monster) {
		Lua::pushBoolean(L, false);
		return 1;
	...
	uint16_t count = Lua::getNumber<uint16_t>(L, 3, 1);
	auto container = monster->getInventoryContainer();
	if (!container) {
		Lua::pushBoolean(L, false);
		return 1;
	}
	auto item = Item::CreateItem(itemId, count);
	if (item) {
...
int MonsterFunctions::luaMonsterGetRaceId(lua_State* L) {
    const auto &monster = Lua::getUserdataShared<Monster>(L, 1, "Monster");
    if (!monster) {
        lua_pushnil(L);
        return 1;
    }
    Lua::pushNumber(L, static_cast<int>(monster->getRace()));
    return 1;
}
