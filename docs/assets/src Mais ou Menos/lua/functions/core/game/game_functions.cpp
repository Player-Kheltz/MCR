...
int GameFunctions::luaGameCreateMonsterType(lua_State* L) {
	...
	if (Lua::isString(L, 1)) {
		const std::string originalName = Lua::getString(L, 1);
		// Converte o nome para Latin-1 (codificação esperada pelo cliente clássico)
		const std::string name = toLatin1(originalName);
		std::string uniqueName = name;
		auto variant = Lua::getString(L, 2, "");
		const std::string originalAlternate = Lua::getString(L, 3, "");
		const std::string alternateName = toLatin1(originalAlternate);
		...
		// se a variante começa com "!", é a única variante para este monstro; registra com ambos os nomes
		...
		for (const auto &altName : names) {
			if (!g_monsters().tryAddMonsterType(altName, monsterType)) {
				lua_pushstring(L, fmt::format("The monster with name {} already registered", altName).c_str());
				lua_error(L);
				return 1;
			}
		}
...
int GameFunctions::luaGameGetMonsterTypeByName(lua_State* L) {
	...
	std::string name = Lua::getString(L, 1);
	name = toLatin1(name);
...
int GameFunctions::luaGameCreateMonster(lua_State* L) {
	...
	std::string name = Lua::getString(L, 1);
	name = toLatin1(name);
	const auto &monster = Monster::createMonster(name);
	if (!monster) {
		lua_pushnil(L);
		return 1;
	}
...
int GameFunctions::luaGameGenerateNpc(lua_State* L) {
	...
	std::string name = Lua::getString(L, 1);
	name = toLatin1(name);
	const auto &npc = Npc::createNpc(name);
	if (!npc) {
		lua_pushnil(L);
		return 1;
	} else {
