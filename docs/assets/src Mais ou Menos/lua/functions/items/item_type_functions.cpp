...
void ItemTypeFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "ItemType", "getSlotType", ItemTypeFunctions::luaItemTypeGetSlotType);

	ItemClassificationFunctions::init(L);
}

int ItemTypeFunctions::luaItemTypeGetSlotType(lua_State* L) {
    // Verifica se o argumento é um ItemType válido
    if (!lua_isuserdata(L, 1)) {
        lua_pushnil(L);
        return 1;
    }
    void* ud = luaL_testudata(L, 1, "ItemType");
    if (!ud) {
        lua_pushnil(L);
        return 1;
    }
    const auto& itemType = *static_cast<std::shared_ptr<ItemType>*>(ud);
    if (!itemType) {
        lua_pushnil(L);
        return 1;
    }
    if (itemType->slotPosition & SLOTP_TWO_HAND) {
        lua_pushstring(L, "two-handed");
    } else {
        lua_pushstring(L, "one-handed");
    }
    return 1;
}
