...
void ItemFunctions::init(lua_State* L) {
	...
	// MCR – Atributos de itens para o SPA
	Lua::registerMethod(L, "Item", "getElementalBond", ItemFunctions::luaItemGetElementalBond);
	Lua::registerMethod(L, "Item", "getMagicLevelPoints", ItemFunctions::luaItemGetMagicLevelPoints);
	Lua::registerMethod(L, "Item", "getSkillFist", ItemFunctions::luaItemGetSkillFist);
	Lua::registerMethod(L, "Item", "getSkillSword", ItemFunctions::luaItemGetSkillSword);
	Lua::registerMethod(L, "Item", "getSkillAxe", ItemFunctions::luaItemGetSkillAxe);
	Lua::registerMethod(L, "Item", "getSkillClub", ItemFunctions::luaItemGetSkillClub);
	Lua::registerMethod(L, "Item", "getSkillDistance", ItemFunctions::luaItemGetSkillDistance);
	Lua::registerMethod(L, "Item", "getSkillShield", ItemFunctions::luaItemGetSkillShield);
	Lua::registerMethod(L, "Item", "getCriticalHitChance", ItemFunctions::luaItemGetCriticalHitChance);
	Lua::registerMethod(L, "Item", "getCriticalHitDamage", ItemFunctions::luaItemGetCriticalHitDamage);
	Lua::registerMethod(L, "Item", "getLifeLeechAmount", ItemFunctions::luaItemGetLifeLeechAmount);
	Lua::registerMethod(L, "Item", "getLifeLeechChance", ItemFunctions::luaItemGetLifeLeechChance);
	Lua::registerMethod(L, "Item", "getManaLeechAmount", ItemFunctions::luaItemGetManaLeechAmount);
	Lua::registerMethod(L, "Item", "getManaLeechChance", ItemFunctions::luaItemGetManaLeechChance);
	Lua::registerMethod(L, "Item", "getReflectDamage", ItemFunctions::luaItemGetReflectDamage);
	Lua::registerMethod(L, "Item", "getShootType", ItemFunctions::luaItemGetShootType);
	Lua::registerMethod(L, "Item", "isStackable", ItemFunctions::luaItemIsStackable);
	Lua::registerMethod(L, "Item", "isFluidContainer", ItemFunctions::luaItemIsFluidContainer);
	Lua::registerMethod(L, "Item", "getWeaponType", ItemFunctions::luaItemGetWeaponType);
	Lua::registerMethod(L, "Item", "getLightLevel", ItemFunctions::luaItemGetLightLevel);
	Lua::registerMethod(L, "Item", "getLightColor", ItemFunctions::luaItemGetLightColor);
	Lua::registerMethod(L, "Item", "getDecayTo", ItemFunctions::luaItemGetDecayTo);
	Lua::registerMethod(L, "Item", "getDecayTime", ItemFunctions::luaItemGetDecayTime);
	Lua::registerMethod(L, "Item", "isShowDuration", ItemFunctions::luaItemIsShowDuration);
	Lua::registerMethod(L, "Item", "getRange", ItemFunctions::luaItemGetRange);
	...
	Lua::registerMethod(L, "Item", "getArmor", ItemFunctions::luaItemGetArmor);
	Lua::registerMethod(L, "Item", "getSpeed", ItemFunctions::luaItemGetSpeed);
	Lua::registerMethod(L, "Item", "getHealthGain", ItemFunctions::luaItemGetHealthGain);
	Lua::registerMethod(L, "Item", "getManaGain", ItemFunctions::luaItemGetManaGain);
	Lua::registerMethod(L, "Item", "getTransformEquipTo", ItemFunctions::luaItemGetTransformEquipTo);
	Lua::registerMethod(L, "Item", "getSlotPosition", ItemFunctions::luaItemGetSlotPosition);
	Lua::registerMethod(L, "Item", "getAmmoType", ItemFunctions::luaItemGetAmmoType);
	Lua::registerMethod(L, "Item", "getElementType", ItemFunctions::luaItemGetElementType);
...
int ItemFunctions::luaItemGetElementType(lua_State* L) {
    ...
    if (item) {
        ...
        if (it.abilities && it.abilities->elementType != COMBAT_NONE) {
            Lua::pushNumber(L, static_cast<int>(it.abilities->elementType));
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetAmmoType(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.ammoType));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetRange(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.shootRange));  // correção: shootRange
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetAttack(lua_State* L) {
    ...
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.attack));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetDefense(lua_State* L) {
    ...
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.defense));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetArmor(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.armor));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetSpeed(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.speed));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetHealthGain(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->healthGain);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetManaGain(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->manaGain);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetTransformEquipTo(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.transformEquipTo));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetSlotPosition(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.slotPosition));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemIsStackable(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushBoolean(L, it.stackable);
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemIsFluidContainer(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushBoolean(L, it.isFluidContainer());
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetWeaponType(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.weaponType));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetLightLevel(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.lightLevel));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetLightColor(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.lightColor));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetDecayTo(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.decayTo));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetDecayTime(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.decayTime));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemIsShowDuration(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushBoolean(L, it.showDuration);
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetShootType(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.shootType));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetElementalBond(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        Lua::pushNumber(L, static_cast<int>(it.elementalBond));
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int ItemFunctions::luaItemGetMagicLevelPoints(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->stats[STAT_MAGICPOINTS]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetSkillFist(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_FIST]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetSkillSword(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_SWORD]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetSkillAxe(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_AXE]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetSkillClub(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_CLUB]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetSkillDistance(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_DISTANCE]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetSkillShield(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_SHIELD]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetCriticalHitChance(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_CRITICAL_HIT_CHANCE]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetCriticalHitDamage(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_CRITICAL_HIT_DAMAGE]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetLifeLeechAmount(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_LIFE_LEECH_AMOUNT]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetLifeLeechChance(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_LIFE_LEECH_CHANCE]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetManaLeechAmount(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_MANA_LEECH_AMOUNT]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetManaLeechChance(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->skills[SKILL_MANA_LEECH_CHANCE]);
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
...
int ItemFunctions::luaItemGetReflectDamage(lua_State* L) {
    const auto &item = Lua::getUserdataShared<Item>(L, 1, "Item");
    if (item) {
        const auto &it = Item::items[item->getID()];
        if (it.abilities) {
            Lua::pushNumber(L, it.abilities->reflectFlat[0]); // índice de COMBAT_PHYSICALDAMAGE
        } else {
            Lua::pushNumber(L, 0);
        }
    } else {
