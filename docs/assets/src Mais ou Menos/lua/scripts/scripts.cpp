	...
	for (const auto &entry : std::filesystem::recursive_directory_iterator(dir)) {
		...
		if (std::string disable("#");
			...
			if (g_configManager().getBoolean(SCRIPTS_CONSOLE_LOGS)) {
				g_logger().info("[script]: {} [desabilitado]", realPath.filename().string());
			}
		...
		if (isLib || (fileFolderView != "lib" && fileFolderView != "events")) {
			...
			if (g_configManager().getBoolean(SCRIPTS_CONSOLE_LOGS)) {
				...
				if (lastDirectory.empty() || lastDirectory != scriptFolderView) {
					...
					g_logger().info("Carregando pasta: [{}]", realPath.parent_path().filename().string());
				}
		...
		if (g_configManager().getBoolean(SCRIPTS_CONSOLE_LOGS)) {
			if (!reload) {
				g_logger().info("[script carregado]: {}", realPath.filename().string());
			} else {
				g_logger().info("[script recarregado]: {}", realPath.filename().string());
			}
		}
	}
