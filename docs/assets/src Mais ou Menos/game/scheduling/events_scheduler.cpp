...
bool EventsScheduler::loadScheduleEventFromJson() {
	...
	if (!file.is_open()) {
		g_logger().error("{} - N\343o \351 poss\355vel abrir o arquivo '{}'", __FUNCTION__, folder);
		consoleHandlerExit();
		return false;
	}
	...
	try {
		...
		g_logger().error("{} - Erro de an\341lise JSON n\343o arquivo '{}': {}", __FUNCTION__, folder, e.what());
		consoleHandlerExit();
		return false;
	}
	...
	for (const auto &event : eventsJson["events"]) {
		...
		if (!event.contains("startdate") || !event.contains("enddate")) {
			g_logger().warn("{} - 'startdate' ou 'enddate' ausente para o evento '{}'", __FUNCTION__, eventName);
			continue;
		}
		...
		if (!parseDateTime(event["startdate"].get<std::string>(), startHour, startTime) || !parseDateTime(event["enddate"].get<std::string>(), endHour, endTime)) {
			g_logger().warn("{} - Formato de data ou hora inv\341lido para evento '{}'", __FUNCTION__, eventName);
			continue;
		}
		if (endTime < startTime) {
			g_logger().warn("{} - O hor\341rio de t\351rmino do evento '{}' \351 anterior ao hor\341rio de in\355cio", __FUNCTION__, eventName);
			continue;
		}
		...
		if (!eventScript.empty()) {
			if (loadedScripts.contains(eventScript)) {
				g_logger().warn("{} - A declara\347\343o do script '{}' est\341 duplicada em '{}'", __FUNCTION__, eventScript, folder);
				continue;
			}
			...
			if (!std::filesystem::exists(filePath) || !std::filesystem::is_regular_file(filePath)) {
				g_logger().warn("{} - N\343o \351 poss\355vel encontrar o arquivo de script '{}'", __FUNCTION__, filePath.string());
				return false;
			}
		...
		for (const auto &[duplicateEventName, rates] : duplicatedRates) {
			if (!rates.empty()) {
				...
				g_logger().warn("{} - Os eventos '{}' e '{}' t\352m as mesmas taxas [{}] n\343o mesmo dia.", __FUNCTION__, eventName, duplicateEventName, ratesString);
			}
		}
	...
	for (const auto &event : eventScheduler) {
		if (now >= event.startTime && now <= event.endTime) {
			g_logger().info("EventScheduler ativo: {}", event.name);
		}
	}
	return true;
}
...
bool EventsScheduler::loadScheduleEventFromXml() {
	...
	for (const auto &eventNode : doc.child("events").children()) {
		...
		if (!eventScript.empty() && loadedScripts.contains(eventScript)) {
			g_logger().warn("{} - Declara\347\343o de script '{}' em duplicado 'data/XML/events.xml'.", __FUNCTION__, eventScript);
			continue;
		}
		...
		for (const auto &[existingName, rates] : eventsOnSameDay) {
			...
			if (!modifiedRates.empty()) {
				...
				g_logger().warn("{} - Os eventos '{}' e '{}' t\352m as mesmas taxas [{}] n\343o mesmo dia.", __FUNCTION__, eventNode.attribute("name").as_string(), existingName.c_str(), ratesString);
			}
		}
