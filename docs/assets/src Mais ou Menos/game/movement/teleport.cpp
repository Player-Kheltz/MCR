...
void Teleport::addThing(int32_t, const std::shared_ptr<Thing> &thing) {
	...
	if (destPos.x == 0 && destPos.y == 0 && destPos.z == 0) {
		...
		g_logger().debug("[Teleport:addThing] Ignorando o teletransporte com destPos inv\341lido (0, 0, 0) n\343o bloco {}, para criatura: {}", getPosition().toString(), thingCreatureName);
		return;
	}
	...
	if (checkInfinityLoop(destTile)) {
		...
		g_logger().warn("[Teleport:addThing] - Teleporte de loop infinito na posi\347\343o: {}", pos.toString());
		return;
	}
