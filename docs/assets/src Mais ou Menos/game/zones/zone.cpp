...
std::shared_ptr<Zone> Zone::addZone(const std::string &name, uint32_t zoneID /* = 0 */) {
	if (name == "default") {
		g_logger().error("O nome da zona {} est\341 reservado", name);
		return nullZone;
	}
	...
	if (zones[name]) {
		g_logger().error("A zona {} j\341 existe", name);
		return nullZone;
	}
...
std::vector<std::shared_ptr<Zone>> Zone::getZones(const Position position) {
	...
	if (duration > 100) {
		g_logger().warn("Zonas {} listadas para a posi\347\343o {} em {} milissegundos", result.size(), position.toString(), duration);
	}
	return result;
}
...
std::vector<std::shared_ptr<Zone>> Zone::getZones() {
	...
	auto duration = bm_getZones.duration();
	if (duration > 100) {
		g_logger().warn("Zonas {} listadas em {} milissegundos", result.size(), duration);
	}
	return result;
}
...
void Zone::refresh() {
	...
	g_logger().trace("Zona atualizada '{}' em {} milissegundos", name, bm_refresh.duration());
}
