...
void IOMapSerialize::loadHouseItems(Map* map) {
	...
	for (const auto &bed : bedsToCheck) {
		...
		if (!isComplete) {
			...
			g_logger().warn("[BedFix] Bug dom\351stico detectado em {}, corrigindo cama.", pos);
	...
	g_logger().info("Itens da casa carregados em {} milissegundos", bm_context.duration());
}
...
bool IOMapSerialize::saveHouseItems() {
	...
	if (!success) {
		g_logger().error("[{}] Ocorreu um erro ao salvar casas", __FUNCTION__);
	}
...
bool IOMapSerialize::loadContainer(PropStream &propStream, const std::shared_ptr<Container> &container) {
	while (container->serializationCount > 0) {
		if (!loadItem(propStream, container)) {
			g_logger().warn("Erro de desserializa\347\343o para item de cont\352iner: {}", container->getID());
			return false;
		}
	...
	if (iType.isBed() || iType.movable || !tile || iType.isCarpet() || iType.isTrashHolder()) {
		...
		if (item) {
			if (item->unserializeAttr(propStream)) {
				...
				g_logger().warn("Erro de desserializa\347\343o em {}", id);
...
bool IOMapSerialize::loadHouseInfo() {
	...
	do {
		...
		if (isTransferOnRestart && newOwner >= 0) {
			...
			if (newOwner == 0) {
				g_logger().debug("Removendo o ID da casa '{}' propriet\341rio", houseId);
				house->setOwner(0);
			} else {
				g_logger().debug("Configurando o ID da casa '{}' propriet\341rio para o GUID do jogador '{}'", houseId, newOwner);
				house->setOwner(newOwner);
			}
		} else if (state == CyclopediaHouseState::Available && timeNow > bidEndDate && bidder > 0) {
			g_logger().debug("[BID] - Definir o ID da casa '{}' do propriet\341rio para o GUID do jogador '{}'", houseId, bidder);
			...
			g_logger().debug("[MOVE OUT] - Removendo o ID da casa '{}' propriet\341rio", houseId);
			g_game().setTransferPlayerHouseItems(houseId, owner);
			house->setOwner(0);
			bidEndDate = 0;
		} else {
...
bool IOMapSerialize::saveHouseInfo() {
	...
	if (!success) {
		g_logger().error("[{}] Ocorreu um erro ao salvar as informa\347\365es das casas", __FUNCTION__);
	}
