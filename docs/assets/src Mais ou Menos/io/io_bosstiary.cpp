...
void IOBosstiary::loadBoostedBoss() {
	...
	if (!result) {
		g_logger().error("[{}] Falha ao detectar banco de dados de chefes aprimorados. (C\323DIGO 01)", __FUNCTION__);
		return;
	}

	const auto &bossMap = getBosstiaryMap();
	if (bossMap.size() <= 1) {
		g_logger().error("[{}] N\343o \351 poss\355vel criar um chefe boostado com apenas um chefe cadastrado. (C\323DIGO 02)", __FUNCTION__);
		return;
	}
	...
	if (!result) {
		g_logger().warn("[{}] Nenhum chefe otimizado encontrado em g_database(). Um novo ser\341 selecionado.", __FUNCTION__);
	} else {
		...
		if (date == today) {
			...
			g_logger().info("Boss Fortalecido {}", bossName);
			return;
		}
	}
	...
	if (bossInfo.empty()) {
		g_logger().error("Falha ao impulsionar um chefe. N\343o h\341 nenhum chefe registrado na Raridade Arquiinimigo.");
		return;
	}
	...
	if (!database.executeQuery(query)) {
		g_logger().error("[{}] Falha ao detectar banco de dados de chefes aprimorados. (C\323DIGO 03)", __FUNCTION__);
		return;
	}
	...
	if (!database.executeQuery(query)) {
		g_logger().error("[{}] Falha ao redefinir o slot 1 do chefe selecionado pelos jogadores. (C\323DIGO 03)", __FUNCTION__);
	}
	...
	if (!database.executeQuery(query)) {
		g_logger().error("[{}] Falha ao redefinir o slot 2 do chefe selecionado pelos jogadores. (C\323DIGO 03)", __FUNCTION__);
		return;
	}
...
std::shared_ptr<MonsterType> IOBosstiary::getMonsterTypeByBossRaceId(uint16_t raceId) const {
	for ([[maybe_unused]] const auto &[bossRaceId, bossName] : getBosstiaryMap()) {
		if (bossRaceId == raceId) {
			...
			if (!monsterType) {
				g_logger().error("[{}] Chefe com id {} n\343o encontrado n\343o mapa do chefe", __FUNCTION__, raceId);
				continue;
			}
...
void IOBosstiary::addBosstiaryKill(const std::shared_ptr<Player> &player, const std::shared_ptr<MonsterType> &mtype, uint32_t amount /*= 1*/) const {
	...
	if (value != 1 && newBossLevel == 2) {
		...
		std::string podiumMessage = "Congratulations! You have reached the Expertise level for your first boss. "
									"Como recompensa, um Podium of Vigor gratuito foi enviado para a caixa de entrada da sua Loja. "
									"Voc\352 pode colocar este p\363dio em qualquer casa que possua com este personagem. "
									"usar-o para exibir chefes para os quais voc\352 atingiu pelo menos o n\355vel de Especializa\347\343o.";
...
std::vector<uint16_t> IOBosstiary::getBosstiaryFinished(const std::shared_ptr<Player> &player, uint8_t level /* = 1*/) const {
	...
	for (const auto &[bossId, bossName] : getBosstiaryMap()) {
		...
		if (!mType) {
			g_logger().error("[{}] Chefe com id {} e nome {} n\343o encontrado n\343o mapa do chefe", __FUNCTION__, bossId, bossName);
			continue;
		}
		...
		if (it != levelInfos.end()) {
			...
			g_logger().warn("[{}] chefe com id {} e nome {} n\343o encontrado em bossRace", __FUNCTION__, bossId, bossName);
		}
	}
...
uint8_t IOBosstiary::getBossCurrentLevel(const std::shared_ptr<Player> &player, uint16_t bossId) const {
	...
	if (auto it = levelInfos.find(bossRace);
		...
		g_logger().warn("[{}] chefe com id {} e nome {} n\343o encontrado em bossRace", __FUNCTION__, bossId, mType->name);
	}
