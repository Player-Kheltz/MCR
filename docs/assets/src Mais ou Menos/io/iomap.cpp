...
void IOMap::loadMap(Map* map, const Position &pos) {
	...
	if (!stream.startNode()) {
		throw IOMapException("Não foi possível ler o n\363 do mapa.");
	}
	...
	if (version > 5) {
		throw IOMapException("Vers\343o desconhecida do OTBM detectada.");
	}

	if (majorVersionItems < 3) {
		throw IOMapException("Este mapa precisa ser atualizado usando a vers\343o mais recente do editor de mapas para poder carregar corretamente.");
	}
	...
	g_logger().debug("Mapa carregado {} ({}x{}) em {} milissegundos", map->path.filename().string(), map->width, map->height, bm_mapLoad.duration());
}
	...
	while (stream.startNode(OTBM_TILE_AREA)) {
		...
		while (stream.startNode()) {
			...
			if (tileType != OTBM_HOUSETILE && tileType != OTBM_TILE) {
				throw IOMapException("Não foi possível ler o n\363 do tipo de bloco.");
			}
			...
			if (tileType == OTBM_HOUSETILE) {
				...
				if (!map.houses.addHouse(tile->houseId)) {
					throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] Não foi possível criar o ID da casa: {}", x, y, z, tile->houseId));
				}
			}
				...
				if (!tile->isHouse() || !iType.isBed()) {
					...
					if (tile->isHouse() && iType.movable) {
						g_logger().warn("[IOMap::loadMap] - "
						                "Item m\363vel com ID: {}, em casa: {}, "
						                "na posi\347\343o: x {}, y {}, z {}",
				...
				switch (type) {
					case OTBM_ITEM: {
						...
						if (!item->unserializeItemNode(stream, x, y, z)) {
							throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] Falha ao carregar o item {}, Tipo de n\363.", x, y, z, id));
						}
					...
					case OTBM_TILE_ZONE: {
						...
						for (uint16_t i = 0; i < zoneCount; ++i) {
							...
							if (!zoneId) {
								throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] ID de zona inv\341lido.", x, y, z));
							}
					...
					default:
						throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] Não foi possível ler o n\363 do item/zona.", x, y, z));
				}

				if (!stream.endNode()) {
					throw IOMapException(fmt::format("[x:{}, y:{}, z:{}] Não foi possível finalizar o n\363.", x, y, z));
				}
			}
		...
		if (!stream.endNode()) {
			throw IOMapException("Não foi possível encerrar o n\363.");
		}
	}
}
...
void IOMap::parseTowns(FileStream &stream, Map &map) {
	if (!stream.startNode(OTBM_TOWNS)) {
		throw IOMapException("Não foi possível ler o n\363 de cidades.");
	}
...
void IOMap::parseWaypoints(FileStream &stream, Map &map) {
	if (!stream.startNode(OTBm_globalPathS)) {
		throw IOMapException("Não foi possível ler o n\363 de waypoints.");
	}

	while (stream.startNode(OTBm_globalPath)) {
