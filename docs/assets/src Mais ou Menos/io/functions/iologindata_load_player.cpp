	...
	try {
		do {
			...
			try {
				...
				g_logger().warn("[{}] - Exce\347\343o durante a cria\347\343o ou desserializa\347\343o do item: {}", __FUNCTION__, e.what());
				continue;
			}
		} while (result->next());
	} catch (const std::exception &e) {
		g_logger().error("[{}] - Exce\347\343o geral durante o carregamento do item: {}", __FUNCTION__, e.what());
	}
}
...
bool IOLoginDataLoad::preLoadPlayer(const std::shared_ptr<Player> &player, const std::string &name) {
	...
	g_logger().debug("[DEBUG] preLoadPlayer chamado com nome: {}", name);
	std::string utf8Name = latin1ToUtf8(name);
	g_logger().debug("[DEBUG] preLoadPlayer utf8Name: {}", utf8Name);
	query << "SELECT `id`, `account_id`, `group_id`, `deletion` FROM `players` WHERE `name` = " << db.escapeString(utf8Name);
	...
	if (!group) {
		g_logger().error("O jogador {} tem o ID do grupo {} que n\343o existe", player->name, result->getNumber<uint16_t>("group_id"));
		return false;
	}
	...
	if (!player->setAccount(accountId)) {
		g_logger().error("O jogador {} tem o ID da conta {} que n\343o existe", player->name, accountId);
		return false;
	}

	auto [coins, error] = player->account->getCoins(CoinType::Normal);
	if (error != AccountErrors_t::Ok) {
		g_logger().error("Falha ao obter moedas para o jogador {}, erro {}", player->name, static_cast<uint8_t>(error));
		return false;
	}
	...
	if (errorT != AccountErrors_t::Ok) {
		g_logger().error("Falha ao obter moedas transfer\355veis para o jogador {}, erro {}", player->name, static_cast<uint8_t>(errorT));
		return false;
	}
...
bool IOLoginDataLoad::loadPlayerBasicInfo(const std::shared_ptr<Player> &player, const DBResult_ptr &result) {
	if (!result || !player) {
		g_logger().warn("[{}] - Jogador ou Resultado nullptr", __FUNCTION__);
		return false;
	}
	...
	player->setGUID(result->getNumber<uint32_t>("id"));
	player->setName(toLatin1(result->getString("name")));
	...
	if (!town) {
		...
		if (thaisTown) {
			...
			g_logger().warn("Atribu\355da a cidade 'Thais' ao jogador {}", player->name);
		} else {
			for (const auto &[townId, currentTown] : g_game().map.towns.getTowns()) {
				if (townId != 0 && currentTown) {
					player->town = currentTown;
					g_logger().warn("Atribu\355da a primeira cidade v\341lida {} (id: {}) ao jogador {}", currentTown->getName(), townId, player->name);
				}
			}

			if (!player->town) {
				g_logger().error("O jogador {} possui um ID de cidade {} inv\341lido. Nenhuma cidade v\341lida encontrada para atribuir.", player->name, result->getNumber<uint16_t>("town_id"));
				return false;
			}
		}
	} else {
...
void IOLoginDataLoad::loadPlayerDefaultOutfit(const std::shared_ptr<Player> &player, const DBResult_ptr &result) {
	...
	if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && player->defaultOutfit.lookType != 0 && !g_game().isLookTypeRegistered(player->defaultOutfit.lookType)) {
		g_logger().warn("[{}] Uma criatura do tipo looktype n\343o registrada com id '{}' foi bloqueada para evitar travamento do cliente.", __FUNCTION__, player->defaultOutfit.lookType);
		return;
	}
	...
	if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && player->defaultOutfit.lookFamiliarsType != 0 && !g_game().isLookTypeRegistered(player->defaultOutfit.lookFamiliarsType)) {
		g_logger().warn("[{}] Uma criatura do tipo looktype n\343o registrada com id '{}' foi bloqueada para evitar travamento do cliente.", __FUNCTION__, player->defaultOutfit.lookFamiliarsType);
		return;
	}
	...
	if ((result = db.storeQuery(query.str()))) {
		do {
			...
			if (wareId > 0 && wareId != itemType.id) {
				g_logger().warn("[{}] - O ID do item {} \351 um item de mercadoria, para o jogador: {}, pulando.", __FUNCTION__, itemId, player->getName());
				continue;
			}
...
void IOLoginDataLoad::loadPlayerBestiaryCharms(const std::shared_ptr<Player> &player, DBResult_ptr result) {
	...
	if ((result = db.storeQuery(query.str()))) {
		...
		for (uint8_t id = magic_enum::enum_value<charmRune_t>(1); id <= magic_enum::enum_count<charmRune_t>(); id++) {
			...
			g_logger().debug("O jogador {} carregou o ID de charme {} com raceId {} e n\355vel {}", player->name, id, raceId, tier);
		}
...
void IOLoginDataLoad::loadPlayerInstantSpellList(const std::shared_ptr<Player> &player, DBResult_ptr result) {
	if (!player) {
		g_logger().warn("[{}] - Jogador nullptr", __FUNCTION__);
		return;
	}
...
void IOLoginDataLoad::loadPlayerInboxItems(const std::shared_ptr<Player> &player, DBResult_ptr result) {
	...
	if ((result = g_database().storeQuery(query))) {
		...
		if (!playerInbox) {
			g_logger().warn("[{}] - Caixa de entrada do jogador nullptr", __FUNCTION__);
			return;
		}
...
void IOLoginDataLoad::loadPlayerForgeHistory(const std::shared_ptr<Player> &player) {
	...
	if (!player->forgeHistory().load()) {
		g_logger().warn("[{}] - Falha ao carregar o hist\363rico do forge para o jogador: {}", __FUNCTION__, player->getName());
	}
}

void IOLoginDataLoad::loadPlayerBosstiary(const std::shared_ptr<Player> &player, DBResult_ptr result) {
	if (!result) {
		g_logger().warn("[{}] - Resultado nullptr", __FUNCTION__);
		return;
	}
