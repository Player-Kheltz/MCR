...
bool IOLoginDataSave::saveItems(const std::shared_ptr<Player> &player, const ItemBlockList &itemList, DBInsert &query_insert, PropWriteStream &propWriteStream) {
	...
	for (const auto &it : itemList) {
		...
		if (!query_insert.addRow(ss)) {
			g_logger().error("Erro ao adicionar linha \340 consulta.");
			return false;
		}
	}
	...
	while (!queue.empty()) {
		...
		for (auto &item : container->getItemList()) {
			...
			if (!query_insert.addRow(ss)) {
				g_logger().error("Erro ao adicionar linha para consultar o item do cont\352iner. ");
				return false;
			}
		}
	...
	if (!query_insert.execute()) {
		g_logger().error("Erro ao executar a consulta.");
		return false;
	}
	return true;
}
...
bool IOLoginDataSave::savePlayerFirst(const std::shared_ptr<Player> &player) {
	...
	if (!result) {
		g_logger().warn("[IOLoginData::savePlayer] - Error for select result query from player: {}", latin1ToUtf8(player->getName()));
		return false;
	}
	...
	query << "`name` = " << db.escapeString(latin1ToUtf8(player->name)) << ",";
...
bool IOLoginDataSave::savePlayerStash(const std::shared_ptr<Player> &player) {
	...
	for (const auto &[itemId, itemCount] : player->getStashItems()) {
		...
		if (wareId > 0 && wareId != itemType.id) {
			g_logger().warn("[{}] - O ID do item {} \351 um item de mercadoria, para o jogador: {}, pulando.", __FUNCTION__, itemId, latin1ToUtf8(player->getName()));
			continue;
		}
...
bool IOLoginDataSave::savePlayerBestiarySystem(const std::shared_ptr<Player> &player) {
	...
	for (uint8_t id = magic_enum::enum_value<charmRune_t>(1); id <= magic_enum::enum_count<charmRune_t>(); id++) {
		...
		g_logger().debug("O jogador {} salvou raceId {} e n\355vel {} para encantar Id {}", player->name, charm.raceId, charm.tier, id);
	}
	size_t size;
	...
	if (!db.executeQuery(query.str())) {
		g_logger().warn("[IOLoginData::savePlayer] - Error saving bestiary data from player: {}", latin1ToUtf8(player->getName()));
		return false;
	}
	return true;
}
...
bool IOLoginDataSave::savePlayerItem(const std::shared_ptr<Player> &player) {
	...
	if (!db.executeQuery(query.str())) {
		g_logger().warn("[IOLoginData::savePlayer] - Error delete query 'player_items' from player: {}", latin1ToUtf8(player->getName()));
		return false;
	}
	...
	if (!saveItems(player, itemList, itemsQuery, propWriteStream)) {
		g_logger().warn("[IOLoginData::savePlayer] - Failed for save items from player: {}", latin1ToUtf8(player->getName()));
		return false;
	}
	return true;
}
	...
	if (g_configManager().getBoolean(PREY_ENABLED)) {
		...
		for (uint8_t slotId = PreySlot_First; slotId <= PreySlot_Last; slotId++) {
			if (const auto &slot = player->getPreySlotById(static_cast<PreySlot_t>(slotId))) {
				...
				if (!db.executeQuery(query.str())) {
					g_logger().warn("[IOLoginData::savePlayer] - Error saving prey slot data from player: {}", latin1ToUtf8(player->getName()));
					return false;
				}
			}
		}
	...
	if (g_configManager().getBoolean(TASK_HUNTING_ENABLED)) {
		...
		for (uint8_t slotId = PreySlot_First; slotId <= PreySlot_Last; slotId++) {
			if (const auto &slot = player->getTaskHuntingSlotById(static_cast<PreySlot_t>(slotId))) {
				...
				if (!db.executeQuery(query.str())) {
					g_logger().warn("[IOLoginData::savePlayer] - Error saving task hunting slot data from player: {}", latin1ToUtf8(player->getName()));
					return false;
				}
			}
		}
