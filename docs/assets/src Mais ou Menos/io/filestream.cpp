...
uint32_t FileStream::size() const {
	...
	if (size > std::numeric_limits<uint32_t>::max()) {
		g_logger().error("O tamanho do arquivo excede o intervalo uint32_t");
		return {};
	}
...
bool FileStream::read(T &ret, bool escape) {
	static_assert(std::is_trivially_copyable_v<T>, "O tipo T deve ser trivialmente copi\341vel");

	const auto size = sizeof(T);

	if (m_pos + size > m_data.size()) {
		g_logger().error("Falha na leitura");
		return false;
	}
...
uint8_t FileStream::getU8() {
	...
	if (m_pos + 1 > m_data.size()) {
		g_logger().error("Falha ao obterU8");
		return {};
	}
