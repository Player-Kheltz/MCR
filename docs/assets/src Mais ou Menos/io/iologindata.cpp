...
bool IOLoginData::gameWorldAuthentication(const std::string &accountDescriptor, const std::string &password, std::string &characterName, uint32_t &accountId, bool oldProtocol, const uint32_t ip) {
	...
	if (AccountErrors_t::Ok != account.load()) {
		g_logger().error("Não foi possível carregar a conta [{}].", account.getDescriptor());
		return false;
	}
	...
	if (!g_accountRepository().getCharacterByAccountIdAndName(account.getID(), characterName)) {
		g_logger().warn("IP [{}] tentando se conectar a outro personagem da conta", convertIPToString(ip));
		return false;
	}

	if (AccountErrors_t::Ok != account.load()) {
		g_logger().error("Falha ao carregar conta [{}]", accountDescriptor);
		return false;
	}

	auto [players, result] = account.getAccountPlayers();
	if (AccountErrors_t::Ok != result) {
		g_logger().error("Falha ao carregar conta [{}] jogadores", accountDescriptor);
		return false;
	}

	if (players[characterName] != 0) {
		g_logger().error("Conta [{}] jogador [{}] n\343o encontrada ou exclu\355da.", accountDescriptor, characterName);
		return false;
	}
...
bool IOLoginData::loadPlayer(const std::shared_ptr<Player> &player, const DBResult_ptr &result, bool disableIrrelevantInfo /* = false*/) {
	if (!result || !player) {
		...
		g_logger().warn("[{}] - {} \351 nullptr", __FUNCTION__, nullptrType);
		return false;
	}
	...
	try {
		...
		if (!IOLoginDataLoad::loadPlayerBasicInfo(player, result)) {
			g_logger().warn("[{}] - Falha ao carregar informa\347\365es b\341sicas do jogador", __FUNCTION__);
			return false;
		}
		...
		g_logger().warn("[{}] Erro ao carregar o jogador: {}", __FUNCTION__, error.what());
		return false;
	} catch (const std::exception &e) {
		g_logger().warn("[{}] Erro ao carregar o jogador: {}", __FUNCTION__, e.what());
		return false;
	}
}
...
bool IOLoginData::savePlayer(const std::shared_ptr<Player> &player) {
	try {
		...
		if (!success) {
			g_logger().error("[{}] Ocorreu um erro ao salvar o jogador", __FUNCTION__);
		}

		return success;
	} catch (const DatabaseException &e) {
		g_logger().error("[{}] Ocorreu uma exce\347\343o: {}", __FUNCTION__, e.what());
	}
...
bool IOLoginData::savePlayerGuard(const std::shared_ptr<Player> &player) {
	if (!player) {
		throw DatabaseException("Jogador nullptr em fun\347\343o: " + std::string(__FUNCTION__));
	}

	if (!IOLoginDataSave::savePlayerFirst(player)) {
		throw DatabaseException("[" + std::string(__FUNCTION__) + "] - Falha ao salvar o jogador primeiro: " + player->getName());
	}
...
std::string IOLoginData::getNameByGuid(uint32_t guid) {
	...
	return toLatin1(result->getString("name"));
}

uint32_t IOLoginData::getGuidByName(const std::string &name) {
    Database &db = Database::getInstance();
    std::string utf8Name = latin1ToUtf8(name);  // <-- converte para UTF‑8

    std::ostringstream query;
    query << "SELECT `id` FROM `players` WHERE `name` = " << db.escapeString(utf8Name);
...
bool IOLoginData::getGuidByNameEx(uint32_t &guid, bool &specialVip, std::string &name) {
    ...
    std::ostringstream query;
    query << "SELECT `name`, `id`, `group_id`, `account_id` FROM `players` WHERE `name` = " << db.escapeString(utf8Name);
    DBResult_ptr result = db.storeQuery(query.str());
    if (!result) {
        return false;
    }

    name = toLatin1(result->getString("name"));  // já estava correto
...
bool IOLoginData::formatPlayerName(std::string &name) {
    ...
    std::ostringstream query;
    query << "SELECT `name` FROM `players` WHERE `name` = " << db.escapeString(utf8Name);
	...
	if (const auto &result = Database::getInstance().storeQuery(query)) {
		...
		do {
			entries.emplace_back(
				...
				toLatin1(result->getString("name")),
...
void IOLoginData::addVIPEntry(uint32_t accountId, uint32_t guid, const std::string &description, uint32_t icon, bool notify) {
	...
	if (!g_database().executeQuery(query)) {
		g_logger().error("Falha ao adicionar entrada VIP para conta {}. CONSULTA: {}", accountId, query.c_str());
	}
}
...
void IOLoginData::editVIPEntry(uint32_t accountId, uint32_t guid, const std::string &description, uint32_t icon, bool notify) {
	...
	if (!g_database().executeQuery(query)) {
		g_logger().error("Falha ao editar a entrada VIP da conta {}. CONSULTA: {}", accountId, query.c_str());
	}
}
...
void IOLoginData::addVIPGroupEntry(uint8_t groupId, uint32_t accountId, const std::string &groupName, bool customizable) {
	...
	if (!g_database().executeQuery(query)) {
		g_logger().error("Falha ao adicionar entrada de Grupo VIP para conta {} e grupo {}. CONSULTA: {}", accountId, groupId, query.c_str());
	}
}
...
void IOLoginData::addGuidVIPGroupEntry(uint8_t groupId, uint32_t accountId, uint32_t guid) {
	...
	if (!g_database().executeQuery(query)) {
		g_logger().error("Falha ao adicionar entrada de grupo VIP guia para conta {}, jogador {} e grupo {}. CONSULTA: {}", accountId, guid, groupId, query.c_str());
	}
}
