...
uint8_t IOMarket::getTierFromDatabaseTable(const std::string &string) {
	...
	if (tier > g_configManager().getNumber(FORGE_MAX_ITEM_TIER)) {
		g_logger().error("{} - Falha ao obter o valor num\351rico {} para o resultado da tabela de camadas", __FUNCTION__, tier);
		return 0;
	}
			...
			if (itemType.stackable) {
				...
				while (tmpAmount > 0) {
					...
					if (g_game().internalAddItem(playerInbox, item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
						g_logger().error("[{}] Ocorreu um erro ao adicionar item com id {} ao jogador {}", __FUNCTION__, itemType.id, player->getName());
