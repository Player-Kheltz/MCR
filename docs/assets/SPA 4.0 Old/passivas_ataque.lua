...
local PASSIVAS_ATAQUE = {
    ...
    [121] = { nome="Dano Crítico", nivelMin=15, postura=FIGHTMODE_ATTACK, chance=10, efeito=function(p,t,d) local extra=math.floor(d*1.0); doTargetCombatHealth(p,t,COMBAT_PHYSICALDAMAGE,extra,extra,CONST_ME_DRAWBLOOD); t:getPosition():sendMagicEffect(CONST_ME_DRAWBLOOD) end },
    [122] = { nome="Lentidão à Distância", nivelMin=15, postura=FIGHTMODE_DEFENSE, chance=20, condicaoEscudo=true, efeito=function(p,t,d) local dist=getCreatureDistance(p,t); if dist>4 then local c=Condition(CONDITION_PARALYZE,CONDITIONID_COMBAT,2000); c:setParameter(CONDITION_PARAM_SPEED,-30); t:addCondition(c); t:getPosition():sendMagicEffect(CONST_ME_SOUND_BLUE) end end },
    [120] = { nome="Dano Crítico", nivelMin=15, postura=FIGHTMODE_ATTACK, chance=12, efeito=function(p,t,d) local extra=math.floor(d*1.5); doTargetCombatHealth(p,t,COMBAT_PHYSICALDAMAGE,extra,extra,CONST_ME_DRAWBLOOD); t:getPosition():sendMagicEffect(CONST_ME_DRAWBLOOD) end },
    ...
    [15]  = { nome="Empurrão Defensivo", nivelMin=15, postura=FIGHTMODE_DEFENSE, chance=15, efeito=function(p,t,d) local dir=Position(p:getPosition()):getDirectionTo(t:getPosition()); if dir then t:knockback(dir); t:getPosition():sendMagicEffect(CONST_ME_HITAREA) end end },
