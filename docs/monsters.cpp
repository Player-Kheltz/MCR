...
static std::string monsters_to_latin1(const std::string &str) {
    bool isUtf8 = true;
    size_t i = 0;
    while (i < str.size()) {
...
bool MonsterDatabase::loadFromLuaDir(const wxString &directory, wxString &error, wxArrayString &warnings) {
    ...
    for (const auto &filePath : luaFiles) {
        ...
        // Converte o nome para Latin-1 (se estiver em UTF-8) para exibição correta
        name = monsters_to_latin1(name);
