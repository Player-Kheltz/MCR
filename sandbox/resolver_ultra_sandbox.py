...
def detectar_sintaxe_python(texto):
    ...
    Se o arquivo INTEIRO for Python (nao tem comentarios Lua), ignora."""
    linhas = texto.split('\n')
    ...
    if not primeira_linha.startswith('--'):
        ...
        linhas_lua = 0
        for linha in linhas:
            if re.search(r'\b(def|class|import)\b', linha.split('--')[0]):
                linhas_python += 1
            if re.match(r'^\s*(local\s+)?function\b|require\b', linha):
                linhas_lua += 1
        # Se tem mais Python que Lua, arquivo e Python legitimo
        if linhas_python >= 3 and linhas_python > linhas_lua:
            return False
    
    # Detecta Python dentro de Lua (o caso REAL)
    for linha in linhas:
