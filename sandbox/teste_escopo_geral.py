"""Teste: pergunta GERAL deve ignorar contexto MCR."""
...
try:
    ...
    has_mcr = 'projeto mcr' in resp.lower() or 'tibia' in resp.lower() or 'canary' in resp.lower()
    print()
    print('RESPOSTA (' + str(len(resp)) + ' chars):')
    print(resp[:800])
    print()
    print('Mencionou MCR: ' + ('SIM (ainda vazando!)' if has_mcr else 'NAO'))
