local internalNpcName = "Sábio Gorthan"
local npcType = Game.createNpcType(internalNpcName)
local npcConfig = {}

npcConfig.name = internalNpcName
npcConfig.description = "Quest NPC - Sábio Gorthan"
...
local function creatureSayCallback(npc, creature, type, message)
    ...
    if MsgContains(message, "mission") or MsgContains(message, "quest") then
        if player:getStorageValue(Storage.Quest.Custom.SÁBIOGORTHAN) < 1 then
    ...
    elseif MsgContains(message, "yes") then
        if npcHandler:getTopic(playerId) == 1 then
            player:setStorageValue(Storage.Quest.Custom.SÁBIOGORTHAN, 1)
        ...
        elseif npcHandler:getTopic(playerId) == 2 then
            if player:getItemCount(3031) >= 1 then
                ...
                player:setStorageValue(Storage.Quest.Custom.SÁBIOGORTHAN, 2)
...
npcHandler:setMessage(MESSAGE_GREET, "Greetings, adventurer! Are you looking for a mission?")
