...
def detectar_sintaxe_python(texto):
    """Detecta codigo Python DENTRO de arquivos Lua.
    Diferencia 'def' Python (def nome:) de 'def' Lua (def = variavel).
    Palavra seguida de espaco = Python. Seguida de . ou = = Lua."""
    linhas = texto.split('\n')
    primeira_linha = linhas[0].strip() if linhas else ''
    
    # So considera Python se keyword for seguida de ESPACO (nao . ou =)
    def e_python(linha):
        # 'def nome:' (Python) vs 'def.HEALTH' ou 'def =' (Lua)
        return bool(re.search(r'\b(def |class |import )', linha.split('--')[0]))
    
    # Se primeira linha nao e comentario Lua, verifica proporcao
    if not primeira_linha.startswith('--'):
        ...
        for linha in linhas:
            if e_python(linha):
                linhas_python += 1
            if re.match(r'^\s*(local\s+)?function\b|require\b', linha):
                linhas_lua += 1
        if linhas_python >= 3 and linhas_python > linhas_lua:
            return False
    
    # Detecta Python dentro de Lua
    for linha in linhas:
        if not e_python(linha):
            continue
        linha_clean = linha.split('--')[0].strip()
        if re.match(r'^\s*(def |class |import )', linha_clean):
            return True
    return False
