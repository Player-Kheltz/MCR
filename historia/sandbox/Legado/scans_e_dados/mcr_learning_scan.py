...
class LearningScan:
    ...
    def gerar_template(self, tipo):
        ...
        # IA gera o template
        prompt = f"""Crie um template Lua para {tipo} usando as funcoes reais do projeto MCR.

Funcoes CONFIRMADAS (usadas em 2+ arquivos):
{chr(10).join(f'- mon:{f}()' for f in funcoes_comuns)}

Funcoes POSSIVEIS (usadas em 1 arquivo):
{chr(10).join(f'- mon:{f}()' for f in funcoes_possiveis)}

Retorne APENAS o template no formato:
-- {tipo}: {{nome}}
local mon = Monster("{{nome}}")
mon:setHealth({{hp}})
mon:setAttack({{atk}})
... (use as funcoes reais)
print("{tipo} {{nome}} carregado.")

Use APENAS funcoes que existem no projeto real."""
